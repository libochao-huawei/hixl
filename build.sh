...
checkopts() {
  ...
  # Process the options
  parsed_args=$(getopt -a -o j:hv -l help,verbose,pkg-type:,examples,cann_3rd_lib_path:,cann-3rd-lib-path:,output_path:,output-path:,build_type:,build-type:,sign-script:,sign_script:,asan,cov,enable_sign,enable-sign -- "$@") || {
    usage
    exit 1
  }
