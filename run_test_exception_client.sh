...
# Client 参数格式（带冒号，is_client = true）：
# device_id local_engine remote_engine tcp_port transfer_mode transfer_op local_comm_res remote_comm_res test_case
# remote_engine 带端口号（格式：IP:PORT），表示 Client 模式

echo "启动 Client 端..."
$BENCHMARK_PROGRAM 0 10.23.144.161 10.23.144.163:17000 17000 $transfer_mode $transfer_op "$local_comm_res" "$remote_comm_res" $test_case
