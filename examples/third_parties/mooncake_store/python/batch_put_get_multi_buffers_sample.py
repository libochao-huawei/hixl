...
def init_mooncake_store(arg) -> MooncakeDistributedStore:
    logging.info(f"Start to init mooncake store for batch put/get multi buffers sample.")
    store = MooncakeDistributedStore()
    port = 12345 + int(arg.device_id)
    store.setup(
        "127.0.0.1:" + str(port),
...
def init_process_group(args):
    logging.info(f"Start to init process group for batch put/get multi buffers sample.")
...
def run_batch_put_get_multi_buffers_sample(store: MooncakeDistributedStore, args):
    ...
    for block_i in range(32):
        ...
        keys.append("hello_" + str(args.device_id) + "_" + str(block_i))
...
if __name__ == "__main__":
    ...
    logging.info(f"Running batch put/get multi buffers sample, schema={args.schema}")
    ...
    run_batch_put_get_multi_buffers_sample(mooncake_store, args)
