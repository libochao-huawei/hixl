...
def init_mooncake_store(args) -> MooncakeDistributedStore:
    logging.info(f"Start to init mooncake store for batch put/get sample.")
...
def init_process_group(args):
    logging.info(f"Start to init process group for batch put/get sample.")
...
def run_batch_put_get_sample(store: MooncakeDistributedStore, args):
    ...
    for block_i in range(32):
        ...
        for key_, result_ in zip(get_keys, results):
            if result_ > 0:
                logging.info(f"Retrieved {key_} : {result_} bytes")
            else:
                logging.info(f"Failed to retrieve {key_}: error {result_}")
    store.close()
...
if __name__ == "__main__":
    ...
    logging.info(f"Running batch put/get sample, shema: {args.schema}")
