...
from mooncake_sample_common import create_parser, setup_environment
...
class BatchPutGet(MooncakeSampleBase):
    def run(self):
        ...
        for block_i in range(32):
            ...
            self._log_results(get_keys, results)
        
        self.cleanup()
    
    def _log_results(self, keys, results):
        for k, sz in zip(keys, results):
            if result_ > 0:
                logging.info(f"Retrieved {k} : {sz} bytes")
            else:
                logging.info(f"Failed to retrieve {k}: error {sz}")
