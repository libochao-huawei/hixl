...
class BandwidthBenchmark(MooncakeSampleBase):
    ...
    def run(self):
        ...
        if self.args.register_size_gb is not None:
            logging.info(
                f"Custom register size specified: {self.args.register_size_gb} GB (stress testing mode)"
            )
    ...
    def _log_startup_info(self):
        logging.info("Starting bandwidth benchmark")
    ...
    def _prepare_buffers(self, block_size_kb):
        ...
        if self.args.register_size_gb is not None:
            tensor_size = (
                int(self.args.register_size_gb * 1024 * 1024 * 1024) + ALIGNMENT * 2
            )
            logging.info(
    ...
    def _create_single_tensor(self, size, fill_value, is_host):
        if is_host:
            return torch.full(
                (size,), fill_value, dtype=torch.int8, pin_memory=True
            ).cpu()
        else:
    ...
    def _register_buffer(self, tensor, buffer_type):
        ...
        if register_size <= 0:
            raise ValueError(
                f"Invalid register size: {register_size}, data_ptr={data_ptr}, aligned={aligned_addr}"
            )
    ...
    def _log_benchmark_config(self, block_size_kb):
        logging.info(f"\n{'=' * 80}")
        logging.info("Benchmark Configuration:")
    ...
    def _run_one_put_multiple_gets(self, block_size_kb, addr, remote_addr):
        ...
        for _ in range(self.args.num_iters):
            get_time = self._timed_get_operation(
                block_size_kb, addr, remote_addr, iter_id
            )
            get_times.append(get_time)

        return {"put_time": put_time, "get_times": get_times}
    ...
    def _compute_bandwidth(self, block_size_kb, metrics):
        ...
        logging.info(
            f"  Put: {put_time:.6f}s => {put_bw:.3f} GB/s ({put_data_gb:.3f} GB)"
        )
        logging.info(
    ...
    def _timed_get_pairwise(self, block_size_kb, addr, remote_addr, iter_id):
        ...
        remote_addrs = self._generate_addrs(
            remote_addr, block_size_kb, self.args.num_blocks
        )
        sizes = [block_size_kb * 1024] * self.args.num_blocks
    ...
    def _print_summary(self, results):
        ...
        for size in sorted(results.keys()):
            r = results[size]
            logging.info(
                f"{size:<15} {r['put_bandwidth']:<15.3f} {r['get_bandwidth']:<15.3f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--block_sizes",
        type=str,
        default="1,4,16,64,144,256,512,1024",
        help="Comma-separated block sizes in KB",
    )
    parser.add_argument(
        "--num_blocks", type=int, default=100, help="Number of blocks per iteration"
    )
    parser.add_argument(
        "--num_iters", type=int, default=10, help="Number of GET iterations after PUT"
    )
    parser.add_argument(
        "--register_size_gb",
        type=float,
