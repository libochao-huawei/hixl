...
// 正常场景3：一个client对一个server建链，注册大量内存，比如1k，10k个，关注注册与传输耗时
...
constexpr uint32_t kArgIndexRegisterNum = 7;
...
int32_t Register(Hixl &hixl_engine, bool is_host, std::vector<MemDesc> &descs, 
                 std::vector<MemHandle> &handles, std::vector<int32_t *> &sources, int32_t num) {
  for (int i = 0; i < num; i++) {
    int32_t *src = nullptr;
    void *tmp = nullptr;
  ...
  MemType mem_type = is_host ? MemType::MEM_HOST : MEM_DEVICE;
  const auto start = std::chrono::steady_clock::now();
  for (int i = 0; i < num; i++) {
...
int32_t RunClient(const char *local_engine, const char *remote_engine, uint16_t tcp_port,
                  const std::string &transfer_mode, std::string op_type, bool is_client, int32_t num) {
  ...
  addresses = tcp_server.ReceiveUint64Array();
  if (addresses.size() != 0) {
    printf("[INFO] Success to receive server mem addresses\n");
  }
  ...
  if (Register(hixl_engine, is_host, descs, handles, sources, num) != 0) {
    printf("[ERROR] Register mem failed.");
    return -1;
  }
  ...
  const auto start = std::chrono::steady_clock::now();
  for (int i = 0; i < num; i ++) {
    if (Transfer(hixl_engine, *sources[i], remote_engine, addresses[i], op_type) != 0) {
  ...
  auto time_cost =
      std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count();
  printf("[INFO] All Memory Tansfer success, time cost: %ld us\n", time_cost);
...
int32_t RunServer(const char *local_engine, const char *remote_engine, uint16_t tcp_port,
                  const std::string &transfer_mode, bool is_client, int32_t num) {
...
int32_t main(int32_t argc, char **argv) {
  ...
  if (argc == kExpectedArgCnt) {
    ...
    register_num = argv[kArgIndexRegisterNum];
