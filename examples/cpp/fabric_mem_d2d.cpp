...
int AllocateBuffer(int32_t device_id, size_t mem_size, void *&va, aclrtDrvMemHandle &pa_handle) {
  ...
  memset_s(host_data, device_id, mem_size);
...
int32_t TransferAndVerify(Hixl &hixl_engine, bool connected, const char *local_engine, const char *remote_engine,
                          MemHandle &handle) {
...
int32_t Run(int32_t device_id, const char *local_engine, const char *remote_engine) {
  ...
  connected = true;
  return TransferAndVerify(hixl_engine, connected, local_engine, remote_engine, handle);
}
