...
inline int ConnectControlServer(const sockaddr_in &remote_addr, const char *remote_ip, uint16_t remote_port,
  ...
  if (retry_interval_us <= 0) {
    printf("[ERROR] Invalid retry interval for %s: %d us\n", message_desc, retry_interval_us);
    return -1;
  }
  if (timeout_sec <= 0) {
    printf("[ERROR] Invalid timeout for %s: %d seconds\n", message_desc, timeout_sec);
    return -1;
  }

  int conn_fd = -1;
  constexpr int32_t kUsecPerSec = 1000000;
  const int64_t timeout_us = static_cast<int64_t>(timeout_sec) * kUsecPerSec;
  for (int64_t elapsed_us = 0; elapsed_us + retry_interval_us <= timeout_us; elapsed_us += retry_interval_us) {
    ...
    CloseFd(conn_fd);
    usleep(static_cast<useconds_t>(retry_interval_us));
  }
