...
以下所有用例的执行命令以A2环境为例演示，在执行前请先确认**两个device之间互通**，例如A3环境一卡双带之间不互通，01不通，23不通，以此类推，需要在执行时将device_id进行替换。可以用hccn_tool按照以下步骤确认两个设备之间的连通性，假设要测试a和b两台设备间的连通性：  
