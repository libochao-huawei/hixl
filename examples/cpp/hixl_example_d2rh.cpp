...
int32_t ParseArgs(int32_t argc, char **argv, int32_t &device_a, int32_t &device_b,
  ...
  if (version == kVersionLegacy && protocol != "roce:device") {
    printf("[ERROR] version 0 only supports roce:device, got %s\n", protocol.c_str());
    return -1;
  }
  bool valid = false;
...
int32_t InitEngine(EngineCtx &ctx, const std::string &protocol, int32_t version, uint8_t fill_value) {
  CHECK_ACL(aclrtSetDevice(ctx.device_id));
  std::map<AscendString, AscendString> options;
  bool need_host_register = true;
...
int32_t Run(int32_t device_a, int32_t device_b, const std::string &protocol, int32_t version) {
  ...
  ctx_b.name = kEngineB;
  if (InitEngine(ctx_a, protocol, version, kFillA) != 0) {
    return -1;
  }
  if (InitEngine(ctx_b, protocol, version, kFillB) != 0) {
    return -1;
  }
...
int main(int32_t argc, char **argv) {
  ...
  std::string protocol;
  int32_t version = 1;
  if (ParseArgs(argc, argv, device_a, device_b, protocol, version) != 0) {
    return -1;
  }
