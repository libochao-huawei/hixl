...
int32_t GetRemoteAddr(EngineCtx &ctx) {
  ...
  while (received < static_cast<ssize_t>(sizeof(ctx.remote_addr))) {
    ssize_t n = recv(fd, reinterpret_cast<char *>(&ctx.remote_addr) + received,
                     sizeof(ctx.remote_addr) - received, 0);
    if (n <= 0) {
...
int32_t ServerSendAddr(EngineCtx &ctx) {
  ...
  while (sent < static_cast<ssize_t>(sizeof(buf_addr))) {
    ssize_t n = send(ctx.conn_fd, reinterpret_cast<const char *>(&buf_addr) + sent,
                     sizeof(buf_addr) - sent, 0);
    if (n <= 0) {
