...
def init_llm_datadist(args) -> LLMDataDist:
    ...
    logging.info(
        f"init {role} success, cluster_id={ip_port_to_int(args.local_ip_port)}"
    )
    return datadist
...
def run_prompt_sample(datadist, args):
    ...
    tensor = torch.full(
        (BLOCKS_NUM, KV_SHAPE), ip_port_to_int(args.local_ip_port), dtype=torch.float
    ).npu()

    addr = int(tensor.data_ptr())
    cache = cache_manager.register_blocks_cache(
        cache_desc, [addr], BlocksCacheKey(ip_port_to_int(args.local_ip_port), 0)
    )
    ...
    with control_server(args.local_ip_port) as server:
        wait_control_message(server, DECODER_READY_MESSAGE, "decoder ready")
        wait_control_message(server, PULL_DONE_MESSAGE, "pull done")
        wait_control_message(server, UNLINK_DONE_MESSAGE, "unlink done")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local_ip_port", type=str, help="local ip port, eg:10.10.10.1:26000"
    )
    parser.add_argument(
        "--remote_ip_port",
        type=str,
