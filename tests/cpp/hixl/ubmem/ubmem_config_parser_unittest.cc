...
class UbMemConfigParserUTest : public ::testing::Test {};

TEST_F(UbMemConfigParserUTest, UbmemProtocolDescEnablesUbMem) {
  std::map<AscendString, AscendString> options;
  options[OPTION_GLOBAL_RESOURCE_CONFIG] = AscendString(R"({"comm_resource_config.protocol_desc":["ubmem"]})");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), SUCCESS);
  EXPECT_TRUE(result.EnableUbMem().value_or(false));
}

TEST_F(UbMemConfigParserUTest, DisabledByDefaultWhenOptionMissing) {
  std::map<AscendString, AscendString> options;
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), SUCCESS);
  EXPECT_FALSE(result.EnableUbMem().value_or(false));
}

TEST_F(UbMemConfigParserUTest, DisabledWhenEnableOptionEmpty) {
  std::map<AscendString, AscendString> options;
...
TEST_F(UbMemConfigParserUTest, DisabledWhenEnableOptionZero) {
  std::map<AscendString, AscendString> options;
...
TEST_F(UbMemConfigParserUTest, EnableRejectsNonBinaryValue) {
  std::map<AscendString, AscendString> options;
...
TEST_F(UbMemConfigParserUTest, EnableRejectsNonNumericValue) {
  std::map<AscendString, AscendString> options;
...
TEST_F(UbMemConfigParserUTest, AllFieldsParsedCorrectly) {
...
TEST_F(UbMemConfigParserUTest, InvalidJsonReturnsError) {
  auto options = MakeOptionsWithJson("{invalid json");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), PARAM_INVALID);
}

TEST_F(UbMemConfigParserUTest, JsonArrayReturnsError) {
  auto options = MakeOptionsWithJson("[1, 2, 3]");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), PARAM_INVALID);
}

TEST_F(UbMemConfigParserUTest, CapacityZeroRejected) {
  auto options = MakeOptionsWithJson(R"({"fabric_memory": {"max_capacity": 0}})");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), PARAM_INVALID);
}

TEST_F(UbMemConfigParserUTest, CapacityBoundaryMinAccepted) {
...
TEST_F(UbMemConfigParserUTest, CapacityBoundaryMaxAccepted) {
...
TEST_F(UbMemConfigParserUTest, CapacityAboveMaxRejected) {
  auto options = MakeOptionsWithJson(R"({"fabric_memory": {"max_capacity": 1025}})");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), PARAM_INVALID);
}

TEST_F(UbMemConfigParserUTest, CapacityNonNumericRejected) {
  auto options = MakeOptionsWithJson(R"({"fabric_memory": {"max_capacity": "abc"}})");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), PARAM_INVALID);
}

TEST_F(UbMemConfigParserUTest, StartAddressNegativeRejected) {
  auto options = MakeOptionsWithJson(R"({"fabric_memory": {"start_address": -1}})");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), PARAM_INVALID);
}

TEST_F(UbMemConfigParserUTest, StartAddressBoundaryMinAccepted) {
...
TEST_F(UbMemConfigParserUTest, StartAddressBoundaryMaxAccepted) {
...
TEST_F(UbMemConfigParserUTest, StartAddressAboveMaxRejected) {
  auto options = MakeOptionsWithJson(R"({"fabric_memory": {"start_address": 1025}})");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), PARAM_INVALID);
}

TEST_F(UbMemConfigParserUTest, TaskStreamNumZeroRejected) {
  auto options = MakeOptionsWithJson(R"({"fabric_memory": {"task_stream_num": 0}})");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), PARAM_INVALID);
}

TEST_F(UbMemConfigParserUTest, TaskStreamNumBoundaryMinMaxAccepted) {
...
TEST_F(UbMemConfigParserUTest, TaskStreamNumAboveMaxRejected) {
  auto options = MakeOptionsWithJson(R"({"fabric_memory": {"task_stream_num": 9}})");
  HixlOptions result;
  EXPECT_EQ(HixlOptions::Parse(options, result), PARAM_INVALID);
}

TEST_F(UbMemConfigParserUTest, MissingSubFieldsKeepDefaults) {
...
TEST_F(UbMemConfigParserUTest, NonUbMemoryJsonKeysPassThrough) {
...
TEST_F(UbMemConfigParserUTest, AutoConnectEmptyValueRejected) {
  auto options = MakeOptions();
...
TEST_F(UbMemConfigParserUTest, AutoConnectZeroAndOneAccepted) {
...
TEST_F(UbMemConfigParserUTest, AutoConnectNonBinaryRejected) {
  auto options = MakeOptions();
...
TEST_F(UbMemConfigParserUTest, AutoConnectNonNumericRejected) {
  auto options = MakeOptions();
...
TEST_F(UbMemConfigParserUTest, EnabledSkipsParsingWhenDisabled) {
  std::map<AscendString, AscendString> options;
  options[OPTION_ENABLE_USE_FABRIC_MEM] = AscendString("0");
