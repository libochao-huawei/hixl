...
class HixlEngineTest : public ::testing::Test {
 protected:
  ...
  std::map<AscendString, AscendString> options1_ub_pair;  // 与 options1 互补的 UB endpoint，用于同实例 UB 匹配
  void SetUp() override {
    ...
    // 与 options1 同 net_instance_id，UB endpoint 的 comm_id/dst_eid 互换，用于 UB 成对匹配
...
// auto_connect=1 + 同实例（UB handler）：先 transfer 再显式 Connect 再 transfer
...
// 二次 Connect 返回 ALREADY_CONNECTED（显式 Connect 强制 is_lazy=false → SupportsReconnect=false）
