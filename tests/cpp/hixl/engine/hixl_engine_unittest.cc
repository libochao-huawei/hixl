...
class MockEngineMmpaStub : public llm::MmpaStubApiGe {
 public:
...
std::string CreateTempFileWithContent(const std::string &content) {
  char file_template[] = "/tmp/hixl_engine_ut_XXXXXX";
  ...
  std::ofstream ofs(file_template);
...
class HixlEngineTest : public ::testing::Test {
 protected:
  ...
  void SetHccnConfContent(const std::string &content) {
    const std::string file_path = CreateTempFileWithContent(content);
