...
TEST_F(HixlEngineTest, EngineFactoryRejectsConflictProtocolDesc) {
  std::map<AscendString, AscendString> options;
  options[hixl::OPTION_GLOBAL_RESOURCE_CONFIG] =
      R"({"comm_resource_config.protocol_desc": ["uboe:device", "ubg:device"]})";
  HixlOptions parsed;
  auto engine = EngineFactory::CreateEngine("127.0.0.1:26000", options, parsed);
...
TEST_F(HixlEngineTest, TestInitializeSetsProtocolLockNoneForExplicitEndpoints) {
...
TEST_F(HixlEngineTest, TestInitializeSetsProtocolLockUboeForProtocolDescGenerated) {
  SetSocStub("Ascend950A", 0, 3, 45, 67);
  auto uboe_stub = std::make_shared<hixl::test::UboeTestMmpaStub>();
  uboe_stub->access_ok_ = true;
  uboe_stub->real_path_ok_ = true;
  llm::MmpaStub::GetInstance().SetImpl(uboe_stub);
  const std::string uboe_script = "/tmp/hccn_tool";
  std::ofstream ofs(uboe_script);
...
TEST_F(HixlEngineTest, TestInitializeSetsProtocolLockUbgForProtocolDescGenerated) {
  SetSocStub("Ascend950A", 0, 3, 45, 67);
  ...
  HixlEngine ubg_engine("127.0.0.1");
  ...
  EXPECT_EQ(ubg_engine.Initialize(ubg_parsed), SUCCESS);
  EXPECT_EQ(ubg_engine.protocol_lock_, ProtocolLock::kUbg);
  ubg_engine.Finalize();
