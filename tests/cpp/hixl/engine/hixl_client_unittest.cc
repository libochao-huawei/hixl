...
class MockHixlServer {
 public:
  ...
  void RegMem(CommMem *mem_list, size_t size) {
    for (size_t i = 0; i < size; ++i) {
      ...
      RemoteMemInfo rmi;
      rmi.type = (mem_list[i].type == COMM_MEM_TYPE_DEVICE) ? MEM_DEVICE : MEM_HOST;
      rmi.addr = reinterpret_cast<uintptr_t>(mem_list[i].addr);
      rmi.size = mem_list[i].size;
      registered_mem_.push_back(rmi);
    }
  }
 ...
 private:
  ...
  std::vector<RemoteMemInfo> registered_mem_{};
  int32_t conn_fd_ = -1;
...
TEST_F(HixlClientUTest, LazyConnectOnTransferSync) {
  ...
  Status st = client_->TransferSync(op_descs, WRITE, kDefaultTimeoutMs);
  EXPECT_EQ(st, SUCCESS);
  EXPECT_FALSE(ub_handler->connected_types_.empty());
...
TEST_F(HixlClientUTest, LazyConnectIncremental) {
  ...
  EXPECT_EQ(st, SUCCESS);
  EXPECT_NE(ub_handler->connected_types_.count(CommType::COMM_TYPE_UB_D2D), 0U);
  auto after_first = ub_handler->connected_types_.size();
  ...
  EXPECT_GT(ub_handler->connected_types_.size(), after_first);
}
...
TEST_F(HixlClientUTest, LazyConnectSkipsAlreadyConnected) {
  ...
  st = client_->TransferSync(op_descs, WRITE, kDefaultTimeoutMs);
  EXPECT_EQ(st, SUCCESS);
  EXPECT_EQ(ub_handler->connected_types_.size(), after_first);
}
...
TEST_F(HixlClientUTest, LazyConnectNoMatchingEndpoint) {
  ...
  // 只注册 D2D handle，缺少 H2D
  handler.handles_[CommType::COMM_TYPE_UB_D2D] = reinterpret_cast<HixlClientHandle>(0x1000);

  std::vector<CommType> types = {CommType::COMM_TYPE_UB_D2D, CommType::COMM_TYPE_UB_H2D};
  Status st = handler.EnsureLinksConnected(types, kDefaultTimeoutMs);
  EXPECT_EQ(st, FAILED);
}

// auto_connect 模式用户显式调 Connect，补齐全部剩余链路
TEST_F(HixlClientUTest, ExplicitConnectAfterLazyConnect) {
  ...
  ASSERT_NE(ub_handler, nullptr);
  EXPECT_TRUE(ub_handler->lazy_mode_);
  // 首次 Connect（SetupLazyTransferTest 内部）应延迟建链
  EXPECT_TRUE(ub_handler->connected_types_.empty());
  EXPECT_TRUE(ub_handler->connect_triggered_);

  // 用户显式调 Connect：二次调用应补齐全部链路
  Status st = client_->Connect(kDefaultTimeoutMs);
  EXPECT_EQ(st, SUCCESS);
  EXPECT_EQ(ub_handler->connected_types_.size(), 4U);
}

// transfer 按需建部分链路后，用户显式 Connect 补齐剩余
TEST_F(HixlClientUTest, ExplicitConnectAfterTransfer) {
  SetupLazyTransferTest(true);

  auto *ub_handler = dynamic_cast<UbClientHandler *>(client_->client_handler_.get());
  ASSERT_NE(ub_handler, nullptr);

  // transfer 触发 D2D 按需建链
  auto d2d_ops = CreateTransferOps(1, &kLocalMems[0], &kRemoteMems[2]);
  Status st = client_->TransferSync(d2d_ops, WRITE, kDefaultTimeoutMs);
  EXPECT_EQ(st, SUCCESS);
  EXPECT_NE(ub_handler->connected_types_.count(CommType::COMM_TYPE_UB_D2D), 0U);
  auto after_transfer = ub_handler->connected_types_.size();

  // 用户显式 Connect：补齐剩余链路
  st = client_->Connect(kDefaultTimeoutMs);
  EXPECT_EQ(st, SUCCESS);
  EXPECT_GT(ub_handler->connected_types_.size(), after_transfer);
  EXPECT_EQ(ub_handler->connected_types_.size(), 4U);
}

// 非 lazy handler 已建链后再次 Connect 返回 ALREADY_CONNECTED
TEST_F(HixlClientUTest, ConnectNonLazyReturnsAlreadyConnected) {
  SetupTransferTest(true);  // is_lazy=false，非 lazy 模式

  // 首次 Connect 成功
  EXPECT_TRUE(client_->is_connected_);

  // 二次 Connect：SupportsReconnect()=false，应返回 ALREADY_CONNECTED
  Status st = client_->Connect(kDefaultTimeoutMs);
  EXPECT_EQ(st, ALREADY_CONNECTED);
}

// UbClientHandler lazy 模式下 SupportsReconnect 返回 true
TEST_F(HixlClientUTest, SupportsReconnectForLazyUbHandler) {
  UbClientHandler handler({});
  handler.lazy_mode_ = true;
  EXPECT_TRUE(handler.SupportsReconnect());

  handler.lazy_mode_ = false;
  EXPECT_FALSE(handler.SupportsReconnect());
}

// DirectClientHandler 不支持重复建链
TEST_F(HixlClientUTest, SupportsReconnectForDirectHandler) {
  auto handle = reinterpret_cast<HixlClientHandle>(0x1000);
  DirectClientHandler handler(handle);
  EXPECT_FALSE(handler.SupportsReconnect());
}
