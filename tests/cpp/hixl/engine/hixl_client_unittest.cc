...
class MockHixlServer {
 public:
  ...
  // 仅注册端点回调，不注册 kGetMemInfoReq（用于测试超时）
  void ListenServerNoMemInfo() {
    MsgProcessor send_endpoint_cb = [this](int32_t fd, const char *msg, uint64_t msg_len) -> Status {
      (void)msg;
      (void)msg_len;
      conn_fd_ = fd;
...
class HixlClientUTest : public ::testing::Test {
 protected:
  ...
  void SetupLazyTransferTest(bool use_4ub = true) {
    ...
    Status st = client_->Initialize(local_endpoint_list, kDefaultTimeoutMs, true);  // is_lazy=true
    EXPECT_EQ(st, SUCCESS);

    st = client_->SetLocalMemInfo(use_4ub ? Make4UbMemInfoList() : MakeMemInfoList());
    EXPECT_EQ(st, SUCCESS);

    st = client_->Connect(kDefaultTimeoutMs);  // lazy 模式：不应真正建链
    EXPECT_EQ(st, SUCCESS);
  }
...
// 对端无注册内存 → mem_info 为空 → ClassifyTransfers 对端地址查找失败
...
// 所需 CommType 无对应端点 → EnsureLinksConnected 中 handles_.find 失败
...
// 服务端不注册 kGetMemInfoReq 回调 → SendMemInfoReq 超时
TEST_F(HixlClientUTest, LazyConnectMemInfoTimeout) {
  server_->SetMode(MockHixlServerMode::k4UbNormal);
  auto st = server_->CreateServer(Make4UbRemoteEpList());
  ASSERT_EQ(st, SUCCESS);
  server_->RegMem(default_remote_mem_list, default_list_num);
  server_->ListenServerNoMemInfo();  // 不注册 kGetMemInfoReq
  ...
  // Initialize 会发 kGetMemInfoReq，但服务端无回调 → RecvMemInfoResp 超时
  st = client_->Initialize(local_list, kDefaultTimeoutMs, true);
