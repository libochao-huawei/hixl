...
class MockClientAclRuntimeStub : public llm::AclRuntimeStub {
 public:
 ...
 public:
  ...
  Status CreateServer(const std::vector<EndpointConfig> &remote_endpoint_list) {
    ...
    for (const auto &ep : remote_endpoint_list) {
      ...
      ConvertToEndpointDesc(ep, endpointInfo);
...
TEST_F(HixlClientUTest, DeserializePreservesDeviceInfoAfterSerializeRoundTrip) {
  std::vector<EndpointConfig> input_list;
  EndpointConfig ep{};
  ...
  [
    {
      ...
      "placement": "device",
      "plane": "plane-a",
      "dst_eid": "00010002000300040005000600070008",
      "net_instance_id": "superpod_legacy"
    }
  ])";
