...
extern void DsmiStubSetInterconType(unsigned int type);
extern void DsmiStubSetSuperPodId(unsigned int id);
extern void DsmiStubSetSlotId(unsigned int slot_id, int ret);
extern void DsmiStubSetDeviceInfoRet(int ret);
...
TEST_F(EndpointGeneratorUTest, AutoGenUbgByInterconTypeOnA5) {
  acl_stub_->soc_name_ = "Ascend950A";
  acl_stub_->device_id_ = 0;
  acl_stub_->phy_device_id_ = 3;
  ...
  for (const auto &ep : endpoint_list) {
    if (ep.protocol == kProtocolUbg) {
      has_ubg = true;
      EXPECT_EQ(ep.placement, kPlacementDevice);
      EXPECT_EQ(ep.comm_id.size(), 32U);
      EXPECT_EQ(ep.comm_id.substr(14, 2), "80");
    }
    if (ep.protocol == kProtocolUbCtp || ep.protocol == kProtocolUbTp) {
      has_ub = true;
    }
  }
  EXPECT_TRUE(has_ubg) << "UBG endpoint should be auto-generated by InterconType=4";
  EXPECT_TRUE(has_ub) << "UB endpoint should coexist with scale-out";

  DcmiStubSetEnableUbgEid(false);
}

TEST_F(EndpointGeneratorUTest, AutoGenUboeByInterconTypeOnA5) {
  acl_stub_->soc_name_ = "Ascend950A";
  acl_stub_->device_id_ = 0;
  acl_stub_->phy_device_id_ = 3;
  ...
  bool has_uboe = false;
  for (const auto &ep : endpoint_list) {
    if (ep.protocol == kProtocolUboe) {
      has_uboe = true;
    }
  }
  EXPECT_TRUE(has_uboe) << "UBoE endpoint should be auto-generated by InterconType=2";

  (void)remove(script_path.c_str());
}

TEST_F(EndpointGeneratorUTest, AutoGenFallsBackToUbWhenInterconTypeIsRoce) {
  acl_stub_->soc_name_ = "Ascend950A";
  acl_stub_->device_id_ = 0;
  acl_stub_->phy_device_id_ = 3;
  ...
  for (const auto &ep : endpoint_list) {
    EXPECT_TRUE(ep.protocol == kProtocolUbCtp || ep.protocol == kProtocolUbTp)
        << "InterconType=RoCE should fall back to UB only";
  }
  EXPECT_FALSE(endpoint_list.empty()) << "UB endpoints should be generated";
}
llm::MmpaStub::GetInstance().SetImpl(std::make_shared<UboeMmpaStub>());
const std::string script_path = CreateExecutableScript("hccn_tool", "#!/bin/sh\necho \"ipaddr:192.168.100.203\"\n");
setenv("PATH", "/tmp", 1);
