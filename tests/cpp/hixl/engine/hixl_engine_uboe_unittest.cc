...
// 测试带有 protocol_desc 的配置
TEST_F(HixlEngineUboeTest, InitializeWithProtocolDesc) {
  std::map<AscendString, AscendString> options_with_desc;
  options_with_desc[hixl::OPTION_LOCAL_COMM_RES] = R"(
  {
      "net_instance_id": "protocol_desc_test",
      "endpoint_list": [
          {
              "protocol": "uboe",
      ...
      "comm_resource_config.protocol_desc": ["uboe", "roce"],
      "version": "1.3"
  }
  )";

  Hixl engine;
  EXPECT_EQ(engine.Initialize("127.0.0.1", options_with_desc), SUCCESS);
  engine.Finalize();
}
...
TEST_F(HixlEngineUboeTest, InitializeWithGlobalResourceConfig) {
  ...
  // 必须提供空的 endpoint_list，才能触发 GenDefaultEndpointConfig
  options[hixl::OPTION_LOCAL_COMM_RES] = R"(
  {
      "net_instance_id": "uboe_default_test",
      "version": "1.3"
  }
  )";
