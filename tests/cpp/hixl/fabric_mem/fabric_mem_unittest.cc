...
bool StartDefaultShareHandleServer(FabricMemControlServer &server, int32_t &port, std::string &remote) {
  ...
  return server.Start(remote, [](std::vector<ShareHandleInfo> &handles) {
...
TEST(FabricMemControlUTest, ServerClosesSelfLoopbackOnHeartbeatTimeout) {
  FabricMemControlServer::SetHeartbeatTimeoutMs(100);
  int32_t port = 0;
  std::string remote;
  FabricMemControlServer server;
...
TEST(FabricMemControlUTest, AdxlControlSendMsgOverSocketPair) {
  ...
  EXPECT_EQ(FabricMemAdxlControl::SendHeartBeat(fds[0]), SUCCESS);
  EXPECT_EQ(FabricMemAdxlControl::SendMsg(fds[0], FabricMemAdxlMsgType::kHeartBeat, R"({"msg":"H"})", 3000000ULL),
            SUCCESS);
  (void)close(fds[0]);
  (void)close(fds[1]);
}

TEST(FabricMemControlUTest, ServerClosesClientOnOversizedAdxlBody) {
  int32_t port = 0;
  std::string remote;
  FabricMemControlServer server;
  ASSERT_TRUE(StartDefaultShareHandleServer(server, port, remote));
  ...
  int32_t conn_fd = -1;
  ASSERT_EQ(FetchFabricMemClientConn(remote, "oversized_body_client", conn_fd), SUCCESS);
  SendRawAdxlMsg(conn_fd, kFabricMemAdxlMagic, static_cast<int32_t>(FabricMemAdxlMsgType::kHeartBeat),
                 std::string(kMaxAdxlSessionBodySize, 'x'));
...
TEST(FabricMemControlUTest, AdxlControlSendHeartBeatOnClosedFd) {
  ...
  (void)close(fds[0]);
  (void)close(fds[1]);
  EXPECT_NE(FabricMemAdxlControl::SendHeartBeat(fds[0]), SUCCESS);
}
...
TEST(FabricMemEngineUTest, OutboundHeartbeatSucceedsOnLiveFd) {
  FabricMemEngine engine(AscendString("127.0.0.1"));
  AttachTestContext(engine);
  engine.auto_connect_ = true;
  engine.is_initialized_ = true;

  const std::string remote = "127.0.0.1:28102";
  int32_t fds[2] = {-1, -1};
  ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, fds), 0);
  DrainSocketInBackground(fds[1]);
