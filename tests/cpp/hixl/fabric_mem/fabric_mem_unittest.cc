...
TEST(FabricMemControlUTest, ServerSkipsHeartbeatTimeoutForSelfLoopback) {
  FabricMemControlServer::SetHeartbeatTimeoutMs(100);
  int32_t port = 0;
  std::string remote;
  FabricMemControlServer server;
  ...
  server.CheckClientHeartbeatTimeouts();
  {
    std::lock_guard<std::mutex> lock(server.state_->mutex);
    EXPECT_EQ(server.state_->sessions.size(), 1U);
    EXPECT_EQ(server.state_->client_id_to_fd.count(remote), 1U);
    EXPECT_EQ(server.state_->client_id_to_fd.count("peer_client"), 0U);
    const auto self_it = server.state_->client_id_to_fd.find(remote);
    ASSERT_NE(self_it, server.state_->client_id_to_fd.end());
    EXPECT_FALSE(server.state_->sessions.at(self_it->second).with_heartbeat);
  }
