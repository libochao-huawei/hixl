...
// 注册本地内存并设置传输参数（供重复测试用例共享）
static void SetupLocalMemAndComMem(HixlCSClient& cli, MemHandle& local_handle,
                                   CommunicateMem& com_mem, void*& query_handle) {
  CommMem local{};
  ...
  com_mem = CommunicateMem{1, remote_list, local_list, len_list};
}
...
TEST_F(HixlCSClientFixture, ReleaseCompleteHandleTest) {
  ...
  void *query_handle = nullptr;
  SetupLocalMemAndComMem(cli, local_handle, com_mem, query_handle);
