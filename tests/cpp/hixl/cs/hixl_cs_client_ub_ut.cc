...
// [修改] 依赖现有的 aclrtWaitAndResetNotify 打桩返回栈，替换掉旧的 rtNotifyWait
...
void PrepareKernelReadyForUt(HixlCSClient &cli) {
  ...
  // [修改] 变量名已从 stub 改为 func，且强制转换为 void*
...
// [修改] 状态参数从 int32_t* 改为 HixlCompleteStatus*
...
TEST_F(HixlCSClientUbFixture, BatchPutUbDeviceNotifyWaitFail) {
  // [修改] 控制 aclrtWaitAndResetNotify 的桩代码返回失败 (-1 转换为 aclError)
