...
static int32_t HcommAclrtNotifyRecordOnThread(ThreadHandle thread, uint64_t dstNotifyId)
{
  return 0;
}

TEST(HixlKernelBasicTest, BatchGetHccsSuccess) {
  // 模拟远端源数据和本地目标缓冲区
  ...
  // ========== 核心修改2：直接传递地址值 ==========
  auto param = CreateTestParamFixed<3>(remote_addr, local_addr, lens_storage, remote_flag_addr_hccs, local_flag_addr_hccs);
