...
class AdxlAsyncHostFlagUTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(AdxlAsyncHostFlagUTest, GetTransferStatusFailsWhenHostFlagNeverSet) {
  auto pair = MakeConnectedInt32Pair();
  TransferOpDesc desc = MakeWriteDesc(pair);
  TransferReq req = nullptr;
  TransferStatus status = TransferStatus::WAITING;

  llm::AsyncHostFlagNeverSetMock fail_mock;
  llm::AclRuntimeStub::Install(&fail_mock);
  EXPECT_EQ(pair.engine1->TransferAsync(kPeerEngine, WRITE, {desc}, {}, req), SUCCESS);
  EXPECT_EQ(pair.engine1->GetTransferStatus(req, status), FAILED);
  EXPECT_EQ(status, TransferStatus::FAILED);
