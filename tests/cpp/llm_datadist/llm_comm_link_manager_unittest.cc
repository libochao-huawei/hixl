...
class LLMCommLinkManagerUTest : public ::testing::Test {
 protected:
  ...
  static std::map<ge::AscendString, ge::AscendString> MakeLinkTestOptions(uint32_t link_total_time = 10,
    ...
    options["llm.LinkTotalTime"] = ge::AscendString(std::to_string(link_total_time).c_str());
    options["llm.LinkRetryCount"] = ge::AscendString(std::to_string(link_retry_count).c_str());
    return options;
  }
