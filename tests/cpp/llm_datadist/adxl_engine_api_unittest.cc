...
class AdxlEngineUTest : public ::testing::Test {
 protected:
  ...
  void TearDown() override {
    adxl::StatisticManager::GetInstance().Reset();
  ...
  void ExpectConnectStatistic(const std::string &channel_id, bool expect_hccl, bool expect_bind_mem) {
...
TEST_F(AdxlEngineUTest, TestConnectStatisticForDirectTransfer) {
  ...
  EXPECT_EQ(engine1.Connect("127.0.0.1:26001"), SUCCESS);
  ExpectConnectStatistic("127.0.0.1:26001", true, true);
...
TEST_F(AdxlEngineUTest, TestConnectStatisticForFabricMemMode) {
  ...
  EXPECT_EQ(engine1.Connect("127.0.0.1:26001"), SUCCESS);
  ExpectConnectStatistic("127.0.0.1:26001", false, false);
  engine1.Finalize();
  engine2.Finalize();
}
