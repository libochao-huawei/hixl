...
constexpr uint8_t kPatternB = 0xBB;
...
class ScopedRuntimeMock {
 public:
  explicit ScopedRuntimeMock(const std::shared_ptr<llm::RuntimeStub> &instance) { llm::RuntimeStub::SetInstance(instance); }
  ~ScopedRuntimeMock() { llm::RuntimeStub::Reset(); }
...
class ScopedRuntimeFunctionFail {
 public:
  explicit ScopedRuntimeFunctionFail(const std::string &func_name) : old_(g_runtime_stub_mock) { g_runtime_stub_mock = func_name; }
  ~ScopedRuntimeFunctionFail() { g_runtime_stub_mock = old_; }
...
void *GetBackingRemotePtr(const ChannelPtr &channel, uint64_t remote_addr) {
  ...
  for (auto &kv : va_map) {
    ...
    if (info.va_addr == remote_addr) {
      return reinterpret_cast<void *>(new_va);
    }
  }
  return nullptr;
}
...
TEST_F(FabricMemTransferServiceUTest, TestTransfer) {
  ...
  std::memset(backing_remote_ptr, 0, kMemLen); // Clear remote
  ...
  std::memset(backing_remote_ptr, kPatternB, kMemLen); // Fill remote
  ...
  for (size_t i = 0; i < kTransferLen; ++i) {
      EXPECT_EQ(local_buf[i], kPatternB) << "Read verification failed at index " << i;
  }
...
TEST_F(FabricMemTransferServiceUTest, TestTransferAsync) {
  ...
  for (size_t i = 0; i < kTransferLen; ++i) {
      EXPECT_EQ(local_buf[i], kPatternB) << "Async Read verification failed at index " << i;
  }
  channel->Finalize();
}
...
TEST_F(FabricMemTransferServiceUTest, TestTransferAsync_RemoteAddrNotFound) {
  ...
  TransferOpDesc desc{};
  desc.local_addr = reinterpret_cast<uintptr_t>(local_buf.data());
  desc.remote_addr = kRemoteAddr + kMemLen + 1;
  desc.len = kTransferLen;

  TransferReq req = reinterpret_cast<void *>(kReqBase);
...
TEST_F(FabricMemTransferServiceUTest, TestTransferAsync_RemoteAddrOutOfRange) {
  ...
  TransferReq req = reinterpret_cast<void *>(kReqBase + 1);
  EXPECT_EQ(service_->TransferAsync(channel, TransferOp::WRITE, {desc}, req), PARAM_INVALID);
...
TEST_F(FabricMemTransferServiceUTest, TestTransferAsync_EventCreateFail_CleanupOk) {
  ...
  std::fill(local_buf.begin(), local_buf.end(), kPatternA);
  std::memset(backing_remote_ptr, 0, kMemLen);

  TransferReq req = reinterpret_cast<void *>(kReqBase + 2);
  {
    [[maybe_unused]] ScopedRuntimeFunctionFail fail("rtEventCreate");
  ...
  req = reinterpret_cast<void *>(kReqBase + 3);
  ...
  EXPECT_EQ(status, TransferStatus::COMPLETED);
  for (size_t i = 0; i < kTransferLen; ++i) {
    EXPECT_EQ(reinterpret_cast<uint8_t *>(backing_remote_ptr)[i], kPatternA)
        << "Async Write verification failed at index " << i;
  }
  channel->Finalize();
}
...
TEST_F(FabricMemTransferServiceUTest, TestGetTransferStatus_QueryStatusFail_CleanupOk) {
  ...
  TransferReq req = reinterpret_cast<void *>(kReqBase + 4);
  EXPECT_EQ(service_->TransferAsync(channel, TransferOp::WRITE, {desc}, req), SUCCESS);
...
TEST_F(FabricMemTransferServiceUTest, TestGetTransferStatus_StreamSyncFail_CleanupOk) {
  ASSERT_EQ(service_->Initialize(kStreamMax), SUCCESS);
  auto channel = CreateInitializedChannel();
  ShareHandleInfo share_info;
  share_info.va_addr = kRemoteAddr;
  ...
  TransferReq req = reinterpret_cast<void *>(kReqBase + 5);
  EXPECT_EQ(service_->TransferAsync(channel, TransferOp::WRITE, {desc}, req), SUCCESS);
  TransferReq req2 = reinterpret_cast<void *>(kReqBase + 6);
  EXPECT_EQ(service_->TransferAsync(channel, TransferOp::WRITE, {desc}, req2), SUCCESS);

  // Force rtStreamSynchronize() to fail in stub runtime implementation.
  [[maybe_unused]] ScopedEnvVar env_fail("STREAM_SYNC_FAIL", "mock_fail");
  TransferStatus status = TransferStatus::WAITING;
  EXPECT_EQ(service_->GetTransferStatus(channel, req, status), FAILED);
  EXPECT_EQ(status, TransferStatus::FAILED);
...
TEST_F(FabricMemTransferServiceUTest, TestTransferAsync_StreamPoolFull) {
  ...
  for (size_t i = 0; i < kMaxConcurrentReq; ++i) {
    TransferReq req = reinterpret_cast<void *>(kReqBase + 0x100 + i);
    EXPECT_EQ(service_->TransferAsync(channel, TransferOp::WRITE, {desc}, req), SUCCESS);
  }
  TransferReq overflow_req = reinterpret_cast<void *>(kReqBase + 0x100 + kMaxConcurrentReq);
  ...
  TransferReq req = reinterpret_cast<void *>(kReqBase + 0x200);
  EXPECT_EQ(service_->TransferAsync(channel, TransferOp::WRITE, {desc}, req), SUCCESS);
  TransferStatus status = TransferStatus::WAITING;
  int32_t retries = 0;
  while (status != TransferStatus::COMPLETED && retries++ < kMaxPollRetries) {
...
TEST_F(FabricMemTransferServiceUTest, TestSetPid) {
    ASSERT_EQ(service_->Initialize(kStreamMax), SUCCESS);
    MemDesc mem_desc;
    mem_desc.addr = kMemAddr;
    mem_desc.len = kMemLen;
    MemHandle handle;
    EXPECT_EQ(service_->RegisterMem(mem_desc, MemType::MEM_DEVICE, handle), SUCCESS);
    EXPECT_EQ(service_->SetPid(kPid), SUCCESS);
    EXPECT_EQ(service_->DeregisterMem(handle), SUCCESS);
}
