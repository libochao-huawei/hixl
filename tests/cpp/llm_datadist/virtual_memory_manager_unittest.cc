...
TEST_F(VirtualMemoryManagerTest, ReserveMemory_OverflowSize_Fails) {
  ...
  manager.Initialize();
  uintptr_t addr = 0;
  EXPECT_EQ(manager.ReserveMemory(SIZE_MAX, addr), RESOURCE_EXHAUSTED);
  EXPECT_EQ(manager.ReserveMemory(SIZE_MAX - 1, addr), RESOURCE_EXHAUSTED);
  constexpr size_t kBlockSize = 1024UL * 1024UL * 1024UL;
  constexpr size_t kOverflowBoundary = SIZE_MAX - kBlockSize + 2;
  EXPECT_EQ(manager.ReserveMemory(kOverflowBoundary, addr), RESOURCE_EXHAUSTED);
}
