...
TEST_F(StatisticManagerUTest, TestDump) {
  StatisticManager::GetInstance().UpdateBufferTransferCost(kClientChannelId, kCost, kBytes);
  StatisticManager::GetInstance().Dump();
}

TEST_F(StatisticManagerUTest, TestDirectTransferDump) {
  StatisticManager::GetInstance().UpdateDirectTransferCost(kClientChannelId, kCost, kBytes);
  StatisticManager::GetInstance().Dump();
}
...
TEST_F(StatisticManagerUTest, TestTransferStatisticSnapshot) {
  StatisticManager::GetInstance().UpdateBufferTransferCost(kClientChannelId, kCost, kBytes);
  StatisticManager::GetInstance().UpdateDirectTransferCost(kClientChannelId, kCost * 2U, kBytes * 2U);
  StatisticManager::GetInstance().UpdateFabricMemCosts(kClientChannelId, kCost * 3U, kCost, kBytes * 3U);
...
TEST_F(StatisticManagerUTest, TestRemoveChannelAndFabricMemDump) {
  ...
  StatisticManager::GetInstance().UpdateTcpConnectCost(kClientChannelId, kCost);
  StatisticManager::GetInstance().UpdateFabricMemCosts(kClientChannelId, kCost, kCost, kBytes);
  StatisticManager::GetInstance().Dump();
  StatisticManager::GetInstance().RemoveChannel(kChannelId);
