...
class MockAccessTrue : public MmpaStubApiGe {
 public:
...
class MockAccessFalse : public llm::MmpaStubApiGe {
 public:
  INT32 Access(const CHAR *path_name) override {
    return 1;
  }
...
TEST_F(HccnToolTest, TestGetIpFromHccnTool) {
  llm::MmpaStub::GetInstance().SetImpl(std::make_shared<MockAccessTrue>());
