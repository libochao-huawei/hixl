...
TEST_F(HccnToolTest, TestPreProcess) {
  std::string ip = "ipaddr:127.0.0.1";
  EXPECT_EQ(llm::LocalCommResGenerator::PreProcess(ip), ge::SUCCESS);
}

TEST_F(HccnToolTest, TestHccnOutput) {
  std::string cmd = "hccs_tool";
  EXCEPT_EQ(llm::LocalCommResGenerator::GetHccnOutput(cmd).find("not found") != std::string::npos, true);
}
