...
def _wait_for_combo_processes(
    server: subprocess.Popen, client_cmd: list[str]
) -> tuple[int, int] | None:
    try:
...
def run_combo(spec: CommComboSpec) -> bool:
    ...
    server_cmd = _make_cmd(
        CommRunSpec(role="target", device_id=spec.device_id1, **common)
    )
    client_cmd = _make_cmd(
        CommRunSpec(role="initiator", device_id=spec.device_id0, **common)
    )
...
def parse_all_benchmark_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip_comm", action="store_true", help="Skip communication benchmarks"
    )
...
def _validate_args(args: argparse.Namespace, devs: list[int]) -> int | None:
    ...
    if args.target_host != "127.0.0.1":
        log.error(
            "[ERROR] run_all_benchmarks.py does not support true dual-machine orchestration."
        )
        log.error(
...
def _run_one_comm_combo(
    args, platform_id: str, devs: list[int], comm_bin: str, port_offset: int
) -> bool:
...
def run_comm_suite(
    args: argparse.Namespace, platform_id: str, devs: list[int]
) -> tuple[int, int]:
...
def render_perf_report(
    platform_id: str, comm_ok: int, output_path: Path | None
) -> None:
...
def main() -> None:
    ...
    log.info(
        f"[INFO] Platform: {platform_id}, transports: {_transports_for_platform(platform_id)}"
    )
    comm_ok = 0
    ...
    if not args.skip_kv:
        ...
        if not Path(kv_bin).exists():
            log.warning(
                f"[WARN] KV bench binary not found: {kv_bin}, skipping KV benchmarks"
            )
        else:
            kv_output = BENCHMARKS_DIR / "kv_benchmark" / "output"
            kv_transport = default_kv_transport(platform_id)
            log.info(
                f"[INFO] KV benchmark transport={kv_transport} (platform={platform_id})"
            )
            kv_process_count = (
                len(devs) if devs else KV_PROCESS_COUNT_BY_PLATFORM.get(platform_id, 8)
            )
            kv_devs = resolve_kv_devices(platform_id, devs, kv_process_count)
            log.info(
                f"[INFO] KV benchmark devices ({len(kv_devs)}): {','.join(str(d) for d in kv_devs)}"
            )
            kv_ok = run_kv_benchmarks(
                KvSuiteSpec(
                    kv_bin, kv_devs, kv_output, KV_MODELS, kv_transport, platform_id
                )
            )
