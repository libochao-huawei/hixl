...
# Ascend950-class (A5): no HCCS in comm benchmarks — same transports as aggregate A3 minus hccs.
...
def _make_cmd(bench_bin: str, role: str, transport: str, initiator_mem: str,
              target_mem: str, op_type: str, loops: int, output_dir: Path,
              hixl_port: int, tcp_port: int, device_id: int,
              platform_id: str,
              target_host: str = "127.0.0.1",
              hixl_init_options: list[str] | None = None) -> list[str]:
    ...
    if role == "target":
        # Match initiator's --remote_engine address; 0.0.0.0 breaks exchanged channel identity (see ExchangeConnectInfo).
        return [bench_bin, "--role=target",
                f"--device_id={device_id}",
                f"--local_engine={target_host}:{hixl_port}",
                f"--tcp_port={tcp_port}",
                *base_args,
                *opt_args]
    else:
...
def run_combo(bench_bin: str, transport: str, initiator_mem: str,
              target_mem: str, op_type: str, loops: int,
              output_dir: Path, hixl_port: int, tcp_port: int,
              device_id0: int, device_id1: int,
              platform_id: str,
              target_host: str = "127.0.0.1",
              hixl_init_options: list[str] | None = None) -> bool:
    ...
    server_cmd = _make_cmd(bench_bin, "target", transport, initiator_mem,
                           target_mem, op_type, loops, output_dir,
...
def run_kv_benchmarks(kv_bin: str, devices: list[int], output_dir: Path,
                      models: list[str], transport: str, platform_id: str) -> int:
    ok = 0
    for model in models:
        print(f"\n{'='*60}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Repeat full block-step ladder per comm combo (hixl_comm_bench --loops).",
    )
    parser.add_argument("--device_ids", type=str, default="0,1,2,3,4,5,6,7",
                        help="Device IDs (default: 0-7). First 2 used for comm; KV uses all on A2/A5, "
                        "or 0-15 on A3 when fewer than 16 IDs are given.")
    parser.add_argument(
        "--platform",
        type=str,
        default=None,
        choices=["a2", "a3", "a5"],
        help="Skip npu-smi detection and use this platform (a2/a3/a5).",
    )
    ...
    parser.add_argument(
        ...
        metavar="KEY=VALUE",
        help=(
            "HIXL Initialize() option for hixl_comm_bench only, same as its -H=KEY=VALUE (repeatable). "
            "Ignored when --skip_comm."
        ),
    )
    ...
    if args.target_host != "127.0.0.1":
        print("[ERROR] run_all_benchmarks.py does not support true dual-machine orchestration.")
        print("[ERROR] Use benchmarks/comm_benchmark/scripts/run_comm_benchmark.py --role=target/initiator instead.")
        sys.exit(1)
                    ...
                    for op in OP_TYPES:
                        ...
                        if transport == "hccs" and not _hccs_supports_combo(platform_id, im, tm):
                            print(
                                ...
                                f"(A2: HCCS D2D only; A3: add H2rD|rD2H; A5/Ascend950: no HCCS)"
                            )
                            continue
                        comm_total += 1
                        ...
                        port_offset += 1
                        if run_combo(comm_bin, transport, im, tm, op, args.loops,
                                     comm_output, hixl_port, tcp_port, devs[0], devs[1],
                                     platform_id,
                                     args.target_host,
                                     args.hixl_option or None):
                            comm_ok += 1
                        time.sleep(1)
    ...
    if not args.skip_kv:
        ...
        if not Path(kv_bin).exists():
            ...
            print(f"[INFO] KV benchmark transport={kv_transport} (platform={platform_id})")
            ...
            print(f"[INFO] KV benchmark devices ({len(kv_devs)}): "
                  f"{','.join(str(d) for d in kv_devs)}", flush=True)
