...
def _run_npu_smi(npu_smi: str, args: list[str]) -> str:
    ...
    try:
        result = subprocess.run(
            [npu_smi, *args], capture_output=True, text=True, timeout=15
        )
...
def prompt_platform(reason: str | None = None) -> str:
    ...
    if not sys.stdin.isatty():
        raise RuntimeError(
            "Cannot detect platform and stdin is not interactive. Pass --platform=a2|a3|a5."
        )
    while True:
...
def detect_platform(platform_override: str | None = None) -> str:
    ...
    if platform_override is not None:
        ...
        if platform_id in ("a2", "a3", "a5"):
            ...
            return platform_id
        raise ValueError(
            f"Invalid --platform {platform_override!r}; expected a2, a3, or a5"
        )
    ...
    return prompt_platform(
        f"[WARN] Could not detect platform from npu-smi info.{detail}"
    )
