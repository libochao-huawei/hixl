...
def collect_npu_smi_info_text() -> str:
    ...
    if npu_smi is None:
        return ""
    # Primary: `npu-smi info` lists device names (Ascend910 / Ascend910B / Ascend950).
    text = _run_npu_smi(npu_smi, ["info"])
    if classify_platform_from_npu_smi_output(text) is not None:
        return text
    # Fallback for layouts that only expose chip info under board target.
...
def prompt_platform(reason: str | None = None) -> str:
    ...
    if not sys.stdin.isatty():
        print(
            "[ERROR] Cannot detect platform and stdin is not interactive. "
            "Pass --platform=a2|a3|a5.",
            flush=True,
        )
        sys.exit(1)
    while True:
        ...
        if choice in ("a2", "a3", "a5"):
            return choice
        print("Please enter 'a2', 'a3', or 'a5'.")
...
def detect_platform(platform_override: str | None = None) -> str:
    ...
    if platform_override is not None:
        ...
        if platform_id in ("a2", "a3", "a5"):
            print(f"[INFO] Using platform from --platform: {platform_id}", flush=True)
            return platform_id
        print(f"[ERROR] Invalid --platform {platform_override!r}; expected a2, a3, or a5", flush=True)
        sys.exit(1)
    ...
    if platform_id == "a5":
        print("[INFO] Detected A5 (Ascend950) from npu-smi info", flush=True)
        return platform_id
    if platform_id == "a2":
        print("[INFO] Detected A2 (Ascend910B) from npu-smi info", flush=True)
        return platform_id
    if platform_id == "a3":
        print("[INFO] Detected A3 (Ascend910) from npu-smi info", flush=True)
        return platform_id
