...
def prompt_platform(reason: str | None=None) -> str:
    ...
    if not sys.stdin.isatty():
        log.error('[ERROR] Cannot detect platform and stdin is not interactive. Pass --platform=a2|a3|a5.')
        sys.exit(1)
    while True:
...
def detect_platform(platform_override: str | None=None) -> str:
    ...
    if platform_override is not None:
        ...
        if platform_id in ('a2', 'a3', 'a5'):
            ...
            return platform_id
        log.error(f'[ERROR] Invalid --platform {platform_override!r}; expected a2, a3, or a5')
        sys.exit(1)
