...
TRANSPORT_ORDER = ['hccs', 'rdma', 'fabric_mem', 'uboe', 'ubg']
...
TRANSPORT_LABEL: dict[str, str] = {'hccs': 'HCCS', 'rdma': 'ROCE', 'uboe': 'UBOE', 'ubg': 'UBG',
...
def cell_not_supported(direction: str, transport: str, platform: str) -> bool:
    if transport in ('uboe', 'ubg') and platform in ('a2', 'a3'):
        return True
    if transport != 'hccs':
        return False
...
def transports_for_platform(platform: str, deployment: str) -> list[str]:
    ...
    return ['rdma', 'fabric_mem', 'uboe', 'ubg']
