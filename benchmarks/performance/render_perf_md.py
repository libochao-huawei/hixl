...
def collect_columns_for_platform(
    platform: str, deployment: str = "single"
) -> list[tuple[str, str]]:
...
def _build_table_matrix(
    ...
    if not ordered_blocks:
        return None
    headers = ["Block"] + [
        _table_column_header(dname, tport) for (dname, tport) in cols
    ]
    rows: list[list[str]] = []
    for bs in ordered_blocks:
        rows.append(
            [bs]
            + [
                format_bandwidth_cell(dname, tport, bs, lookup, platform)
                for (dname, tport) in cols
            ]
        )
    return (headers, rows)
...
def _render_transport_chart(spec: TransportChartSpec) -> str:
    ...
    ax.set_title(
        f"{spec.platform} — {TRANSPORT_LABEL.get(spec.tport, spec.tport)}", fontsize=12
    )
...
def render_charts(
    lookup: dict[str, dict[str, dict[str, float]]], platform: str, output_dir: Path
) -> list[str]:
    ...
    for tport in sorted(
        ...
        spec = TransportChartSpec(
            mpl, lookup, platform, output_dir, tport, ordered_blocks, colors
        )
...
def main() -> None:
    ...
    if not data_cols:
        log.warning(
            "[WARN] No direction+transport combos with numeric bandwidth data found"
        )
