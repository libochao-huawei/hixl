...
def _build_table_matrix(
    ...
    for bs in ordered_blocks:
        rows.append(
            [bs]
            + [format_bandwidth_cell(dname, tport, bs, lookup, platform) for dname, tport in cols]
        )
    return headers, rows
    ...
    if pid == "a2":
        ...
        if deployment == "dual":
            ...
            lines.append(
                ...
                "测量不同 block size（16K～2M）下的有效带宽。\n\n"
                "**约束**：**HCCS** 在 A2（Ascend910B-class）上仅 **D2D**（`D2rD` / `rD2D`）；"
                "涉及 Host 或其它方向请使用 **RDMA**。\n\n"
            )
    elif pid == "a3":
        lines.append(
            ...
            "测量不同 block size（16K～2M）下的有效带宽。\n\n"
            "**约束**：**HCCS** 在 A3（Ascend910-class）上支持 **D2D**（`D2rD` / `rD2D`）以及 "
            "**H2rD** / **rD2H**；其余方向请使用 **RDMA** 或 **FabricMem**。\n\n"
        )
    elif pid == "a5":
        lines.append(
            ...
            "**测试方式**：在同一台机器上运行 `hixl_comm_bench` 的 target 和 initiator，"
            "测量不同 block size（16K～2M）下的有效带宽。\n\n"
            "**约束**：**Ascend950-class 产品上 comm benchmark 不使用 HCCS**；通信方向请查看 **RDMA** 或 **FabricMem** 列。"
            "表中 **HCCS** 列均为 **不支持**。\n\n"
        )
    lines.append(
        "### 如何读懂表格\n\n"
        ...
        "（例如 A5 不提供 HCCS comm；A2 上 HCCS 仅 D2D）；"
        "**——** 表示尚未采集到数据。\n"
        "- 所有数据来自"
    )
    if deployment == "dual":
...
def main() -> None:
    ...
    if deployment == "dual":
        print("[INFO] Detected dual-machine benchmark data from CSV pattern", file=sys.stderr)
    ...
    # --- Terminal output ---
    print("\n" + "=" * 70)
    print(f"  {platform_title(platform)}")
    print("=" * 70)
    print("  Bandwidth (GB/s, decimal)")
    print()
    print(render_table_terminal(lookup, platform, deployment))
