...
int32_t RunClient(const char *local_engine, const char *remote_engine, uint16_t tcp_port, const std::string &transfer_mode, 
  ...
  if (is_host) {
    RETURN_CHECK_ACL(aclrtMallocHost(&tmp, kTransferMemSize)); 
  } else {
    RETURN_CHECK_ACL(aclrtMalloc(&tmp, kTransferMemSize, ACL_MEM_MALLOC_HUGE_ONLY));
  }
...
int32_t RunServer(const char *local_engine, const char *remote_engine, uint16_t tcp_port, const std::string &transfer_mode, 
  ...
  if (is_host){
    RETURN_CHECK_ACL(aclrtMallocHost(&buffer, kTransferMemSize));
  } else{
    RETURN_CHECK_ACL(aclrtMalloc(&buffer, kTransferMemSize, ACL_MEM_MALLOC_HUGE_ONLY));
  }
...
int32_t main(int32_t argc, char **argv) {
  ...
  RETURN_CHECK_ACL(aclrtSetDevice(device));
  ...
  RETURN_CHECK_ACL(aclrtResetDevice(device));
  return ret;
}