...
void PrintUsage(FILE *out) {
  fprintf(out,
          ...
          "  --remote_engine|-e   client: remote HIXL host:port; TCP client uses host + --tcp_port\n"
          "                        server: optional (TCP binds --tcp_port only; default kept)\n"
...
bool ValidateConfig(const BenchmarkConfig &cfg) {
  ...
  if (cfg.role == Role::kClient) {
    if (cfg.remote_engine.find(':') == std::string::npos) {
      fprintf(stderr,
              "[ERROR] client remote_engine must be host:port (e.g. 127.0.0.1:16001) for HIXL and TCP host\n");
      return false;
    }
  }
...
std::string ExtractTcpHost(const std::string &remote_engine) {
...
bool SendLocalAddr(TCPServer *tcp_server, uint16_t tcp_port, uint64_t addr_to_send) {
  ...
  if (!tcp_server->SendUint64(addr_to_send)) {
    return false;
  }
  return tcp_server->SendTaskStatus();
}

bool RecvNotify(TCPServer *tcp_server) { return tcp_server->ReceiveTaskStatus(); }
...
int32_t RunServer(const BenchmarkConfig &cfg, RunParam *p) {
  ...
  if (!SendLocalAddr(&p->tcp_server, cfg.tcp_port, addr)) {
    return -1;
  }

  printf("[INFO] Wait transfer begin\n");
  if (RecvNotify(&p->tcp_server)) {
