...
int32_t RunClient(const char *local_engine, const char *remote_engine, uint16_t tcp_port, const std::string &transfer_mode, 
  ...
  if (is_host) {
    CHECK_ACL_RET(aclrtMallocHost(&tmp, kTransferMemSize)); 
  } else {
    CHECK_ACL_RET(aclrtMalloc(&tmp, kTransferMemSize, ACL_MEM_MALLOC_HUGE_ONLY));
  }
...
int32_t RunServer(const char *local_engine, const char *remote_engine, uint16_t tcp_port, const std::string &transfer_mode, 
  ...
  if (is_host){
    CHECK_ACL_RET(aclrtMallocHost(&buffer, kTransferMemSize));
  } else{
    CHECK_ACL_RET(aclrtMalloc(&buffer, kTransferMemSize, ACL_MEM_MALLOC_HUGE_ONLY));
  }
...
int32_t main(int32_t argc, char **argv) {
  ...
  CHECK_ACL_RET(aclrtSetDevice(device));
  ...
  CHECK_ACL_RET(aclrtResetDevice(device));
  return ret;
}