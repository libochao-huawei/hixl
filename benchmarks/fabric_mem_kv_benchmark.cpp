...
#define CHECK_ACL(x)                                                                 \
  do {                                                                               \
    aclError __r = (x);                                                              \
    if (__r != ACL_ERROR_NONE) {                                                     \
      std::cerr << __FILE__ << ":" << __LINE__ << " aclError:" << __r << std::endl;  \
      return __r;                                                                    \
    }                                                                                \
  } while (0)
...
void WaitForSyncDirReady(const std::string &path) {
  namespace fs = std::filesystem;
  const std::string flag = path + "/" + kSyncReadyFile;
  constexpr int kMaxWaits = 600000;
...
size_t PeerSliceBytes(int total_blocks, int world_size) {
  const int blocks_per_peer = total_blocks / world_size;
...
bool RunBenchmarkForBlockConfig(AdxlEngine &engine, int rank, int world_size,
  ...
  for (int it = 0; it < kWarmupIterations + kTimedIterations; ++it) {
    const bool count_this = (it >= kWarmupIterations);
    ...
    if (rank == 0) {
      ...
      FillPattern(host_pool, nbytes, it + total_blocks);
...
int main(int argc, char **argv) {
  ...
  std::vector<uintptr_t> peer_host_va(static_cast<size_t>(world_size), 0);
  for (int r = 0; r < world_size; ++r) {
