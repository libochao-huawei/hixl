...
class KvTransferExecutor {
 ...
 private:
  ...
  bool AcquireWorkerTask(KeyTransferTask *task, TransferOp *op, bool *trace_transfer);
  void RunWorkerTask(std::uint32_t worker_id, const KeyTransferTask &task, TransferOp op, bool trace_transfer);
