...
void PrintTransferPlanSummary(const KvBenchConfig &cfg,
  ...
  std::cout << "[TRACE] rank=" << cfg.rank << " transfer_plan op=" << TransferOpName(op)
            ...
            << " segments=" << descs_by_rank.size() << " self_local_copy=1" << std::endl;
...
void GeneratePlacements(const KvBenchConfig &cfg, const std::vector<std::uint64_t> &rank_pool_sizes,
                        WorkloadTransferState *state) {
...
void EnsurePlacementMetadata(const KvBenchConfig &cfg, const std::vector<std::uint64_t> &rank_pool_sizes,
                             WorkloadTransferState *state) {
  if (state->placements_ready) {
    return;
  }
  GeneratePlacements(cfg, rank_pool_sizes, state);
}
...
std::map<std::uint32_t, std::vector<TransferOpDesc>> BuildPutTransferDescs(
    const KvBenchConfig &cfg, const std::vector<RankMeta> &metas,
...
TransferStageTiming ExecuteKvTransfer(const KvBenchConfig &cfg, KvTransferExecutor *transfer_executor,
  ...
  const auto descs_by_rank = op == hixl::WRITE ? BuildPutTransferDescs(cfg, metas, rank_pool_sizes, state)
  ...
  transfer_executor->Transfer(op, descs_by_rank, trace_transfer);
...
KvBenchResult RunWorkload(const KvBenchConfig &cfg, KvRuntime *runtime, KvTransferExecutor *transfer_executor,
  ...
  // Always measure write then read (put_get workload).
  if (!model.IsShared() || cfg.rank == 0U) {
    put = MeasureRepeated(
        cfg, "workload_" + std::to_string(index) + "_put",
        [&](bool trace_transfer) {
          return ExecuteKvTransfer(cfg, transfer_executor, metas, workload, hixl::WRITE, rank_pool_sizes,
                                   &transfer_state, trace_transfer);
        },
        false,
        [&](const TransferStageTiming &timing) {
          PrintStageTiming(cfg, hixl::WRITE, workload, timing.plan_us, timing.transfer_us);
        });
  }
  Barrier(cfg, "workload_" + std::to_string(index) + "_put_done");
  EnsurePlacementMetadata(cfg, rank_pool_sizes, &transfer_state);
