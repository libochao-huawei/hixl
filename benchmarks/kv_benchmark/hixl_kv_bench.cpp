...
void InitRuntime(const KvBenchConfig &cfg, std::uint64_t local_size, std::uint64_t pool_size, KvRuntime *runtime) {
  ...
  if (aclrtMalloc(&runtime->local_buffer, static_cast<size_t>(local_size), ACL_MEM_MALLOC_HUGE_ONLY) != ACL_ERROR_NONE) {
...
const ModelSpec *ResolveKvModel(const KvBenchConfig &cfg) {
...
int RunKvBenchParsed(KvBenchConfig &cfg, KvRuntime *runtime, std::vector<RankMeta> *metas) {
  ...
  const ModelSpec *model = ResolveKvModel(cfg);
  if (model == nullptr) {
    return 1;
  }
