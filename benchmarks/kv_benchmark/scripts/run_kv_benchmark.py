...
def parse_args():
    ...
    parser.add_argument(
        "--token_lengths",
        default=None,
        help="Legacy token lengths; converted by tokens_per_key",
    )
    parser.add_argument("--batch_size", type=int, default=128)
    parser.add_argument(
        "--op_type", choices=["put", "get", "put_get"], default="put_get"
    )
...
def main():
    args = parse_args()
    if args.key_counts and args.token_lengths:
        print(
            "[ERROR] --key_counts and --token_lengths are mutually exclusive",
            flush=True,
        )
        return 1
    ...
    print(
        "[INFO] model=%s model_config=%s key_counts=%s token_lengths=%s segment_size=%s transport=%s output_dir=%s"
        % (
            args.model,
            model_config,
            key_counts if not args.token_lengths else "",
            args.token_lengths or "",
            args.segment_size,
            args.transport,
            args.output_dir,
        ),
    ...
    for rank in range(args.num_processes):
        ...
        if args.token_lengths:
            cmd.append(f"--token_lengths={args.token_lengths}")
        else:
            cmd.append(f"--key_counts={key_counts}")
        print("[INFO]", " ".join(cmd), flush=True)
