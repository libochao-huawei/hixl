...
def parse_args():
    ...
    parser.add_argument(
        "--trace",
        action="store_true",
        help="Enable [TRACE] logs on rank 0 (default: off)",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help="Transport path (default: rdma on A2, fabric_mem on A3/A5; uboe, ubg, ub only available on A5)",
    )
...
def _log_run_config(
    args, platform_id: str, devices: list[str], model_config: str, key_counts: str
) -> None:
    log.info(
...
def main():
    ...
    for rank in range(args.num_processes):
        cmd = _kv_process_cmd(
            KvProcessSpec(
                args, bench_bin, rank, devices[rank], model_config, run_id, key_counts
            )
        )
        log.info("[INFO] %s", " ".join(cmd))
