...
def find_bench_bin() -> str:
    ...
    candidates = [Path(__file__).resolve().parents[3] / 'build' / 'benchmarks' / 'kv_benchmark' / 'hixl_kv_bench', Path('build/benchmarks/kv_benchmark/hixl_kv_bench'), Path('./hixl_kv_bench')]
    for p in candidates:
        if p.exists():
            return str(p)
...
def find_model_config() -> str:
    ...
    candidates = [Path(__file__).resolve().parents[1] / 'config' / 'models.json', Path('kv_benchmark/config/models.json'), Path('benchmarks/kv_benchmark/config/models.json')]
    for p in candidates:
        if p.exists():
            return str(p)
...
def parse_args():
    ...
    parser.add_argument('--platform', choices=['a2', 'a3', 'a5'], default=None, help='Ascend platform; A3 uses 16 KV ranks/devices when not overridden explicitly')
    ...
    parser.add_argument('--local_buffer_min', default='1G', help='Minimum device local buffer size (e.g. 1G); allocation is max(this, workload key span)')
    ...
    parser.add_argument('--key_counts', default=None, help='KV block/key counts, comma-separated (default: 16,32,48,64)')
    parser.add_argument('--transfer_threads', type=int, default=8, help='Worker threads for concurrent key transfers (default: 8)')
    parser.add_argument('--transport', choices=['rdma', 'fabric_mem'], default=None, help='Transport path (default: rdma on A2, fabric_mem on A3/A5)')
...
def main():
    ...
    if len(devices) < target_processes:
        log.error('[ERROR] resolved device count (%d) is less than required processes (%d)' % (len(devices), target_processes))
        return 1
    args.num_processes = len(devices)
    prepare_output_dir(args.output_dir)
    log.info('[INFO] platform=%s num_processes=%s devices=%s model=%s model_config=%s key_counts=%s transport=%s output_dir=%s' % (platform_id, args.num_processes, ','.join(devices), args.model, model_config, key_counts, args.transport, args.output_dir))
    procs = []
    for rank in range(args.num_processes):
        cmd = [bench_bin, f'--rank={rank}', f'--device_id={devices[rank]}', f'--num_processes={args.num_processes}', f'--local_buffer_min={args.local_buffer_min}', f'--pool_memory={args.pool_memory}', f'--model={args.model}', f'--model_config={model_config}', f'--transfer_threads={args.transfer_threads}', f'--transport={args.transport}', f'--output_dir={args.output_dir}', f'--run_id={run_id}', f'--base_port={args.base_port}', f'--listen_host={args.listen_host}', f'--connect_host={args.connect_host}', f'--warmup={args.warmup}', f'--repeat={args.repeat}', f'--sync_timeout_sec={args.sync_timeout_sec}', f'--key_counts={key_counts}']
        log.info('[INFO] %s', ' '.join(cmd))
