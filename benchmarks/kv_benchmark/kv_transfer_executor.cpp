...
void TraceSegment(const char *stage, std::uint32_t local_rank, const SegmentTransferTask &task, TransferOp op,
                  std::mutex *trace_mu, std::uint64_t elapsed_us = 0U) {
  if (trace_mu == nullptr || task.descs == nullptr) {
    return;
  }
  std::lock_guard<std::mutex> guard(*trace_mu);
  std::cout << "[TRACE] kv_transfer " << stage << " rank=" << local_rank << " op=" << OpName(op)
            << " segment=" << task.segment_id << " endpoint=" << task.endpoint << " descs=" << task.descs->size()
...
void TraceLocalCopy(const char *stage, std::uint32_t local_rank, const SegmentTransferTask &task, TransferOp op,
                    std::mutex *trace_mu, std::uint64_t elapsed_us = 0U) {
  if (trace_mu == nullptr || task.descs == nullptr) {
    return;
  }
  std::lock_guard<std::mutex> guard(*trace_mu);
  std::cout << "[TRACE] local_copy " << stage << " rank=" << local_rank << " op=" << OpName(op)
            << " segment=" << task.segment_id << " descs=" << task.descs->size()
            << " bytes=" << SumBytes(*task.descs);
  if (elapsed_us != 0U) {
    std::cout << " elapsed_us=" << elapsed_us;
  }
...
void RunLocalSegmentCopy(std::uint32_t local_rank, const SegmentTransferTask &task, TransferOp op,
                         const char *(*recent_errmsg)(), std::mutex *trace_mu) {
  TraceLocalCopy("begin", local_rank, task, op, trace_mu);
  ...
  TraceLocalCopy("end", local_rank, task, op, trace_mu, elapsed_us);
}

void RunOneRemoteSegment(hixl::Hixl &hixl, std::uint32_t local_rank, const SegmentTransferTask &task, TransferOp op,
                         std::int32_t timeout_ms, const char *(*recent_errmsg)(), std::mutex *trace_mu) {
  if (task.descs == nullptr || task.descs->empty()) {
    throw std::runtime_error("internal error: empty remote segment transfer task");
  }
  TraceSegment("begin", local_rank, task, op, trace_mu);
  const auto start = std::chrono::steady_clock::now();
  ...
  const auto elapsed_us = static_cast<std::uint64_t>(
      std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count());
  TraceSegment("end", local_rank, task, op, trace_mu, elapsed_us);
}
...
void KvTransferExecutor::StartWorkers() {
  ...
  try {
    for (std::uint32_t i = 0U; i < worker_count_; ++i) {
      workers_.emplace_back(&KvTransferExecutor::WorkerLoop, this);
    }
    WaitWorkersReady();
  } catch (...) {
    StopWorkers();
...
void KvTransferExecutor::WorkerLoop() {
  try {
  ...
  while (true) {
    ...
    try {
      if (task.is_self) {
        RunLocalSegmentCopy(self_rank_, task, op, recent_errmsg_, trace_transfer ? &trace_mu_ : nullptr);
      } else {
        RunOneRemoteSegment(*hixl_, self_rank_, task, op, timeout_ms_, recent_errmsg_,
