...
int main(int argc, char **argv) {
  ...
  const int device_id = std::stoi(argv[1]);
  const std::string local_engine = argv[2];
  const std::string mode = argv[3];
  const size_t bytes = argc == 5 ? static_cast<size_t>(std::stoull(argv[4])) : kDefaultBytes;
  ...
  auto init_ret = hixl_engine.Initialize(local_engine.c_str(), options);
  if (init_ret != SUCCESS) {
    std::cerr << "[ERROR] Initialize failed ret=" << init_ret << " errmsg=" << RecentErrMsg() << "\n";
    return 1;
  }
  std::cout << "[INFO] Initialize success device=" << device_id << " engine=" << local_engine
            << " mode=" << mode << " bytes=" << bytes << "\n";

  Allocation alloc;
  if (!Allocate(device_id, mode, bytes, alloc)) {
    std::cout << "[RESULT] mode=" << mode << " allocate=failed register=not_run\n";
    hixl_engine.Finalize();
    (void)aclrtResetDevice(device_id);
    aclFinalize();
    return 1;
  }
  std::cout << "[INFO] Allocate success mode=" << mode << " addr=" << alloc.ptr << "\n";
  if (alloc.seed_pa) {
  ...
  auto reg_ret = hixl_engine.RegisterMem(desc, MEM_DEVICE, handle);
  if (reg_ret != SUCCESS) {
    std::cout << "[RESULT] mode=" << mode << " allocate=success register=failed ret=" << reg_ret
              << " errmsg=" << RecentErrMsg() << "\n";
    hixl_engine.Finalize();
  ...
  std::cout << "[RESULT] mode=" << mode << " allocate=success register=success handle=" << handle << "\n";
