...
bool BenchmarkConfigParser::ApplyTransportEnvironment(const BenchmarkConfig &cfg) {
  ...
  // hccs / fabric_mem / uboe / ubg: do not modify HCCL environment variables (fabric_mem uses init options only)
  return true;
}
...
bool ValidateTransport(const std::string &transport) {
  if (transport != "hccs" && transport != "rdma" && transport != "fabric_mem" && transport != "uboe" &&
      transport != "ubg") {
    fprintf(stderr, "[ERROR] Invalid transport: %s (expect hccs|rdma|fabric_mem|uboe|ubg)\n", transport.c_str());
    return false;
  }
  return true;
}
