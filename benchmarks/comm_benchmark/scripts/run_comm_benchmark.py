...
TRANSPORTS_A5 = ["rdma", "roce", "fabric_mem", "uboe", "ub"]
...
def _apply_csv_transport_row(
    row: dict[str, str], transports: set[str], directions: set[str]
) -> None:
...
def infer_platform_from_csv_dir(csv_dir: Path) -> str | None:
    ...
    if not all_transports:
        return None
    if "hccs" in all_transports and (
        "H2rD" in all_directions or "rD2H" in all_directions
    ):
        return "a3"
...
def resolve_report_platform(
    args, gate_soc: str | None = None, csv_dir: Path | None = None
) -> str:
    ...
    if csv_dir is not None:
        ...
        if inferred is not None:
            log.info(
                f"[INFO] Infer report platform {inferred} from CSV transports/directions"
            )
            return inferred
...
def supported_types_for_transport(transport: str, gate_soc: str | None) -> list[str]:
    ...
    return [
        bench_type
        for bench_type in ALL_TYPES
        if hccs_combo_allowed(transport, bench_type, gate_soc)
    ]
...
def resolve_bench_types(
    args, gate_soc: str | None, transport: str | None = None
) -> list[str]:
...
def build_dual_run_plan(args, gate_soc: str | None) -> list[tuple[str, list[str]]]:
    ...
    transports = (
        supported_transports_for_soc(gate_soc, dual=True)
...
def _resolve_one_to_many_initiator(args, devices: list[int]) -> DualTopology | None:
    ...
    if len(devices) != 1:
        log.warning(
            "[WARN] one_to_many initiator uses the first --device_ids entry only"
        )
...
def _many_to_one_initiator_ids(args, devices: list[int]) -> list[int] | None:
    ...
    log.info(
        f"[ERROR] --num_initiators={args.num_initiators} exceeds --device_ids count ({len(devices)})"
    )
    return None
...
def resolve_dual_topology(args, devices: list[int], role: str) -> DualTopology | None:
    if args.pattern == "one_to_many":
        resolver = (
            _resolve_one_to_many_target
            if role == "target"
            else _resolve_one_to_many_initiator
        )
        return resolver(args, devices)
    if args.pattern == "many_to_one":
        resolver = (
            _resolve_many_to_one_target
            if role == "target"
            else _resolve_many_to_one_initiator
        )
        return resolver(args, devices)
...
def _add_topology_args(parser) -> None:
    parser.add_argument(
        "--pattern",
        choices=["pairwise", "one_to_many", "many_to_one"],
        default="pairwise",
    )
    parser.add_argument(
        "--num_targets",
    ...
    parser.add_argument(
        "--bench_bin", default=None, help="Path to hixl_comm_bench (auto-detected)"
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
...
def _add_transfer_args(parser) -> None:
    ...
    parser.add_argument(
        "--transport",
        choices=["hccs", "rdma", "roce", "fabric_mem", "uboe", "ub", "all"],
        default=None,
    ...
    parser.add_argument(
        ...
        "LocalCommRes is used). Note: this is separate from host IP (--target-host) used for TCP coordination (control plane).",
    )
...
def _peer_launcher_common_args(spec: PeerLauncherSpec) -> list[str]:
    ...
    transport = (
        spec.transport_flag if spec.transport_flag is not None else args.transport
    )
    cmd = [
        "python3",
        initiator_script,
...
def build_peer_initiator_launcher_cmd(
    ...
    device_ids = (
        ...
        else resolve_default_device_ids("initiator")
    )
...
def _dual_target_lanes(
    topology: DualTopology, local_ip: str, base_hixl_port: int, base_tcp_port: int
):
...
def _dual_initiator_lanes(
    topology: DualTopology, target_host: str, base_hixl_port: int, base_tcp_port: int
):
    if topology.pattern == "one_to_many":
        remotes = ",".join(
            (
                endpoint(target_host, base_hixl_port + i)
                for i in range(topology.target_count)
            )
        )
...
def _print_peer_initiator_hint(
    ctx: DualTargetContext, run_plan: list[tuple[str, list[str]]]
):
    args = ctx.args
    ...
    log.info(
        f"  HIXL base port: {ctx.base_hixl_port},  TCP base port: {ctx.base_tcp_port}"
    )
    if is_all_transports_mode(args):
...
def _run_dual_target_step(
    ctx: DualTargetContext, transport: str, bench_type: str
) -> int:
    args = ctx.args
    ...
    procs = []
    try:
        for lane in _dual_target_lanes(
            ctx.topology, ctx.local_ip, ctx.base_hixl_port, ctx.base_tcp_port
        ):
...
def _run_target_directions(
    ...
    run_spec = DualDirectionsRunSpec(
        transport, bench_types, run_counter, _run_dual_target_step, "target"
    )
    return _run_dual_directions(ctx, run_spec)


def _launch_target(
    args, bench_bin: str, topology: DualTopology, run_plan: list[tuple[str, list[str]]]
):
    ...
    if total_runs > 1:
        log.info(
            f"[DUAL] Plan: {total_runs} run(s) — {_format_run_plan_summary(run_plan)}"
        )
...
def _run_dual_initiator_step(
    ctx: DualInitiatorContext, transport: str, bench_type: str
) -> int:
    args = ctx.args
    saved_transport = args.transport
    args.transport = transport
    procs = []
    try:
        for lane in _dual_initiator_lanes(
            ctx.topology, args.target_host, ctx.base_hixl_port, ctx.base_tcp_port
        ):
...
def _run_initiator_directions(
    ...
    run_spec = DualDirectionsRunSpec(
        transport, bench_types, run_counter, _run_dual_initiator_step, "initiator"
    )
    return _run_dual_directions(ctx, run_spec)


def _launch_initiator(
    args,
    bench_bin: str,
    topology: DualTopology,
    run_plan: list[tuple[str, list[str]]],
...
def infer_single_mode_counts(
    pattern: str, devices: list[int], num_targets, num_initiators
):
    if len(devices) < 2:
    ...
    if pattern == "one_to_many":
        if num_initiators not in (None, 1):
            ...
            return None
        return validate_single_mode_counts(
            num_targets or len(devices) - 1, 1, len(devices)
        )
    if pattern == "many_to_one":
        if num_targets not in (None, 1):
            ...
            return None
        return validate_single_mode_counts(
            1, num_initiators or len(devices) - 1, len(devices)
        )
    if (
        num_targets is not None
        and num_initiators is not None
        and (num_targets != num_initiators)
    ):
...
def validate_single_mode_counts(
    target_count: int, initiator_count: int, device_count: int
):
...
def initiator_peer_args(
    pattern: str, idx: int, target_endpoints: list[str], tcp_ports: list[int]
):
    if pattern == "one_to_many":
...
def _run_single_direction(
    args, bench_bin: str, devices: list[int], bench_type: str, tcp_accept_wait_s: int
) -> int:
    counts = infer_single_mode_counts(
        args.pattern, devices, args.num_targets, args.num_initiators
    )
    if counts is None:
        return -1
    ...
    target_endpoints = [
        endpoint(args.host, args.base_hixl_port + i) for i in range(target_count)
    ]
    ...
    for idx in range(initiator_count):
        (remotes, ports) = initiator_peer_args(
            args.pattern, idx, target_endpoints, tcp_ports
        )
...
def _launch_single(args, bench_bin: str, devices: list[int], bench_types: list[str]):
    ...
    for idx, bench_type in enumerate(bench_types, start=1):
        ...
        last_ret = _run_single_direction(
            args, bench_bin, devices, bench_type, tcp_accept_wait_s
        )
        if last_ret != 0:
            log.info(
...
def _normalize_launch_args(args) -> bool:
    ...
    if args.transport is None:
        ...
        log.error(
            "[ERROR] --transport=all is only supported in dual-machine mode (--role=target|initiator)"
        )
        return False
    if args.role in ("target", "initiator"):
...
def _validate_launch_plan(args, gate_soc, run_plan, single_bench_types) -> int | None:
    ...
    if is_all_directions_mode(args):
        if not single_bench_types:
            log.info(
                f"[ERROR] No supported directions for transport={args.transport} on this platform."
            )
            return -1
        return None
...
def _launch_by_role(spec: LaunchDispatchSpec):
    ...
    if args.role == "initiator":
        ...
        if topology is None:
            return -1
        return _launch_initiator(
            args, spec.bench_bin, topology, spec.run_plan, spec.gate_soc
        )
...
def launch(args):
    ...
    if not _normalize_launch_args(args):
        return -1
    run_plan = (
        build_dual_run_plan(args, gate_soc)
        if args.role in ("target", "initiator")
        else []
    )
    single_bench_types = (
        resolve_bench_types(args, gate_soc)
        if args.role not in ("target", "initiator")
        else []
    )
    ...
    return _launch_by_role(
        LaunchDispatchSpec(
            args, bench_bin, devices, run_plan, single_bench_types, gate_soc
        )
    )
