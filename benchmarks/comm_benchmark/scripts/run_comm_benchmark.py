...
def resolve_report_platform(
    args,
    gate_soc: str | None = None,
    ...
    if csv_dir is not None:
        ...
        if inferred is not None:
            print(f"[INFO] Infer report platform {inferred} from CSV transports/directions", flush=True)
            return inferred
    print("[WARN] SOC detection failed; report platform defaults to a2", flush=True)
    return "a2"
...
def resolve_dual_topology(args, devices: list[int], role: str) -> DualTopology | None:
    ...
    if pattern == "one_to_many":
        if role == "target":
            if not devices:
                print("[ERROR] one_to_many target requires at least one --device_id", flush=True)
                return None
            target_ids = devices
            if args.num_targets is not None:
                if args.num_targets <= 0 or args.num_targets > len(devices):
                    print(
                        f"[ERROR] --num_targets must be in 1..{len(devices)} for one_to_many target",
                        flush=True,
                    )
                    return None
        ...
        if len(devices) != 1:
            print("[WARN] one_to_many initiator uses the first --device_ids entry only", flush=True)
        if args.num_targets is not None:
            ...
            print(
                "[ERROR] one_to_many initiator requires --num_targets or multiple --device_ids",
                flush=True,
            )
            return None
        if target_count <= 0:
            print("[ERROR] one_to_many initiator --num_targets must be positive", flush=True)
            return None
    ...
    if pattern == "many_to_one":
        if role == "target":
            if not devices:
                print("[ERROR] many_to_one target requires one --device_id", flush=True)
                return None
            if len(devices) != 1:
                print("[WARN] many_to_one target uses the first --device_id only", flush=True)
            initiator_count = args.num_initiators if args.num_initiators is not None else 1
            if initiator_count <= 0:
                print("[ERROR] many_to_one target --num_initiators must be positive", flush=True)
                return None
            return DualTopology(pattern, [devices[0]], list(range(initiator_count)))
        if not devices:
            print("[ERROR] many_to_one initiator requires at least one --device_id", flush=True)
            return None
        if args.num_initiators is not None:
            if args.num_initiators <= 0:
                print("[ERROR] many_to_one initiator --num_initiators must be positive", flush=True)
                return None
            if len(devices) == 1 and args.num_initiators > 1:
                ...
                print(
                    f"[ERROR] --num_initiators={args.num_initiators} exceeds "
                    f"--device_ids count ({len(devices)})",
                    flush=True,
                )
                return None
        else:
    ...
    if not devices:
        print("[ERROR] dual-machine mode requires at least one --device_id", flush=True)
        return None
...
def parse_args():
    ...
    parser.add_argument("--num_targets", type=int, default=None,
                        help="Target lane count. Dual one_to_many: required on initiator; optional cap on target. "
                             "Single-machine: optional topology override.")
    parser.add_argument("--num_initiators", type=int, default=None,
                        help="Initiator lane count for dual many_to_one target (default 1). "
                             "Optional cap on initiator side. Single-machine: topology override.")
    ...
    parser.add_argument(
        "--device_ids",
        default=None,
        help="Device IDs (comma-separated). Dual-machine default: 0. Single-machine default: 0,1. "
             "one_to_many target: one ID per target lane; many_to_one initiator: one ID per initiator lane.",
    )
    parser.add_argument(
        "--type",
        choices=[*ALL_TYPES, "all"],
        default=None,
        help="Transfer direction (e.g. D2rD). Dual-machine default: all supported directions for "
             "--transport. Single-machine default: all directions when --transport is set, else D2rD.",
    )
    parser.add_argument("--transport", choices=["hccs", "rdma", "fabric_mem", "all"], default=None,
                        help="Transport path. Dual-machine default: all platform-supported transports. "
                             "Single-machine default: hccs.")
    parser.add_argument(
        "--soc_variant",
        choices=["auto", "a2", "a3", "a5"],
        default=None,
        help="SOC class forwarded to hixl_comm_bench (A5/Ascend950: no HCCS). Default: npu-smi hint or auto (ACL probe).",
    )
    ...
    parser.add_argument("--plot", dest="plot", action="store_true", default=True,
                        help="Generate PNG plots for CSVs produced by this run (default)")
    parser.add_argument("--skip_plot", "--skip-plot", dest="plot", action="store_false",
                        help="Skip PNG plot generation")
    parser.add_argument("--report", dest="report", action="store_true", default=True,
                        help="Generate perf.md report on dual-machine initiator (default)")
    parser.add_argument("--skip_report", dest="report", action="store_false",
                        help="Skip perf.md report generation")
    parser.add_argument("--report_path", default=None,
                        help="perf.md output path (default: {output_dir}/perf.md)")
    parser.add_argument(
        "-H",
        "--hixl_option",
        action="append",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Target TCP accept timeout per direction (hixl_comm_bench --tcp_accept_wait_s). "
             "Default: 30 for one direction, 300 when running all directions.",
    )
    parser.add_argument(
        "--inter_run_delay_s",
        type=int,
        default=DEFAULT_INTER_RUN_DELAY_SEC,
        help="Dual-machine: seconds to wait between sequential transport/direction runs "
             f"(default {DEFAULT_INTER_RUN_DELAY_SEC}).",
    )
    parser.add_argument(
        "--connect_timeout",
        type=int,
        default=DEFAULT_CONNECT_TIMEOUT_MS,
        help="Initiator TCP/HIXL connect timeout in ms (hixl_comm_bench --connect_timeout).",
    )
...
def build_target_cmd(
    ...
    tcp_client_count: int = 1,
) -> list[str]:
    return [
        bench_bin,
        "--role=target",
        f"--device_id={device_id}",
        f"--local_engine={endpoint(local_ip, hixl_port)}",
        f"--tcp_port={tcp_port}",
        f"--tcp_client_count={tcp_client_count}",
        f"--tcp_accept_wait_s={tcp_accept_wait_s}",
        *base_options(args, bench_type),
        *hixl_init_extra_args(args),
    ]


def build_initiator_cmd(
    bench_bin: str,
    args,
    bench_type: str,
    device_id: int,
    ...
    hixl_port: int,
    tcp_port: int | str,
    remote_engines: str | None = None,
    local_hixl_port: int | None = None,
) -> list[str]:
...
def _peer_launcher_common_args(
    args,
    local_ip: str,
    base_hixl_port: int,
    base_tcp_port: int,
    ...
    cmd = [
        ...
        f"--base_hixl_port={base_hixl_port}",
        f"--base_tcp_port={base_tcp_port}",
...
def print_average_summary(csvs):
    ...
    for csv_path in sorted(csvs):
        try:
            ...
            print(f"[WARN] failed to read {csv_path}: {exc}", flush=True)

    if not groups:
        return

    print("\n[AVG] Bandwidth summary (same mean as perf.md)", flush=True)
...
def generate_perf_report(args, gate_soc: str | None) -> None:
    ...
    if not list(csv_dir.glob("comm_result_*.csv")):
        print(f"[WARN] no comm CSV result found in {csv_dir}, skip perf.md", flush=True)
        return
    if not RENDER_PERF_SCRIPT.exists():
        print(f"[WARN] render script not found: {RENDER_PERF_SCRIPT}", flush=True)
        return
    ...
    if ret.returncode != 0:
        print(f"[WARN] perf.md generation failed with code {ret.returncode}", flush=True)
    else:
        print(f"[INFO] perf.md written to {report_path}", flush=True)
...
def _print_peer_initiator_hint(
    args,
    local_ip: str,
    base_hixl_port: int,
    base_tcp_port: int,
    ...
    initiator_cmd = build_peer_initiator_launcher_cmd(
        args, local_ip, base_hixl_port, base_tcp_port, transport_flag=transport_flag
    )
    total_runs = sum(len(bench_types) for _, bench_types in run_plan)
    sep = "=" * 60
    print(f"\n{sep}", flush=True)
    print(f"  Target ready.  Local IP: {local_ip}", flush=True)
    print(f"  Pattern: {topology.pattern}", flush=True)
    print(
        f"  Target lanes: {topology.target_count} (devices={format_device_ids(topology.target_device_ids)})",
        flush=True,
    )
    print(
    ...
    print(f"  HIXL base port: {base_hixl_port},  TCP base port: {base_tcp_port}", flush=True)
    if is_all_transports_mode(args):
        print(f"  Transports: {', '.join(t for t, _ in run_plan)}", flush=True)
    else:
        print(f"  Transport: {args.transport}", flush=True)
    if is_all_directions_mode(args):
        print(f"  Total runs: {total_runs} ({_format_run_plan_summary(run_plan)})", flush=True)
    elif total_runs == 1:
        print(f"  Direction: {run_plan[0][1][0]}", flush=True)
    print("  Run this on the INITIATOR machine:", flush=True)
...
def _dual_inter_run_delay(args, run_index: int) -> None:
    ...
    if run_index <= 1 or args.inter_run_delay_s <= 0:
        return
    print(f"[DUAL] Waiting {args.inter_run_delay_s}s before next run ...", flush=True)
    time.sleep(args.inter_run_delay_s)


def _run_dual_target_step(
    args,
    bench_bin: str,
    topology: DualTopology,
    transport: str,
    ...
    try:
        for lane in _dual_target_lanes(topology, local_ip, base_hixl_port, base_tcp_port):
            target_cmd = build_target_cmd(
                ...
                lane["tcp_port"],
                tcp_accept_wait_s,
                tcp_client_count=lane["tcp_client_count"],
            )
...
def _run_target_directions(
    ...
    for idx, bench_type in enumerate(bench_types, start=1):
        ...
        if ret != 0:
            print(
                f"[ERROR] Target failed for transport={transport} direction={bench_type} with code {ret}",
                flush=True,
            )
            return ret
        if len(bench_types) > 1:
...
def _launch_target(args, bench_bin: str, topology: DualTopology, run_plan: list[tuple[str, list[str]]]):
    ...
    if not run_plan:
        print("[ERROR] No transport/direction combinations to run on target", flush=True)
        return -1
    ...
    print(
        f"\n[DUAL] Starting target on {local_ip}, pattern={topology.pattern}, "
        f"lanes={topology.target_count}",
        flush=True,
    )
    if total_runs > 1:
        print(f"[DUAL] Plan: {total_runs} run(s) — {_format_run_plan_summary(run_plan)}", flush=True)
    ...
    for t_idx, (transport, bench_types) in enumerate(run_plan, start=1):
        if len(run_plan) > 1:
            print(
                f"\n[DUAL] Transport [{t_idx}/{len(run_plan)}] {transport} — "
                f"{len(bench_types)} direction(s)",
                flush=True,
            )
        ret = _run_target_directions(
            args,
            bench_bin,
            topology,
            transport,
    ...
    if total_runs > 1:
        print(f"[DUAL] All {total_runs} run(s) completed on target", flush=True)
    return last_ret


# ------- Dual-machine: initiator side -------

def _run_dual_initiator_step(
    args,
    bench_bin: str,
    topology: DualTopology,
    transport: str,
    ...
    try:
        for lane in _dual_initiator_lanes(topology, args.target_host, base_hixl_port, base_tcp_port):
            initiator_cmd = build_initiator_cmd(
                ...
                base_hixl_port,
                lane["tcp_port"],
                remote_engines=lane["remote_engines"],
                local_hixl_port=lane["local_hixl_port"],
            )
...
def _run_initiator_directions(
    ...
    for idx, bench_type in enumerate(bench_types, start=1):
        ...
        ret = _run_dual_initiator_step(
            args, bench_bin, topology, transport, bench_type, base_hixl_port, base_tcp_port
        )
        if ret != 0:
            print(
                f"[ERROR] Initiator failed for transport={transport} direction={bench_type} "
                f"with code {ret}",
                flush=True,
            )
            return ret
...
def _launch_initiator(args, bench_bin: str, topology: DualTopology, run_plan: list[tuple[str, list[str]]], gate_soc):
    ...
    if not args.target_host:
        ...
        return -1
    if not run_plan:
        print("[ERROR] No transport/direction combinations to run on initiator", flush=True)
        return -1
    ...
    if total_runs > 1:
        print(
            ...
            f"initiator lanes={topology.initiator_count}; "
            f"{total_runs} run(s) — {_format_run_plan_summary(run_plan)}",
            flush=True,
        )
    else:
    ...
    if total_runs > 1:
        print(f"[DUAL] All {total_runs} run(s) completed on initiator", flush=True)
    return last_ret
...
def infer_single_mode_counts(pattern: str, devices: list[int], num_targets, num_initiators):
    if len(devices) < 2:
        print("[ERROR] single-machine mode requires at least two device IDs", flush=True)
        return None
    if num_targets is not None and num_targets <= 0:
        print("[ERROR] --num_targets must be positive", flush=True)
        return None
    if num_initiators is not None and num_initiators <= 0:
        print("[ERROR] --num_initiators must be positive", flush=True)
        return None

    if pattern == "one_to_many":
        if num_initiators not in (None, 1):
            print("[ERROR] one_to_many supports exactly one initiator", flush=True)
            return None
        return validate_single_mode_counts(num_targets or (len(devices) - 1), 1, len(devices))

    if pattern == "many_to_one":
        if num_targets not in (None, 1):
            print("[ERROR] many_to_one supports exactly one target", flush=True)
            return None
    ...
    if num_targets is not None and num_initiators is not None and num_targets != num_initiators:
        print("[ERROR] pairwise requires --num_targets == --num_initiators", flush=True)
        return None
    pair_count = num_targets or num_initiators
    if pair_count is None:
        if len(devices) % 2 != 0:
            print("[ERROR] pairwise mode requires an even number of device IDs", flush=True)
            return None
...
def validate_single_mode_counts(target_count: int, initiator_count: int, device_count: int):
    if target_count + initiator_count > device_count:
        print(
            f"[ERROR] device_ids has {device_count} device(s), but topology needs "
            f"{target_count + initiator_count}",
            flush=True,
        )
        return None
        ...
        procs.append(
            start_process(
                [bench_bin, "--role=target",
                 f"--device_id={devices[idx]}",
                 f"--local_engine={target}",
                 f"--tcp_port={tcp_ports[idx]}",
                 f"--tcp_client_count={target_peer_count(args.pattern, initiator_count)}",
                 f"--tcp_accept_wait_s={tcp_accept_wait_s}",
                 *base_options(args, bench_type),
                 *hixl_init_extra_args(args)]
            )
        ...
        procs.append(
            start_process(
                [bench_bin, "--role=initiator",
                 f"--device_id={devices[target_count + idx]}",
...
def _launch_single(args, bench_bin: str, devices: list[int], bench_types: list[str]):
    ...
    if not bench_types:
        print("[ERROR] No supported directions to run in single-machine mode", flush=True)
        return -1
    ...
    for idx, bench_type in enumerate(bench_types, start=1):
        ...
        if len(bench_types) > 1:
            print(
                f"[SINGLE] [{idx}/{len(bench_types)}] direction={bench_type} — starting ...",
                flush=True,
            )
        last_ret = _run_single_direction(args, bench_bin, devices, bench_type, tcp_accept_wait_s)
        if last_ret != 0:
            print(
                f"[ERROR] Single-machine run failed for direction={bench_type} with code {last_ret}",
                flush=True,
            )
            return last_ret
        if len(bench_types) > 1:
            print(
                f"[SINGLE] [{idx}/{len(bench_types)}] direction={bench_type} — done",
                flush=True,
            )
...
def launch(args):
    ...
    if args.transport is None:
        ...
        print("[ERROR] --transport=all is only supported in dual-machine mode (--role=target|initiator)", flush=True)
        return -1
    ...
    if args.role in ("target", "initiator"):
        if not run_plan:
            print(
                "[ERROR] No supported transport/direction combinations on this platform "
        ...
        if not single_bench_types:
            print(
                "[ERROR] No supported directions for "
                f"transport={args.transport} on this platform.",
                flush=True,
            )
            return -1
    elif not hccs_combo_allowed(args.transport, args.type, gate_soc):
        print(
            "[ERROR] transport=hccs: not supported on Ascend950-class (A5); on A2 only D2D; on A3 also H2rD|rD2H; "
            "otherwise use rdma or fabric_mem.",
            flush=True,
        )
        return -1
