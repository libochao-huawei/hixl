...
int32_t DoTransfer(HixlClientHandle client_handle, uint8_t *local_addr, uint8_t *remote_addr,
                   const std::string &transfer_op, const char *thread_name, TransferResult *result) {
  HixlOneSideOpDesc desc = {.remote_buf = remote_addr, .local_buf = local_addr, .len = kTransferMemSize};
...
int32_t RunClientThread(const std::string &connect_engine, const std::string &local_ep_str,
                        const std::string &remote_ep_str, const std::string &transfer_mode,
  ...
  if (InitEndPointInfo(local_ep_str, local_ep) != 0 || InitEndPointInfo(remote_ep_str, remote_ep) != 0) {
  ...
  if (DoTransfer(client_handle, static_cast<uint8_t *>(mem.addr), remote_addr, transfer_op, thread_name, &result) !=
      0) {
    ClientFinalize(client_handle, {mem_handle});
    return -1;
  }
...
// tcp_port: TCP控制通道端口
int32_t RunServerThread(const std::string &listen_engine, const std::string &local_ep_str,
                        const std::string &remote_ep_str, uint16_t tcp_port, const std::string &transfer_mode,
                        const std::string &transfer_op, const char *thread_name) {
...
void PrintUsage(const char *prog_name) {
  (void)printf(
      "Usage: %s <device_id> <listen_engine> <connect_engine> <tcp_port> "
  ...
  (void)printf(
      "  %s 0 192.168.1.161:19998 192.168.1.163:19999 19997 h2h write client "
      "'{\"location\":\"host\",\"protocol\":\"roce\",\"addr\":\"192.168.1.161\"}' "
      "'{\"location\":\"host\",\"protocol\":\"roce\",\"addr\":\"192.168.1.163\"}'\n",
      prog_name);
  (void)printf("\nOn Node B (server role):\n");
  (void)printf(
      "  %s 0 192.168.1.163:19999 192.168.1.161:19998 19998 h2h write server "
      "'{\"location\":\"host\",\"protocol\":\"roce\",\"addr\":\"192.168.1.163\"}' "
      "'{\"location\":\"host\",\"protocol\":\"roce\",\"addr\":\"192.168.1.161\"}'\n",
      prog_name);
}
...
int32_t main(int32_t argc, char **argv) {
  ...
  if (args.my_role == "client") {
    ...
    threads.emplace_back([&args, &results]() {
      int32_t ret = RunClientThread(args.connect_engine, args.local_ep, args.remote_ep, args.transfer_mode,
                                    args.transfer_op, "Thread-Client");
      if (ret != 0) results.fetch_or(1);
    });

    threads.emplace_back([&args, &results]() {
      int32_t ret = RunServerThread(args.listen_engine, args.local_ep, args.remote_ep, args.tcp_port,
                                    args.transfer_mode, args.transfer_op, "Thread-Server");
