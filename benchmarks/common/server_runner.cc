...
bool ServerRunner::AllocServerBufferForRun() {
  ...
  if (is_host_) {
    ...
    aclError ar_memset = aclrtMemset(buffer_, alloc_size, static_cast<uint8_t>('S'), alloc_size);
    if (ar_memset != ACL_ERROR_NONE) {
      std::printf("[ERROR] server aclrtMemset failed acl=%d\n", static_cast<int>(ar_memset));
      return false;
    }
  }
  return true;
