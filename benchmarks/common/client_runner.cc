...
int32_t SubmitAsyncRequests(Hixl &hixl_engine, const TransferBlockStepCtx &ctx, uint64_t per_req_size,
  ...
  for (uint32_t batch_idx = 0; batch_idx < ctx.cfg->async_batch_num; ++batch_idx) {
    ...
    const uintptr_t req_base = ctx.base + batch_idx * per_req_size;
    const uintptr_t req_remote_base = ctx.dst_addr + batch_idx * per_req_size;
...
int32_t WaitAsyncRequests(Hixl &hixl_engine, AsyncTransferContext &async_ctx) {
  ...
  while (has_waiting && std::chrono::steady_clock::now() < deadline) {
    ...
    for (size_t i = 0; i < async_ctx.reqs.size(); ++i) {
      if (statuses[i] == TransferStatus::WAITING) {
        if (hixl_engine.GetTransferStatus(async_ctx.reqs[i], statuses[i]) != SUCCESS) {
          std::printf("[ERROR] GetTransferStatus failed at req %zu\n", i);
          return -1;
        }
        if (statuses[i] == TransferStatus::WAITING) has_waiting = true;
        else if (statuses[i] != TransferStatus::COMPLETED) {
          std::printf("[ERROR] Transfer failed at req %zu, status=%d\n", i, static_cast<int>(statuses[i]));
          return -1;
        }
      }
    }
    if (has_waiting) std::this_thread::sleep_for(std::chrono::microseconds(1));
  }
...
int32_t TransferOneBlockStepAsync(Hixl &hixl_engine, const TransferBlockStepCtx &ctx) {
  ...
  if (SubmitAsyncRequests(hixl_engine, ctx, per_req_size, block_size, per_req_trans_num, async_ctx) != 0) return -1;
