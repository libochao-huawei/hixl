...
int32_t TransferOneBlockStep(Hixl &hixl_engine, const TransferBlockStepCtx &ctx) {
  ...
  std::printf(
      ...
      "throughput: %.3lf GB/s\n",
      ctx.loop + 1U, ctx.cfg->loops, block_size, trans_num, static_cast<long>(time_cost), throughput);
  return 0;
}
...
bool SharedRemoteConnectTransferAndCleanup(Hixl *hixl, size_t idx, void *slice_base, const std::string &remote,
  ...
  if (RunTransfer(*hixl, slice_base, remote.c_str(), remote_addr, cfg) != 0) {
    MarkFirstFail(first_fail, fail_mu);
...
void SharedRemoteWorker(size_t idx, Hixl *hixl, const BenchmarkConfig &cfg, void *slice_base, std::atomic<int> *first_fail,
  ...
  if (!GetRemoteAddr(&tcp_client, remote, cfg.tcp_port, &remote_addr)) {
    MarkFirstFail(first_fail, fail_mu);
    return;
  }
  if (remote_addr != 0U) {
...
bool LaneWorkerRemoteTransferPhase(LaneState *p, const BenchmarkConfig &cfg, const std::string &remote,
                                   std::atomic<int> *first_fail, std::mutex *fail_mu) {
  uint64_t remote_addr = 0;
  if (!GetRemoteAddr(&p->tcp_client, remote, cfg.tcp_port, &remote_addr)) {
    MarkFirstFail(first_fail, fail_mu);
    return false;
  }
  if (remote_addr != 0U) {
  ...
  if (RunTransfer(p->hixl, p->buffer, remote.c_str(), remote_addr, cfg) != 0) {
    MarkFirstFail(first_fail, fail_mu);
    return false;
  }
  return true;
...
void LaneWorkerEntry(size_t idx, LaneState *p, const BenchmarkConfig &cfg, std::atomic<int> *first_fail,
  ...
  if (!LaneWorkerRemoteTransferPhase(p, cfg, remote, first_fail, fail_mu)) {
    return;
  }
...
void ClientRunner::ReleaseLaneResources() {
  ...
  if (lane_hixl_initialized_) {
    for (const auto &re : cfg_.expanded_remote_engines) {
      const auto ret = lane_hixl_.Disconnect(AscendString(re.c_str()));
      if (ret != SUCCESS) {
...
int ClientRunner::RunOnePair(const std::string &remote, void *src_slice, size_t register_len) {
  ...
  uint64_t remote_addr = 0;
  if (!GetRemoteAddr(&lane_tcp_, remote, cfg_.tcp_port, &remote_addr)) {
    return -1;
  }
