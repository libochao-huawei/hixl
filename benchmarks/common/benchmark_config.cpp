...
void BenchmarkConfigParser::PrintUsage(FILE *out) {
  fprintf(out,
          ...
          "  --remote_engine|-e   client: one or more host:port (comma-separated); TCP uses each host + --tcp_port\n"
...
bool BenchmarkConfigParser::Validate(BenchmarkConfig *cfg) {
  ...
  if (cfg->role == BenchmarkRole::kServer && n_max > 1U) {
...
void BenchmarkConfigParser::LogExpandedEndpoints(FILE *out, const BenchmarkConfig &cfg) {
  ...
  for (size_t i = 0; i < n; ++i) {
    fprintf(out, "[INFO]   [%zu] device_id=%d local_engine=%s remote_engine=%s\n", i,
