...
**在测什么**：单块大小为 DeepSeek-R1 KV 形状，**61×128K + 61×16K = 8784KB/块**。总块数 16/32/48/64 时，块在多卡间均分；**Put（D2RH）仅在 rank 0**；**Get（RH2D）每卡都执行**。计时为 **1 次 warmup + 10 次 timed**，对外打印的 Get 时间为 **10 次 timed 的算术平均**（每轮计时覆盖「整轮多源 RH2D」）。

![fabric_mem_benchmark.jpg](fabric_mem_benchmark.jpg)
