...
- [ ] **`HixlSyncTransferCtx` Kernel 与 Device 侧映射表管理**：新增 `HixlSyncTransferCtx` Kernel（复用现有 JSON 配置，新增独立函数），在 Device 侧维护全局映射表 `{thread -> {user_stream_id, notify_id, err_flag_dev_va}}`，通过 `HixlSyncTransferCtxParam` 入参接收操作列表，每条 `HixlSyncTransferCtxOp` 自带 `sync_type`（ADD/DELETE）。**调用时机**：Initialize 时批量 ADD、Finalize 时批量 DELETE、Abort 时先 DELETE 等待完成再销毁再 ADD。首次注册时自动向 Hcomm 注册异常回调，映射表清空时自动注销。
...
3. 填充 HixlSyncTransferCtxParam{entries_addr=(uint64_t)dev_entries, count}
...
sequenceDiagram
    ...
    Kernel->>Kernel: 遍历 ops，批量写入 g_transfer_ctx_table
    alt 首次添加（表从空到非空）
    ...
    alt 映射表为空
        Kernel->>Hcomm: UnregisterErrorCallback()
    end
    Kernel-->>Host: kernel 完成
