...
FabricMem是`HixlEngine`内部的传输模式，不再由`AdxlInnerEngine`、`ChannelMsgHandler`或`Channel`承载。`EngineFactory`在发现`OPTION_ENABLE_USE_FABRIC_MEM=1`时创建`HixlEngine`，后续注册、建链、传输、统计和资源清理由`src/hixl/fabric_mem`与`src/hixl/engine/hixl_engine.cc`负责。
...
classDiagram
    ...
    EngineFactory --> HixlEngine : EnableUseFabricMem=1时创建
    HixlEngine --> FabricMemTransferService : 持有
    HixlEngine --> FabricMemControlServer : 持有
    HixlEngine --> FabricMemRemoteMemory : 每个remote持有
    HixlEngine --> FabricMemStatistic : 持有
...
sequenceDiagram
   ...
   EngineFactory-->>User: HixlEngine
   User->>HixlEngine: Initialize(options)
   HixlEngine->>HixlEngine: Parse FabricMemConfig
   HixlEngine->>VirtualMemoryManager: Initialize()
   VirtualMemoryManager->>AscendRuntime: aclrtReserveMemAddress预留虚拟内存
   HixlEngine->>FabricMemService: Initialize(max_streams, task_streams, statistic)
   HixlEngine->>FabricMemControl: Start(local_engine, share_handle_provider)

   User->>HixlEngine: RegisterMem(mem_desc, type, handle)
   HixlEngine->>FabricMemService: RegisterMem(mem_desc, type, handle)
   ...
   HixlEngine->>FabricMemControl: Fetch(remote_engine)
   FabricMemControl-->>HixlEngine: remote_share_handles
   HixlEngine->>RemoteMemory: Import(remote_share_handles, device_id)
   ...
   HixlEngine->>HixlEngine: Build FabricMemTransferContext(remote_engine)
   HixlEngine->>FabricMemService: Transfer(context, WRITE, op_descs, timeout)
...
1. **初始化阶段**：
   ...
   - `EngineFactory`根据该选项创建`HixlEngine`，不进入`AdxlInnerEngine`。
   - `HixlEngine`解析`FabricMemConfig`，初始化`VirtualMemoryManager`、`FabricMemTransferService`、`FabricMemControlServer`和`FabricMemStatistic`。
...
3. **连接建立阶段**：
   ...
   - `HixlEngine`通过`FabricMemRemoteMemory`导入远程内存的共享句柄。
   - 使用`aclrtMemImportFromShareableHandleV2`导入共享句柄，映射到虚拟地址空间。
   - 建立远端用户地址到本地映射地址的映射关系，并登记到`fabric_mem_remote_mems_`。

4. **数据传输阶段**：
   - `HixlEngine`根据remote engine取得`FabricMemRemoteMemory`中的地址映射。
...
**检查点列表**：
1. **Engine路由检查**：`OPTION_ENABLE_USE_FABRIC_MEM=1`时必须由`EngineFactory`创建`HixlEngine`。
