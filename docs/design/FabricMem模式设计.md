...
sequenceDiagram
   ...
   FabricMemService->>AscendRuntime: aclrtMemImportFromShareableHandleV2()
   FabricMemService->>VirtualMemoryManager: ReserveMemory()
   FabricMemService->>AscendRuntime: aclrtMapMem()
   FabricMemService->>FabricMemService: 建立用户虚拟地址到本地虚拟地址映射
