...
1. Host 侧通过 `HixlSyncTransferCtx` Kernel 在 Device 侧注册异常回调并维护全局映射表 `{thread -> {user_stream_id, notify_id, err_flag_dev_va}}`。
...
- **Hcomm 异常回调 API 待确认**：Device 侧需调用 Hcomm 注册异常回调，需确认 Hcomm Device 侧是否已提供该能力（如 `HcommRegisterErrorCallback`），以及回调函数的参数格式（预期至少包含 `thread_handle`、`exe_stream_id`、`task_id`、`error_code`）。
- **`HixlSyncTransferCtx` Kernel 的函数签名待确认**：复用现有 kernel JSON 配置新增的函数，其入参结构需与 Device 侧 kernel 开发者对齐。
- **err_flag 错误码值域待确认**：当前设计为 `uint8_t`，0 表示正常，非 0 表示错误码。需确认 Hcomm 回调提供的错误码是否能映射到 `uint8_t` 范围内，或需要扩展为更大类型。
...
- [ ] **`HixlSyncTransferCtx` Kernel 与 Device 侧映射表管理**：新增 `HixlSyncTransferCtx` Kernel（复用现有 JSON 配置，新增独立函数），在 Device 侧维护全局映射表 `{thread -> {user_stream_id, notify_id, err_flag_dev_va}}`，通过 `HixlSyncTransferCtxParam` 入参接收操作列表，每条 `HixlSyncTransferCtxOp` 自带 `sync_type`（ADD/DELETE），支持一次 launch 混合增删；首次注册时自动向 Hcomm 注册异常回调，映射表清空时自动注销。
...
本需求在现有 CS 传输路径上新增一条异常上报旁路，不改变正常数据面的传输流程和 Kernel 入参。异常回调注册、全局映射表维护、异常处理逻辑**全部在 Device 侧完成**，Host 侧仅负责 err_flag 内存管理和通过 kernel 入参下发 batch add/delete 指令。
...
struct HixlSyncTransferCtxParam {
  ...
  uint64_t entries_addr;      // HixlSyncTransferCtxOp 数组的 Device 地址
...
// runtime 自动将 HixlSyncTransferCtxParam 从 Host 拷贝到 Device
extern "C" uint32_t HixlSyncTransferCtx(HixlSyncTransferCtxParam *param);
```
...
一次 launch 可混合 ADD 和 DELETE 操作（如 Abort + re-init 场景：`[DELETE old_thread, ADD new_thread]`）。

#### Host 侧 launch 流程（与 LaunchDeviceKernel 一致）

```
1. aclrtMalloc(&dev_entries, count * sizeof(HixlSyncTransferCtxOp))
2. aclrtMemcpy(dev_entries, ..., host_entries, ..., ACL_MEMCPY_HOST_TO_DEVICE)
3. 填充 HixlSyncTransferCtxParam{count, entries_addr=(uint64_t)dev_entries}
4. aclrtKernelArgsInit(func, &argsHandle)
5. aclrtKernelArgsAppend(argsHandle, &param, sizeof(HixlSyncTransferCtxParam), &paraHandle)
...
Device 侧全局状态（跨 kernel launch 持久化）：
  - g_transfer_ctx_table: 映射表 {thread_handle -> {user_stream_id, notify_id, err_flag_dev_va}}
  - g_callback_registered: 异常回调是否已注册

HixlSyncTransferCtx 流程：
  遍历 entries 数组，逐条处理：
    if (op.sync_type == SYNC_TYPE_ADD):
      写入 g_transfer_ctx_table
      if (!g_callback_registered):
        调用 HcommRegisterErrorCallback(error_handler, context)
        g_callback_registered = true
    elif (op.sync_type == SYNC_TYPE_DELETE):
      从 g_transfer_ctx_table 移除
      if (g_transfer_ctx_table.empty() && g_callback_registered):
        调用 HcommUnregisterErrorCallback()
        g_callback_registered = false

error_handler 回调流程（Hcomm 触发）：
  1. 从 CQE 解析出 thread_handle, error_code
  2. 打印错误日志（必须包含 thread 信息）
  3. 查找 g_transfer_ctx_table[thread_handle]
  4. 写入 err_flag: *(volatile uint8_t*)entry.err_flag_dev_va = error_code
  5. 构造 ts_aicpu_sqe_t 并调用 halEschedSubmitEvent 通知 TSFW
```

#### halEschedSubmitEvent 调用方式（参考 Hcomm `SendTaskExceptionByMBox`）

```cpp
// Device 侧 error_handler 内部
void ErrorHandler(ThreadHandle thread, uint32_t error_code,
                  uint32_t exe_stream_id, uint32_t task_id) {
  auto *entry = g_transfer_ctx_table.Find(thread);
  if (entry == nullptr) { return; }

  // 1. 写入 err_flag
  *(volatile uint8_t *)(entry->err_flag_dev_va) = (uint8_t)error_code;

  // 2. 获取 pid 和 vf_id（直接调用 drv 接口）
  uint32_t hostpid = 0, vf_id = 0;
  drvQueryProcessHostPid(getpid(), nullptr, &vf_id, &hostpid, nullptr);

  // 3. 构造 ts_aicpu_sqe_t（参考 aicpu_hccl_sqcq.cc:407）
  ts_aicpu_sqe_t aicpuSqe = {};
  aicpuSqe.pid = hostpid;
  aicpuSqe.cmd_type = AICPU_RECORD;
  aicpuSqe.vf_id = vf_id;
  aicpuSqe.tid = 0;
  aicpuSqe.ts_id = 0;  // DC 场景固定为 0
  aicpuSqe.u.aicpu_record.record_type = AICPU_MSG_NOTIFY_RECORD;
  aicpuSqe.u.aicpu_record.record_id = entry->notify_id;
  aicpuSqe.u.aicpu_record.fault_stream_id = static_cast<uint16_t>(entry->user_stream_id);
  aicpuSqe.u.aicpu_record.ret_code = SwitchSdmaCqeErrCodeToTsErrCode(error_code);

  // 4. 构造 event_summary 并提交
  ...
  event.subevent_id = 0;
  event.msg_len = sizeof(ts_aicpu_sqe_t);
  event.msg = reinterpret_cast<char *>(&aicpuSqe);
  halEschedSubmitEvent(localDeviceId, &event);
}
```

**依赖头文件：**
- `driver/ascend_hal_base.h`：`halEschedSubmitEvent`（弱符号，`ASCEND_HAL_WEAK`）、`drvQueryProcessHostPid`
- `driver/ascend_hal_external.h`：`struct event_summary`
- `driver/ascend_hal_define.h`：`EVENT_ID`、`SCHEDULE_DST_ENGINE`、`SCHEDULE_POLICY`
- TSFW 头文件（`task_struct.h`）：`ts_aicpu_sqe_t`、`AICPU_RECORD`、`AICPU_MSG_NOTIFY_RECORD`
...
classDiagram
    ...
    class HixlSyncTransferCtxParam {
        +uint32_t count
        +uint64_t entries_addr
    }
    class HixlSyncTransferCtxOp {
        +uint32_t sync_type
    ...
    class SyncTransferCtx_DeviceSide {
        +g_transfer_ctx_table
        +g_callback_registered
        +HixlSyncTransferCtx()
        +error_handler()
    }
    ...
    HixlCSClient --> HixlSyncTransferCtxParam : fills & launches
    HixlSyncTransferCtxParam --> HixlSyncTransferCtxOp : entries_addr points to
    HixlCSClient ..> SyncTransferCtx_DeviceSide : launch with param*
    SyncTransferCtx_DeviceSide ..> Hcomm : register/unregister callback
...
### 功能点 2：`HixlSyncTransferCtx` Kernel 与 Device 侧映射表管理
...
sequenceDiagram
    ...
    participant Kernel as HixlSyncTransferCtx (Device)
    participant Hcomm as Hcomm (Device)

    Note over Host: Slot Acquire 后，ADD
    Host->>DevMem: aclrtMalloc + aclrtMemcpy(ops -> dev)
    Host->>Host: 填充 Op{sync_type=ADD, thread, ...} + Param{count, entries_addr}
    Host->>Kernel: aclrtLaunchKernelWithConfig(HixlSyncTransferCtx, &param)
    Note over Kernel: runtime 自动拷贝 param 到 Device
    Kernel->>Kernel: 遍历 ops，逐条处理 sync_type
    Kernel->>Kernel: ADD: 写入 g_transfer_ctx_table
    alt 首次添加（表从空到非空）
        Kernel->>Hcomm: RegisterErrorCallback(error_handler)
    end
    Kernel-->>Host: kernel 完成
    Host->>DevMem: aclrtFree(dev_entries)

    Note over Host: Slot Abort + re-init，混合 DELETE + ADD
    Host->>DevMem: aclrtMalloc + aclrtMemcpy(ops -> dev)
    Host->>Host: 填充 [Op{DELETE, old_thread}, Op{ADD, new_thread}]
    Host->>Kernel: aclrtLaunchKernelWithConfig(HixlSyncTransferCtx, &param)
...
**load_kernel 扩展：**
- JSON 配置中新增函数名 `"HixlSyncTransferCtx"`。
- `LoadDeviceKernelAndGetHandles` 中通过 `aclrtBinaryGetFunction` 一并加载。

**HixlCSClient 新增方法：**

```cpp
Status LaunchSyncTransferCtxKernel(const std::vector<HixlSyncTransferCtxOp> &ops);
```
...
3. 填充 `HixlSyncTransferCtxParam{count, entries_addr=(uint64_t)dev_entries}`。
4. `aclrtKernelArgsInit` + `aclrtKernelArgsAppend(&param, sizeof(HixlSyncTransferCtxParam))` + `aclrtKernelArgsFinalize`。
...
**调用时机：**
- **单条 ADD**：`AcquireSharedSlot` 成功后，构造 `{sync_type=SYNC_TYPE_ADD, thread, stream_id, notify_id, err_flag_dev_va}` 下发。
- **单条 DELETE**：`ReleaseSharedSlotRef` 或 `DestroySlotLocked` 时，构造 `{sync_type=SYNC_TYPE_DELETE, thread, ...}` 下发。
- **混合 DELETE + ADD**：`AbortSlotByIndexLocked` 后 re-init 时，一次 launch 包含 `[DELETE old_thread, ADD new_thread]`，避免两次 kernel launch。
...
sequenceDiagram
    participant Hcomm as Hcomm (Device)
    participant Handler as error_handler (Device)
    participant Flag as err_flag (Dev/Host共享)
    participant TSFW as TSFW

    Hcomm->>Hcomm: 轮询 RTSQ，捕获错误 CQE
    Hcomm->>Hcomm: 解析 -> thread_handle, error_code
    Hcomm->>Handler: error_handler(thread, error_code, stream_id, task_id)
    Handler->>Handler: 打印错误日志（含 thread）
    Handler->>Handler: 查 g_transfer_ctx_table[thread]
    Handler->>Flag: *(volatile uint8_t*)err_flag_dev_va = error_code
    Handler->>Handler: drvQueryProcessHostPid -> pid, vf_id
    Handler->>Handler: 构造 ts_aicpu_sqe_t
    Handler->>TSFW: halEschedSubmitEvent(devId, &event)
...
- 新增：`docs/design/HIXL异常上报SRS.md`（本文档）
- 新增：`src/ops/hixl_kernel/hixl_sync_transfer_ctx.h/.cc`（Device 侧 Kernel 实现）
- 更新：`docs/cpp/HIXL_CS接口.md`（CheckStatus FAILED 语义扩展）
- 更新：`docs/cpp/HIXL错误码.md`（err_flag 错误码映射表）
- 更新：kernel JSON 配置文件（新增 `HixlSyncTransferCtx` 函数）
...
| Kernel 混合 DELETE+ADD launch | LaunchSyncTransferCtxKernel([DELETE, ADD]) | 一次 launch 包含两条 op，kernel launch 成功 |
...
| slot Abort 后恢复 | Abort + re-init 混合操作 | 一次 launch 包含 [DELETE old, ADD new]，err_flag 资源正确重建，映射表一致 |
| 多 slot 并发异常 | 多 slot 同时触发异常 | 各 slot err_flag 独立正确，Device 侧映射表无冲突 |
| 映射表增删一致性 | 多次 ADD/DELETE 循环 | Device 侧映射表与 Host 侧 slot 状态一致 |
