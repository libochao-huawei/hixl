...
- `channel`：统计聚合 key，客户端通常为 `client:<server_ip>:<server_port>`，服务端通常为 `server:fd:<fd>` 或监听维度 key
- `fd`：控制面 socket
...
- `transfer_submit_device` 高
  优先看 `TransferPool` 资源获取、device buffer 准备和 kernel 启动。
- `transfer_sync_device` 高但 `check_status_device` 不高
  多数是 stream 同步阶段耗时高。
...
- Device 路径通过 `TransferPool + kernel + notify + host_flag` 完成判定，重点看 `transfer_submit_device`、`transfer_sync_device` 和 `check_status_device`
