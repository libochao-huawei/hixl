...
HIXL 当前已支持通过 `LocalCommRes` 1.3 格式描述本地通信资源，并支持通过 `GlobalResourceConfig` 中的 `comm_resource_config.protocol_desc` 自动生成 UBoE endpoint。UBoE 与 UBG 都属于 ScaleOut 口通信资源，但资源标识和获取方式不同：

- UBoE：判断 ScaleOut 口配置为 UBoE 后，使用 `hccn_tool` 获取 Bond 聚合设备 IP，endpoint 的 `comm_id` 填 IP。
- UBG：判断 ScaleOut 口配置为 UBG 后，通过 DCMI/URMA 设备接口获取 EID，endpoint 的 `comm_id` 填按 UBG 建链语义选择出的目标 EID。
...
- UBoE 默认资源获取使用 Bond IP；UBG 默认资源获取使用 DCMI/URMA 查询到的 EID。
...
HIXL 需新增通信资源配置，用于指导本地 UBG 通信资源自动获取：
...
当配置启用 UBG 且未显式提供非空 `endpoint_list` 时，HIXL 自动生成本地通信资源：
...
- `protocol` 固定为 `"ubg"`。
- `comm_id` 填 DCMI/URMA 查询后按 UBG 场景选择出的目标 EID。
...
该能力应复用当前 UBoE 设计，并将 UBoE 专用判断泛化为 ScaleOut Device 协议判断。
...
- ScaleOut 口协议类型判断：用于确认当前设备 ScaleOut 口配置为 UBG。
- EID 获取：通过 DCMI/URMA 设备接口获取当前 NPU 对应的 EID 列表。现有代码已具备 EID 字符串转换、byte6 解析、Mesh/CLOS PG 筛选能力，但该筛选策略当前服务于 UB-CTP/CLOS local comm res 生成，UBG 只能复用其基础查询与解析能力，最终 EID 选择规则需按 UBG 建链语义确认。
...
- 通过 DSMI 查询 `InterconType` 字段，新增或识别 UBG 取值。
- 查询结果有效代表物理链路可用。
- 当 `InterconType=UBG` 时进入 DCMI/URMA EID 获取流程，并按 UBG 确认后的选择规则将目标 EID 写入 UBG endpoint 的 `comm_id`。
- 当 `InterconType=UBOE` 时保持现有 UBoE 路径，查询 Bond IP。
...
注意：上述 `port_to_eid` 和 `clos_pg_eids` 当前由 `local_comm_res_generator_v1.cc` 用于生成 `protocol=ub-ctp` 的 endpoint，不等同于 UBG endpoint 的最终选择策略。UBG 开发时可复用 `DcmiProxy`、`GetEidList`、EID 字符串转换和 `ParseEidByte6` 等基础能力；`CollectMeshPorts`、`CollectClosPgEids` 只能作为候选策略参考。当前仍需确认 UBG endpoint 的 `comm_id` 最终应选择 Mesh EID、CLOS PG EID、DCMI 返回的其他 UBG 专用 EID，还是按场景生成多个 endpoint。
...
flowchart TD
  ...
  DcmiQuery --> FilterEid["按 UBG 语义选择目标 EID"]
...
- DCMI/URMA 能获取非空 EID 列表，并能按 UBG 场景筛选出目标 EID。
...
flowchart TD
  ...
  AutoGenerate --> CheckUbg["protocol_desc 包含 ubg:device"]
  CheckUbg -->|否| EndNoUbg["结束 UBG 生成"]
  CheckUbg -->|是| QueryType["DSMI 查询 InterconType"]
  QueryType --> IsUbg{"InterconType 是 UBG"}
  IsUbg -->|否| FailOrSkip["按配置语义失败或跳过"]
  IsUbg -->|是| QueryEid["DCMI/URMA 查询 EID 列表"]
  QueryEid --> FilterEid["按 die/port/PG 属性筛选 EID"]
  FilterEid --> ValidateEid["校验 EID 非空且格式合法"]
  ValidateEid --> BuildEndpoint["生成 protocol=ubg endpoint"]
  BuildEndpoint --> FillDeviceInfo["填充 device_info"]
  FillDeviceInfo --> Finish["返回 endpoint_list"]
```
...
| `LoadUrmaDevicesFromDcmi` / `GetUbgEid` | 复用现有 DCMI/URMA EID 查询、字符串转换和 byte6 解析能力，按 UBG 语义选择目标 EID。 |
...
| UBG  | UBG            | DCMI/URMA 查询并筛选 EID                            | EID           |
...
其语义为：该协议下 Host 内存需要先映射为 Device VA 后再注册给 Hcomm。
...
flowchart TD
  ...
  Validate --> NeedConvert{"Device endpoint 且协议为 UBG/UBoE"}
...
建议将 `ConvertUboeDescs` 泛化命名为 `ConvertScaleOutDescs`，避免 UBG 复用时语义不清。
...
- 解析 `ubg:device`。
- 自动生成 UBG endpoint。
- DSMI 查询 `InterconType`，DCMI/URMA 查询和筛选 EID。
...
| EID 为空、格式非法或筛选不到目标 Mesh/CLOS PG EID | 返回 `PARAM_INVALID` 或 `FAILED`，日志输出 EID 原始数量、筛选条件和 NPU id。 |
...
- InterconType 返回非 UBG、DCMI/URMA 查询失败、EID 为空或筛选失败时，返回可诊断错误。
...
| Mesh EID 与 CLOS PG EID 选择规则未最终确认      | 可能生成错误的 UBG endpoint             | 现有 Mesh/CLOS PG 筛选服务于 UB-CTP/CLOS local comm res，不能默认等同于 UBG 选择规则；将 UBG EID 选择策略封装为独立函数，待产品/驱动确认后固化。 |
...
- ScaleOut `InterconType=UBG` 的查询接口、命令字和返回结构需由驱动接口确认。
- UBG endpoint `comm_id` 最终应选择 Mesh EID、CLOS PG EID、DCMI 返回的其他 UBG 专用 EID，还是按场景生成多个 endpoint，需结合 UBG 建链语义确认；现有 `CollectMeshPorts` / `CollectClosPgEids` 不能直接作为最终结论。
...
2. 接入 InterconType 与 DCMI/URMA wrapper：完成 ScaleOut 类型判断、EID 查询/筛选封装，并以 mock 覆盖 UT。
