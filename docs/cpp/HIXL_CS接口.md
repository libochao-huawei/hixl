...
`global_resource_config`当前支持的配置项如下。
...
| config | 输入 | Server配置。可传入NULL；如需配置通信资源参数，填写 `HixlServerConfig.global_resource_config`。 |
...
- 构造 `HixlServerDesc` 与 `HixlServerConfig`，如需指定通信资源监听端口，在 `global_resource_config` 中配置 `comm_resource_config.listen_port`。
