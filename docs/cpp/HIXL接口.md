...
| 参数名称 | 输入/输出 | 取值说明 |
| --- | --- | --- |
| status_map | 输出 | 所有传输状态，枚举值如下。<br><br>-  WAITING<br>-  COMPLETED<br>-  TIMEOUT（暂不支持）<br>-  FAILED |
...
```
  ...
  Status query_status = GetTransferStatus(status_map);
  //对传输状态进行检查，判断传输是否完成
  ...
```
...
- SUCCESS：成功
- UNSUPPORTED: AdxlEngine不支持通过该接口查询
- 其他：失败
