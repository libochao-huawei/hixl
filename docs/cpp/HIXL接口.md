...
```
  ...
  GetTransferStatusOptions options = { .max_num = 4, .skip_waiting = true };
  Status query_status = client_engine.GetTransferStatus(out_results, options);
  //对传输状态进行检查，判断传输是否完成
  ...
```
