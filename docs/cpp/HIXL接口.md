...
```
{
    ...
    "comm_resource_config.max_active_channels": "128" //可选，取值范围：[1, 4096]之间的整数，用于配置单设备最大活跃传输链路上限。不配置时默认128
}
```
...
| comm_resource_config.max_active_channels | 数字 | 可选 | 配置单设备最大活跃传输链路上限 | 当前仅支持[1-4096]，当未配置的时候，默认为128。|
