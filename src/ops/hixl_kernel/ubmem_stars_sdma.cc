...
// Must match UbMemRtsqBatch::entries capacity in ubmem_stars_sdma.h.
...
drvError_t __attribute__((weak)) halResourceIdRestore(UbMemDrvResIdKey *info);
}

bool QueryRtsqValues(uint32_t device_id, uint32_t sq_id, drvSqCqPropType_t property, uint32_t &value_low,
                     uint32_t &value_high) {
  return ubmem_rtsq::QueryRtsqValues(device_id, sq_id, property, value_low, value_high, halSqCqQuery);
}

bool QueryRtsqValue(uint32_t device_id, uint32_t sq_id, drvSqCqPropType_t property, uint32_t &value) {
  return ubmem_rtsq::QueryRtsqValue(device_id, sq_id, property, value, halSqCqQuery);
}
}  // namespace

uint32_t UbMemStarsSdma::Submit(const UbMemAicpuKernelParam &param, const UbMemAicpuTransferDesc *descs) {
  if (!ValidateSubmitArgs(param, descs)) {
    return kFailed;
  }
  UbMemRtsqState state;
  if (!InitializeRtsq(param, state)) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] initialize RTSQ failed. host_dev=%u rtsq=%u stream=%u task=%u logic_cq=%u",
...
bool UbMemStarsSdma::ValidateSubmitArgs(const UbMemAicpuKernelParam &param, const UbMemAicpuTransferDesc *descs) {
  if (descs == nullptr || param.desc_count == 0U || param.emit_notify_record > 1U ||
      (param.emit_notify_record != 0U && param.notify_id >= kUbMemA3NotifyIdLimit) ||
      !AreValidDescriptors(descs, param.desc_count)) {
    HIXL_LOGE(PARAM_INVALID, "[UbMem][AICPU] invalid submit args. descs=%p desc_count=%u emit_notify=%u notify_id=%u",
              descs, param.desc_count, param.emit_notify_record, param.notify_id);
    return false;
  }
  return true;
}

bool UbMemStarsSdma::BuildSubmitDeadline(uint64_t timeout_ms, uint64_t &deadline) {
  ...
  if (now_ns == 0U) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] cannot start RTSQ submit deadline. timeout_ms=%llu",
...
bool UbMemStarsSdma::AppendAndPublishSdmaTasks(const UbMemAicpuTransferDesc *descs, uint32_t desc_count,
                                               UbMemRtsqState &state, UbMemRtsqBatch &batch, uint64_t deadline) {
  for (uint32_t desc_idx = 0U; desc_idx < desc_count; ++desc_idx) {
    if (!AppendDescriptorTasks(descs[desc_idx], state, batch, deadline)) {
      HIXL_LOGE(FAILED, "[UbMem][AICPU] append SDMA tasks failed at desc=%u/%u", desc_idx, desc_count);
      return false;
    }
  }
  ...
  if (!PublishRtsqBatch(state, batch, deadline)) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] publish SDMA RTSQ batch failed. sq=%u stream=%u", state.sq_id, state.stream_id);
    return false;
  }
  return true;
}

bool UbMemStarsSdma::EmitNotifyRecord(uint32_t notify_id, UbMemRtsqState &state, UbMemRtsqBatch &batch,
                                      uint64_t deadline) {
  const bool cq_ok = PollLogicCqUntilEmpty(state, deadline);
  if (!cq_ok) {
    HIXL_LOGE(FAILED,
              "[UbMem][AICPU] logic CQ poll failed before NotifyRecord; still emitting notify. "
  ...
  if (!AppendNotifyTask(notify_id, state, batch, deadline)) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] append NotifyRecord failed. notify_id=%u", notify_id);
    return false;
  }
  if (!PublishRtsqBatch(state, batch, deadline)) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] publish NotifyRecord failed. sq=%u stream=%u notify_id=%u", state.sq_id,
...
bool UbMemStarsSdma::IsValidDescriptor(const UbMemAicpuTransferDesc &desc) {
...
bool UbMemStarsSdma::AreValidDescriptors(const UbMemAicpuTransferDesc *descs, uint32_t desc_count) {
  for (uint32_t desc_idx = 0U; desc_idx < desc_count; ++desc_idx) {
    if (!IsValidDescriptor(descs[desc_idx])) {
      HIXL_LOGE(PARAM_INVALID, "[UbMem][AICPU] invalid descriptor. idx=%u/%u src=0x%llx dst=0x%llx length=%llu",
...
bool UbMemStarsSdma::RestoreRtsqStream(const UbMemRtsqState &state) {
  if (halResourceIdRestore == nullptr) {
    return true;
  }
  UbMemDrvResIdKey resource{};
  ...
  if (ret != DRV_ERROR_NONE) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] halResourceIdRestore failed. ret=%d device=%u stream=%u",
...
bool UbMemStarsSdma::ResolveLocalDeviceId(uint32_t host_device_id, uint32_t &local_device_id) {
  if (drvGetLocalDevIDByHostDevID == nullptr) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] drvGetLocalDevIDByHostDevID unavailable. host_dev=%u", host_device_id);
    return false;
  }
  const drvError_t ret = drvGetLocalDevIDByHostDevID(host_device_id, &local_device_id);
  if (ret != DRV_ERROR_NONE) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] drvGetLocalDevIDByHostDevID failed. ret=%d host_dev=%u",
...
uint64_t UbMemStarsSdma::MonotonicNs() {
  struct timespec ts {};
  if (clock_gettime(CLOCK_MONOTONIC, &ts) != 0) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] clock_gettime(CLOCK_MONOTONIC) failed. errno=%d", errno);
    return 0U;
  }
  return static_cast<uint64_t>(ts.tv_sec) * kNsPerSecond + static_cast<uint64_t>(ts.tv_nsec);
}

bool UbMemStarsSdma::DeadlineExceeded(uint64_t deadline_ns) {
  return MonotonicNs() >= deadline_ns;
}

bool UbMemStarsSdma::IsExceptionLogicCqe(const UbMemLogicCqeView &report) {
  return (report.error_type & kStarsExistErrorMask) != 0U;
}

void UbMemStarsSdma::LogLogicCqe(const UbMemRtsqState &state, const UbMemLogicCqeView &report, uint32_t idx,
                                 uint32_t report_count, uint32_t total_reports, bool is_exception) {
  const char *tag = is_exception ? "abnormal" : "drained";
  if (is_exception) {
    HIXL_LOGE(FAILED,
              "[UbMem][CQ] %s logic CQE. device=%u stream=%u sq=%u logic_cq=%u idx=%u/%u "
  ...
  HIXL_LOGD(
      "[UbMem][CQ] %s logic CQE. device=%u stream=%u sq=%u logic_cq=%u idx=%u/%u total=%u "
...
bool UbMemStarsSdma::InspectLogicCqReports(const UbMemRtsqState &state, const uint8_t *reports, uint32_t report_count,
                                           uint32_t total_reports) {
  for (uint32_t idx = 0U; idx < report_count; ++idx) {
    const auto &report =
        *reinterpret_cast<const UbMemLogicCqeView *>(reports + static_cast<size_t>(idx) * kLogicCqeBytes);
...
UbMemLogicCqRecvResult UbMemStarsSdma::RecvLogicCqBatch(const UbMemRtsqState &state, uint8_t *reports,
  ...
  if (ret == DRV_ERROR_WAIT_TIMEOUT || (ret == DRV_ERROR_NONE && recv_info.report_cqe_num == 0U)) {
    report_count = 0U;
    return UbMemLogicCqRecvResult::kEmpty;
  }
  if (ret != DRV_ERROR_NONE) {
    HIXL_LOGE(FAILED, "[UbMem][CQ] halCqReportRecv failed, ret=%d device=%u stream=%u sq=%u logic_cq=%u",
              static_cast<int32_t>(ret), state.device_id, state.stream_id, state.sq_id, state.logic_cq_id);
    return UbMemLogicCqRecvResult::kError;
  }
  if (recv_info.report_cqe_num > kLogicCqReportCount) {
    HIXL_LOGE(FAILED,
              "[UbMem][CQ] halCqReportRecv returned more CQEs than buffer capacity. device=%u stream=%u "
  ...
  return UbMemLogicCqRecvResult::kReports;
}

bool UbMemStarsSdma::PollLogicCqUntilEmpty(const UbMemRtsqState &state, uint64_t deadline) {
  if (halCqReportRecv == nullptr) {
    HIXL_LOGW(
        "[UbMem][CQ] halCqReportRecv unavailable, skip logic CQ poll. "
  ...
  for (;;) {
    if (deadline != 0U && DeadlineExceeded(deadline)) {
      HIXL_LOGE(FAILED,
                "[UbMem][CQ] logic CQ poll hit submit deadline before empty. device=%u stream=%u sq=%u "
                "logic_cq=%u total=%u",
                state.device_id, state.stream_id, state.sq_id, state.logic_cq_id, total_reports);
      return false;
    }
    uint32_t report_count = 0U;
    const UbMemLogicCqRecvResult recv_result = RecvLogicCqBatch(state, reports, report_count);
    if (recv_result == UbMemLogicCqRecvResult::kEmpty) {
      return true;
    }
    if (recv_result == UbMemLogicCqRecvResult::kError) {
      return false;
    }
...
bool UbMemStarsSdma::LoadRtsqQueueState(UbMemRtsqState &state) {
  ...
  if (!QueryRtsqValues(state.device_id, state.sq_id, DRV_SQCQ_PROP_SQ_BASE, base_low, base_high)) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] query SQ_BASE failed. device=%u sq=%u", state.device_id, state.sq_id);
    return false;
  }
  if (!QueryRtsqValue(state.device_id, state.sq_id, DRV_SQCQ_PROP_SQ_DEPTH, state.depth) ||
    ...
    HIXL_LOGE(FAILED, "[UbMem][AICPU] query SQ depth/head/tail failed. device=%u sq=%u", state.device_id, state.sq_id);
    return false;
  }
  ...
  if (state.base_addr == 0U || state.depth <= 1U || state.head >= state.depth || state.tail >= state.depth) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] invalid RTSQ state. device=%u sq=%u base=0x%llx depth=%u head=%u tail=%u",
  ...
  if (state.depth <= kMaxRtsqEntriesPerPublish) {
    HIXL_LOGE(FAILED,
              "[UbMem][AICPU] RTSQ depth too small for max publish batch. device=%u sq=%u depth=%u "
              "min_exclusive=%u",
              state.device_id, state.sq_id, state.depth, kMaxRtsqEntriesPerPublish);
    return false;
  }
  HIXL_LOGD("[UbMem][AICPU] RTSQ ready. device=%u sq=%u stream=%u logic_cq=%u sq_depth=%u head=%u tail=%u",
            state.device_id, state.sq_id, state.stream_id, state.logic_cq_id, state.depth, state.head, state.tail);
  return true;
}

bool UbMemStarsSdma::InitializeRtsq(const UbMemAicpuKernelParam &param, UbMemRtsqState &state) {
  ...
  if (param.rtsq_id > std::numeric_limits<uint16_t>::max()) {
    HIXL_LOGE(PARAM_INVALID, "[UbMem][AICPU] rtsq_id out of A3 CQE u16 ABI range. sq=%u", param.rtsq_id);
    return false;
  }
  if (param.rtsq_stream_id > std::numeric_limits<uint16_t>::max()) {
    HIXL_LOGE(PARAM_INVALID, "[UbMem][AICPU] rtsq_stream_id out of A3 SQE u16 ABI range. stream=%u",
              param.rtsq_stream_id);
    return false;
  }
  if (halSqCqQuery == nullptr || halSqCqConfig == nullptr) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] halSqCqQuery/halSqCqConfig unavailable. query_null=%d config_null=%d",
...
bool UbMemStarsSdma::HasRtsqCapacity(const UbMemRtsqState &state, uint32_t count) {
...
bool UbMemStarsSdma::EnsureRtsqCapacity(UbMemRtsqState &state, uint32_t count, uint64_t deadline) {
  if (count == 0U || count >= state.depth) {
    HIXL_LOGE(PARAM_INVALID,
              "[UbMem][AICPU] invalid RTSQ capacity request. device=%u sq=%u depth=%u need=%u head=%u tail=%u",
  ...
  if (!PollLogicCqUntilEmpty(state, deadline)) {
    HIXL_LOGE(FAILED,
              "[UbMem][AICPU] logic CQ poll failed while refreshing RTSQ capacity. device=%u sq=%u stream=%u "
  ...
  if (!QueryRtsqValue(state.device_id, state.sq_id, DRV_SQCQ_PROP_SQ_HEAD, state.head)) {
    HIXL_LOGE(FAILED,
              "[UbMem][AICPU] query SQ_HEAD failed while refreshing RTSQ capacity. device=%u sq=%u stream=%u "
              "depth=%u need=%u head=%u tail=%u",
              state.device_id, state.sq_id, state.stream_id, state.depth, count, state.head, state.tail);
    return false;
  }
  ...
  const char *const reason = state.depth < kUbMemMinRtsqDepth
                                 ? "this RTSQ is too shallow to ever hold the host submit budget"
                                 : "SDMA has stopped retiring published entries";
  HIXL_LOGE(FAILED,
            "[UbMem][AICPU] no RTSQ capacity after refreshing head: %s. The host queues up to %u entries "
            "between NotifyRecord waits, needing depth >= %u. device=%u sq=%u stream=%u depth=%u need=%u head=%u "
            "tail=%u used=%u",
            reason, kUbMemMaxInFlightRtsqTasks, kUbMemMinRtsqDepth, state.device_id, state.sq_id, state.stream_id,
            state.depth, count, state.head, state.tail, used);
  return false;
}

bool UbMemStarsSdma::BuildSdmaSqe(uint64_t source, uint64_t destination, uint64_t size, uint32_t task_id,
                                  const UbMemRtsqState &state, UbMemA3SdmaSqe &sqe) {
  if (memset_s(&sqe, sizeof(sqe), 0, sizeof(sqe)) != EOK) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] memset SDMA SQE failed, device_id=%u, sq=%u, stream=%u", state.device_id,
              state.sq_id, state.stream_id);
    return false;
  }
  sqe.header.type = kUbMemA3SdmaSqeType;
  ...
  sqe.kernel_credit = kUbMemA3SdmaKernelCredit;
  sqe.sssv = 1U;
  sqe.dssv = 1U;
  sqe.sns = 1U;
  sqe.dns = 1U;
  sqe.qos = kUbMemA3SdmaQosDefault;
  ...
  sqe.link_type = kUbMemA3SdmaLinkOnChip;
  return true;
}

bool UbMemStarsSdma::BuildNotifySqe(uint32_t notify_id, uint32_t task_id, const UbMemRtsqState &state,
                                    UbMemA3NotifySqe &sqe) {
  if (memset_s(&sqe, sizeof(sqe), 0, sizeof(sqe)) != EOK) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] memset NotifyRecord SQE failed, device_id=%u, sq=%u, stream=%u", state.device_id,
              state.sq_id, state.stream_id);
    return false;
  }
  sqe.header.type = kUbMemA3NotifyRecordSqeType;
  sqe.header.wr_cqe = 0U;
  sqe.header.rt_stream_id = static_cast<uint16_t>(state.stream_id);
  sqe.header.task_id = static_cast<uint16_t>(task_id);
  sqe.notify_id = notify_id;
  sqe.kernel_credit = kUbMemA3NotifyKernelCredit;
  return true;
}

bool UbMemStarsSdma::CopySqeBatchToRing(const UbMemRtsqState &state, const UbMemRtsqBatch &batch) {
  ...
  const size_t first_bytes = static_cast<size_t>(first_count) * kUbMemA3RtsqEntryBytes;
  const size_t first_dest_max = static_cast<size_t>(state.depth - state.tail) * kUbMemA3RtsqEntryBytes;
  if (memcpy_s(sq_base + state.tail * kUbMemA3RtsqEntryBytes, first_dest_max, batch.entries, first_bytes) != EOK) {
    HIXL_LOGE(FAILED,
              "[UbMem][AICPU] memcpy SQE ring (first segment) failed. device=%u sq=%u tail=%u first_count=%u "
              "bytes=%zu",
              state.device_id, state.sq_id, state.tail, first_count, first_bytes);
    return false;
  }
  if (first_count < batch.count) {
    const size_t wrap_bytes = static_cast<size_t>(batch.count - first_count) * kUbMemA3RtsqEntryBytes;
    const size_t wrap_dest_max = static_cast<size_t>(state.depth) * kUbMemA3RtsqEntryBytes;
    if (memcpy_s(sq_base, wrap_dest_max, batch.entries + first_count, wrap_bytes) != EOK) {
      HIXL_LOGE(FAILED, "[UbMem][AICPU] memcpy SQE ring (wrap segment) failed. device=%u sq=%u wrap_count=%u bytes=%zu",
...
bool UbMemStarsSdma::CommitRtsqTail(UbMemRtsqState &state, uint32_t new_tail, uint32_t batch_count) {
  ...
  if (ret != DRV_ERROR_NONE) {
    HIXL_LOGE(FAILED,
              "[UbMem][AICPU] halSqCqConfig SQ_TAIL failed. ret=%d device=%u sq=%u stream=%u old_tail=%u "
...
bool UbMemStarsSdma::PublishRtsqBatch(UbMemRtsqState &state, UbMemRtsqBatch &batch, uint64_t deadline) {
  if (batch.count == 0U) {
    return true;
  }
  if (!EnsureRtsqCapacity(state, batch.count, deadline)) {
    HIXL_LOGE(FAILED, "[UbMem][AICPU] publish has no RTSQ capacity. device=%u sq=%u stream=%u batch_count=%u",
...
bool UbMemStarsSdma::AppendDescriptorTasks(const UbMemAicpuTransferDesc &desc, UbMemRtsqState &state,
  ...
  while (remaining > 0U) {
    if (batch.count == kMaxRtsqEntriesPerPublish && !PublishRtsqBatch(state, batch, deadline)) {
      HIXL_LOGE(FAILED,
                "[UbMem][AICPU] flush full SDMA batch failed. device=%u sq=%u remaining=%llu src=0x%llx "
                "dst=0x%llx",
...
bool UbMemStarsSdma::AppendNotifyTask(uint32_t notify_id, UbMemRtsqState &state, UbMemRtsqBatch &batch,
                                      uint64_t deadline) {
  if (batch.count == kMaxRtsqEntriesPerPublish && !PublishRtsqBatch(state, batch, deadline)) {
    HIXL_LOGE(FAILED,
              "[UbMem][AICPU] flush full batch before NotifyRecord failed. device=%u sq=%u stream=%u "
