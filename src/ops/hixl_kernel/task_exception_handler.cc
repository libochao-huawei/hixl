...
std::string HcommExceptionInfoToString(const HcommExceptionInfo *info) {
  ...
  if (info->expandInfo.type == HCOMM_EXCEPTION_STARS) {
    const auto &s = info->expandInfo.u.starsInfo;
...
void HixlTaskExceptionCallback(const HcommExceptionInfo *exceptionInfo) {
  if (exceptionInfo == nullptr) {
    HIXL_LOGW("[TaskExceptionHandler] Exception callback received null exceptionInfo");
    return;
  }
  uint64_t thread_handle = exceptionInfo->thread;
  uint32_t error_code = exceptionInfo->retCode;
  HIXL_LOGW("[TaskExceptionHandler] Exception callback triggered. %s",
            HcommExceptionInfoToString(exceptionInfo).c_str());

  auto ctx = TransferContextManager::Instance().Get(thread_handle);
  if (ctx == nullptr) {
    HIXL_LOGW("[TaskExceptionHandler] Thread %lu not found in mapping table", thread_handle);
    return;
  }

  if (ctx->GetState() != TRANSFER_THREAD_STATE_INITIALIZED) {
    HIXL_LOGW("[TaskExceptionHandler] Thread %lu state is not INITIALIZED, state=%u", thread_handle,
  ...
  if (notify_ret != SUCCESS) {
    HIXL_LOGW("[TaskExceptionHandler] NotifyTsfwTaskException failed, ret=%u", notify_ret);
  }
}
