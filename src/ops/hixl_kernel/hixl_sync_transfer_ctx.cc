...
void HixlTaskExceptionCallback(uint64_t thread_handle, uint32_t error_code, uint32_t exe_stream_id, uint32_t task_id) {
  ...
  *err_flag_ptr = static_cast<uint8_t>(error_code);
  HIXL_LOGI("[HixlSyncTransferContext] Written err_flag=%u to va=%lu", error_code, entry.err_flag_dev_va);
...
uint32_t ProcessSyncTypeAdd(const TransferContextSyncEntry &op) {
...

  {
    ...
    HIXL_LOGI("[HixlSyncTransferContext] ADD: thread=%lu, stream=%u, notify=%u, err_flag_va=%lu", op.thread_handle,
    ...
    if (!g_callback_registered && HcommRegisterTaskExceptionCallback != nullptr) {
      ...
      if (ret == HCOMM_API_SUCCESS) {
        ...
        HIXL_LOGI("[HixlSyncTransferContext] HcommRegisterTaskExceptionCallback success");
      } else {
...
uint32_t ProcessSyncTypeDelete(const TransferContextSyncEntry &op) {
  {
    std::lock_guard<std::mutex> lock(g_table_mutex);
    auto it = g_transfer_ctx_table.find(op.thread_handle);
    if (it != g_transfer_ctx_table.end()) {
      g_transfer_ctx_table.erase(it);
      HIXL_LOGI("[HixlSyncTransferContext] DELETE: thread=%lu removed", op.thread_handle);
    } else {
      HIXL_LOGI("[HixlSyncTransferContext] DELETE: thread=%lu not found", op.thread_handle);
    }
    ...
    if (g_transfer_ctx_table.empty() && g_callback_registered && HcommRegisterTaskExceptionCallback != nullptr) {
      ...
      if (ret == HCOMM_API_SUCCESS) {
        ...
        HIXL_LOGI("[HixlSyncTransferContext] Unregister callback success");
      } else {
...
uint32_t HixlSyncTransferContextImpl(TransferContextSyncParam *param) {
  if (param == nullptr) {
    HIXL_LOGE(PARAM_INVALID, "[HixlSyncTransferContext] param is nullptr");
    return PARAM_INVALID;
  }
  if (param->count == 0U) {
    HIXL_LOGI("[HixlSyncTransferContext] count is 0, nothing to do");
    return SUCCESS;
  }
  if (param->entries_addr == 0U) {
    HIXL_LOGE(PARAM_INVALID, "[HixlSyncTransferContext] entries_addr is null");
    return PARAM_INVALID;
  }

  auto *ops = reinterpret_cast<TransferContextSyncEntry *>(static_cast<uintptr_t>(param->entries_addr));
  for (uint32_t i = 0U; i < param->count; i++) {
    const TransferContextSyncEntry &op = ops[i];
    HIXL_LOGI("[HixlSyncTransferContext] Processing op[%u]: sync_type=%u, thread=%lu", i, op.sync_type,
              op.thread_handle);
    ...
    if (op.sync_type == SYNC_TYPE_ADD) {
      ...
      HIXL_LOGE(PARAM_INVALID, "[HixlSyncTransferContext] Invalid sync_type=%u at index %u", op.sync_type, i);
      return PARAM_INVALID;
    }
    if (ret != SUCCESS) {
      HIXL_LOGE(ret, "[HixlSyncTransferContext] Process op[%u] failed, ret=%u", i, ret);
      return ret;
    }
  }
...
uint32_t HixlSyncTransferContext(TransferContextSyncParam *param) {
  return hixl::HixlSyncTransferContextImpl(param);
}
}  // extern "C"
