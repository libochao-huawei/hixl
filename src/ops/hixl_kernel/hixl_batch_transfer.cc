...
uint32_t HixlBatchTransfer(bool is_read, HixlOneSideOpParam *param) {
  ...
  HIXL_CHK_BOOL_RET_STATUS(ret == 0, hixl::FAILED, "[HixlBatchPutAndGet] HcommBatchModeStart faild, ret is %d", ret);
  ret = HixlBatchTransferTask(is_read, param);
  HIXL_CHK_BOOL_RET_STATUS(ret == 0, hixl::FAILED, "[HixlBatchPutAndGet] HixlBatchTransferTask faild, ret is %d", ret);
  ret = HcommChannelFenceOnThread(param->thread, param->channel);
  HIXL_CHK_BOOL_RET_STATUS(ret == 0, hixl::FAILED, "[HixlBatchPutAndGet] HcommChannelFenceOnThread faild, ret is %d",
                           ret);
  HIXL_LOGI(
  ...
  ret = HcommReadOnThread(
      param->thread, param->channel, reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(param->local_flag_addr)),
  ...
  HIXL_CHK_BOOL_RET_STATUS(ret == 0, hixl::FAILED, "[HixlBatchPutAndGet] HcommBatchModeEnd faild, ret is %d", ret);
