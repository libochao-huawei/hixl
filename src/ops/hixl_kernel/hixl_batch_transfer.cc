  ...
  if (is_read) {
    ...
    for (uint32_t i = 0; i < param->list_num; i++) {
      HIXL_LOGI(
          ...
          param->len_list[i]);
      int32_t ret = HcommReadOnThread(param->thread, param->channel, param->dst_buf_list[i], param->src_buf_list[i],
                                      param->len_list[i]);
      if (ret != 0) {
        HIXL_LOGE(FAILED,
                  "HcommReadOnThread failed. The address information is as follows:dst_buf:%p, scr_buf:%p, buf_len:%u, ret is %d.",
                  param->dst_buf_list[i], param->src_buf_list[i], param->len_list[i], ret);
        return FAILED;
      }
    }
  } else {
    ...
    for (uint32_t i = 0; i < param->list_num; i++) {
      HIXL_LOGI(
          ...
          param->list_num, i, param->thread, param->channel, i, param->dst_buf_list[i], i, param->src_buf_list[i], i,
          param->len_list[i]);
      int32_t ret = HcommWriteOnThread(param->thread, param->channel, param->dst_buf_list[i], param->src_buf_list[i],
                                       param->len_list[i]);
      if (ret != 0) {
        HIXL_LOGE(FAILED,
                  "HcommWriteOnThread failed. The address information is as follows:dst_buf:%p, scr_buf:%p, buf_len:%u, ret is %d.",
                  param->dst_buf_list[i], param->src_buf_list[i], param->len_list[i], ret);
        return FAILED;
      }
    }
...
extern "C" uint32_t HixlBatchTransfer(bool is_read, HixlOneSideOpParam *param) {
  ...
  if (ret != 0) {
    HIXL_LOGE(FAILED, "[HixlBatchPutAndGet] HcommBatchModeStart faild, ret is %d", ret);
    return FAILED;
  }
  ret = HixlBatchTransferTask(is_read, param);
  if (ret != 0) {
    HIXL_LOGE(FAILED, "[HixlBatchPutAndGet] HixlBatchTransferTask faild, ret is %d", ret);
    return FAILED;
  }
  ret = HcommChannelFenceOnThread(param->thread, param->channel);
  if (ret != 0) {
    HIXL_LOGE(FAILED, "[HixlBatchPutAndGet] HcommChannelFenceOnThread faild, ret is %d", ret);
    return FAILED;
  }
  HIXL_LOGI(
      "[HixlBatchPutAndGet] HcommReadOnThread start to read remote flag, param->flag_size=%u, param->local_flag=%lu, "
      "param->remote_flag=%lu",
      param->flag_size, param->local_flag, param->remote_flag);
  ret = HcommReadOnThread(param->thread, param->channel, reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(&param->local_flag))
,reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(&param->remote_flag)),param->flag_size);
  if (ret != 0) {
    HIXL_LOGE(
        FAILED,
        "[HixlBatchPutAndGet] Remote flag read failed. The address information is as follows:dst_buf:%u, scr_buf:%u, buf_len:%u, ret is %d.",
