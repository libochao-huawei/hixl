...
Status AdxlInnerEngine::TransferAsync(const AscendString &remote_engine,
  ...
  std::lock_guard<std::mutex> transfer_lock(channel->GetTransferMutex());
  ADXL_CHK_STATUS_RET(channel->TransferAsync(operation, op_descs, stream_pool, optional_args, req),
...
Status AdxlInnerEngine::GetTransferStatus(const TransferReq &req, const std::shared_ptr<StreamPool> &stream_pool, 
  ...
  ADXL_CHK_BOOL_RET_STATUS(channel != nullptr, NOT_CONNECTED,
                           "Failed to get channel, remote_engine:%s", remote_engine.GetString());
  auto ret = channel->GetTransferStatus(req, stream_pool, status);
