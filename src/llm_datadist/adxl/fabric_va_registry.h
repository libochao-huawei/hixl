...
class FabricVaRegistry {
 public:
  ...
  void RegisterImporterContext(int32_t device_id, aclrtContext context);
  void UnregisterImporterContext(int32_t device_id);
 ...
 private:
  ...
  struct ImportedSegmentMeta {
    VaInfo va_info;
    int32_t importer_device_id = -1;
    std::unordered_set<int32_t> access_granted_devices;
  };

  struct PeerImportedState {
    int consumer_count = 0;
    std::unordered_map<uintptr_t, ImportedSegmentMeta> imported_segments;
  ...
  static Status InvokeMemSetAccessOnImporterContext(void *vir_ptr, size_t size, int32_t target_device_id,
                                                    aclrtContext importer_context);
  Status GrantImportedSegmentAccess(ImportedSegmentMeta &meta, uintptr_t new_va, int32_t target_device_id);
  static uintptr_t FindNewVaForRemote(const PeerImportedState &state, uintptr_t remote_va, size_t len);

  std::mutex mutex_;
  std::unordered_map<std::string, PeerImportedState> peers_;
  std::unordered_map<int32_t, aclrtContext> importer_device_to_context_;
};
