...
constexpr uint64_t kKiloByte = 1000UL;
...
void StatisticManager::RemoveChannel(const std::string &channel_id) {
  ...
  transfer_statistic_info_.erase(GetClientStatisticChannelId(channel_id));
  transfer_statistic_info_.erase(GetServerStatisticChannelId(channel_id));
...
uint64_t GetAvgTransferBytes(uint64_t total_bytes, uint64_t total_times) {
  if (total_bytes == 0U || total_times == 0U) {
    return 0U;
  }
  return total_bytes / total_times;
}
...
void StatisticManager::UpdateFabricMemCosts(const std::string &channel_id, uint64_t transfer_cost,
                                            uint64_t real_copy_cost, uint64_t total_bytes) {
  auto info = GetOrCreateStatisticInfo(channel_id);
...
void StatisticManager::DumpBufferTransferStatisticInfo() {
  ...
  for (const auto &item : transfer_statistic_info_) {
    ...
    LLMEVENT(
        ...
        ToKBytes(GetAvgTransferBytes(total_bytes, transfer_times)),
