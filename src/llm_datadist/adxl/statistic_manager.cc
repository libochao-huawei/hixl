...
void StatisticManager::Reset() {
  std::shared_lock<std::shared_mutex> lock(map_mutex_);
  for (const auto &item : transfer_statistic_info_) {
    item.second->connect_statistic_info.Reset();
    item.second->buffer_transfer_statistic_info.Reset();
    item.second->fabric_mem_transfer_statistic_info.Reset();
    item.second->direct_transfer_statistic_info.Reset();
  }
}
}  // namespace adxl
