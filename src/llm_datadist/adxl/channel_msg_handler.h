...
struct ChannelConnectInfo {
  ...
  int32_t timeout;
  // Client-generated per-connection tag, sent on the wire so both ends build the same unique comm name.
  uint64_t comm_seq{0};
...
class ChannelMsgHandler {
 ...
 private:
  ...
  // Monotonic per-connection tag generated by the client (initiator) to make each comm name unique.
  std::atomic<uint64_t> comm_seq_{0};
