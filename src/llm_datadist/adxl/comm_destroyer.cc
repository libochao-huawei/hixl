...
void CommDestroyer::Enqueue(HcclComm comm, std::vector<void *> bind_handles) {
  if (comm == nullptr) {
    return;
  }
  int64_t delay = 0;
  ...
  cv_.notify_all();
  LLMLOGI("Enqueue comm domain %p for async destroy after %ld ms.", comm, delay);
}
...
void CommDestroyer::DestroyComm(const PendingDestroy &item) {
  for (auto handle : item.bind_handles) {
    ...
    if (unbind_ret != HcclResult::HCCL_SUCCESS) {
      LLMLOGE(FAILED, "Async HcclCommUnbindMem failed, comm:%p, ret:%d.", item.comm, static_cast<int32_t>(unbind_ret));
    }
  }
  auto destroy_ret = llm::HcclAdapter::GetInstance().HcclCommDestroy(item.comm);
  if (destroy_ret != HcclResult::HCCL_SUCCESS) {
    LLMLOGE(FAILED, "Async HcclCommDestroy failed, comm:%p, ret:%d.", item.comm, static_cast<int32_t>(destroy_ret));
    return;
  }
  LLMLOGI("Async HcclCommDestroy success, comm:%p.", item.comm);
}
