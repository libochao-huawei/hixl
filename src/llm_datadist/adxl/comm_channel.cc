...
Status CommChannel::ClearResources() {
  ...
  // (1) Abort the bound slot stream; sleep before comm destroy if the link failed or async work may still be in flight.
  ...
  }
  {
    ...
    if (active_slot_ != nullptr) {
      in_flight = true;
      if (active_slot_->stream != nullptr) {
  ...
  // (2) streamAbort cannot stop tasks already unfolding on aicpu; give them time to finish before destroying the comm.
  if (in_flight) {
  ...
  // (3) Unbind registered memory and destroy the comm.
  for (const auto &reg_handle_it : channel_info_.registered_mems) {
    auto reg_handle = reg_handle_it.first;
  ...
  // (4) Free per-request host flags and recycled flags, then return the bound slot to the pool (aborted).
  {
    std::lock_guard<std::mutex> reqs_lock(transfer_reqs_mutex_);
...
Status CommChannel::TransferAsync(TransferOp operation, const std::vector<TransferOpDesc> &op_descs,
  ...
  (void)optional_args;
  // Serialize the whole submit so concurrent threads never interleave their HcclBatch + host_flag copy on the one
  // shared stream, and so a concurrent poll cannot abort the stream mid-issue.
  std::lock_guard<std::mutex> launch_lock(device_launch_mu_);
  ...
  const Status issue_ret = [this, operation, &op_descs, &slot, host_flag]() -> Status {
    ...
    // Queue a device-to-host copy of dev_const_one behind the batch op; completion is observed by busy-reading
    // the host flag (no stream synchronize on the async path).
...
Status CommChannel::GetTransferStatus(const TransferReq &req, TransferStatus &status) {
  // Serialize against submit and other polls so the stream query / slot abort below cannot race a concurrent issue.
  std::lock_guard<std::mutex> launch_lock(device_launch_mu_);
  ADXL_CHK_STATUS_RET(CheckAvailableLocked(), "Channel unavailable.");
  const auto id = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(req));
  ...
  // 1) Cheap host-flag poll: the D2H copy of dev_const_one on the slot stream sets the flag to 1 on completion.
  ...
  // 2) Fallback: the host flag is a blind busy-wait that cannot sense errors; query the stream to detect
  //    progress (NOT_READY) or failure. No aclrtSynchronizeStream on the async path.
  ...
  // Stream reports complete: re-read the flag to confirm the completion signal actually landed.
  if (*flag_ptr == kHostFlagDoneValue) {
    std::atomic_thread_fence(std::memory_order_acquire);
    CompleteRequest(id, transfer_start, transfer_bytes, op_desc_count);
    status = TransferStatus::COMPLETED;
