...
Status ChannelMsgHandler::CreateChannel(const ChannelInfo &channel_info, bool is_client,
  ...
  StatisticManager::GetInstance().RegisterChannel(
      StatisticManager::GetStatisticChannelId(channel_info.channel_id, is_client));
...
Status ChannelMsgHandler::ProcessConnectRequest(int32_t fd, const char *msg, uint64_t msg_len, bool &keep_fd) {
  ...
  LLM_DISMISSABLE_GUARD(cleanup_failed_connect, ([this, &peer_connect_info]() {
    (void)channel_manager_->DestroyChannel(ChannelType::kServer, peer_connect_info.channel_id);
    StatisticManager::GetInstance().RemoveStatisticChannel(peer_connect_info.channel_id, false);
  }));
  ...
  if (ret == SUCCESS) {
    ...
    LLM_DISMISS_GUARD(cleanup_failed_connect);
  }
...
Status ChannelMsgHandler::DoConnect(const std::string &remote_engine, int32_t timeout_in_millis) {
  ...
  LLM_DISMISSABLE_GUARD(close_fd, ([conn_fd, this, &remote_engine]() {
    ...
    StatisticManager::GetInstance().RemoveStatisticChannel(remote_engine, true);
  }));
  ChannelConnectInfo peer_connect_info = {};
