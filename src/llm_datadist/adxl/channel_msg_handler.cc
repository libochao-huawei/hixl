...
Status ChannelMsgHandler::CreateChannel(const ChannelInfo &channel_info, bool is_client,
  ...
  StatisticManager::GetInstance().RegisterChannel(channel_info.channel_id);
...
Status ChannelMsgHandler::Connect(const std::string &remote_engine, int32_t timeout_in_millis) {
  ...
  StatisticManager::GetInstance().UpdateConnectTotalCost(remote_engine, cost);
...
Status ChannelMsgHandler::ConnectToPeer(const std::string &remote_engine, int32_t timeout_in_millis, int32_t &conn_fd) {
  ...
  StatisticManager::GetInstance().UpdateTcpConnectCost(remote_engine, tcp_cost);
  return SUCCESS;
}
