...
Status ChannelMsgHandler::CreateChannel(const ChannelInfo &channel_info, bool is_client,
  ...
  StatisticManager::GetInstance().RegisterChannel(
      StatisticManager::GetStatisticChannelId(channel_info.statistic_channel_id, is_client));
...
Status ChannelMsgHandler::ConnectInfoProcess(const ChannelConnectInfo &peer_channel_info,
  ...
  channel_info.statistic_channel_id = is_client ? peer_channel_info.channel_id : listen_info_;
...
Status ChannelMsgHandler::ProcessConnectRequest(int32_t fd, const char *msg, uint64_t msg_len, bool &keep_fd) {
  ...
  if (ret == SUCCESS) {
    ...
    StatisticManager::GetInstance().UpdateConnectTotalCost(
        StatisticManager::GetServerStatisticChannelId(listen_info_), cost);
  }
...
Status ChannelMsgHandler::FinalizeClientConnect(const std::string &remote_engine, int32_t timeout_in_millis,
                                                int32_t conn_fd, const ChannelConnectInfo &peer_connect_info) {
  (void)timeout_in_millis;
  (void)peer_connect_info;
  bool keep_fd = false;
  ADXL_CHK_STATUS_RET(StartChannelHeartbeat(remote_engine, ChannelType::kClient, conn_fd, keep_fd),
                      "Failed to start client channel heartbeat");
  (void)keep_fd;
  return SUCCESS;
}
...
Status ChannelMsgHandler::DoConnect(const std::string &remote_engine, int32_t timeout_in_millis) {
  ...
  ADXL_CHK_STATUS_RET(FinalizeClientConnect(remote_engine, timeout_in_millis, conn_fd, peer_connect_info),
                      "Failed to finalize client connect");
...
Status ChannelMsgHandler::CleanupDisconnectResources(const std::string &remote_engine) {
  ...
  StatisticManager::GetInstance().RemoveChannel(listen_info_);
  if (enable_use_fabric_mem_) {
