...
Status FabricVaRegistry::ImportRemoteShares(const std::string &peer_key,
  ...
  for (const auto &s : staged) {
    ...
    auto seg_it = state.imported_segments.find(s.new_va);
    ADXL_CHK_BOOL_RET_STATUS(seg_it != state.imported_segments.end(), FAILED, "Missing segment after staging va %lu.",
                             s.new_va);
    ADXL_CHK_STATUS_RET(GrantImportedSegmentAccess(seg_it->second, s.new_va, device_id),
                        "Failed to grant fabric mem access after first import.");
  }
  return SUCCESS;
}
