...
namespace adxl {
namespace {
constexpr const char *kLibAscendClSo = "libascendcl.so";
constexpr const char *kReserveMemNoUcMemoryFunc = "aclrtReserveMemAddressNoUCMemory";
constexpr size_t kDefaultGlobalVirtualMemorySize = 1024UL * 1024UL * 1024UL * 1024UL * 128UL;      // 128T
constexpr size_t kDefaultNumBlocks = 128UL * 1024UL;                  // 128TB / 1GB = 131072 blocks
...
Status VirtualMemoryManager::Initialize() {
  ...
  void *handle = mmDlopen(kLibAscendClSo, static_cast<int32_t>(static_cast<uint32_t>(MMPA_RTLD_LAZY) | RTLD_NOLOAD));
  if (handle == nullptr) {
    ADXL_CHK_ACL_RET(
        aclrtReserveMemAddress(&global_virtual_memory_, vm_size_, 0, nullptr, kReserveFlagHugePage));
  } else {
    ...
    if (reserve_no_uc_mem_func == nullptr) {
      ...
      int ret = reserve_no_uc_mem_func(&global_virtual_memory_, vm_size_, 0, global_start_va,
  ...
  allocations_.clear();
  initialized_ = true;
  LLMLOGI("VirtualMemoryManager initialized, reserved 128TB at address: %lu", global_virtual_memory_addr_);
  return SUCCESS;
}
