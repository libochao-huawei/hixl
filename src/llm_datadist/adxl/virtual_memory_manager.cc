...
Status VirtualMemoryManager::Initialize() {
  ...
  auto ret = aclrtReserveMemAddressNoUCMemory(&global_virtual_memory_, kGlobalVirtualMemorySize, 0, global_start_va,
                                              kReserveFlagHugePage);
  if (ret == ACL_ERROR_RT_FEATURE_NOT_SUPPORT) {
    ADXL_CHK_ACL_RET(aclrtReserveMemAddress(&global_virtual_memory_, kGlobalVirtualMemorySize, 0, global_start_va,
                                            kReserveFlagHugePage));
  } else {
...
Status VirtualMemoryManager::ReleaseMemory(uintptr_t mem_addr) {
  ...
  if (mem_addr < global_virtual_memory_addr_ || mem_addr >= global_virtual_memory_addr_ + kGlobalVirtualMemorySize) {
