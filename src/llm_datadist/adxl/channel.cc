...
Status Channel::TransferAsync(TransferOp operation,
  ...
  TransferRequest *req_handle = new TransferRequest();
  req_handle->remote_engine = channel_info_.channel_id;
  transfer_reqs_[req_handle] = std::move(event);
  req = req_handle;
  return SUCCESS;
}
...
Status Channel::GetTransferStatus(const TransferReq &req, TransferStatus &status) {
  std::lock_guard<std::mutex> lock(mutex_);
  auto req_handle = reinterpret_cast<TransferRequest*>(req);
  auto it = transfer_reqs_.find(req_handle);
  if (it == transfer_reqs_.end()) {
    status = TransferStatus::FAILED;
    LLMLOGE(FAILED, "Request not found, req:%p.", req_handle);
    return FAILED;
  }
  ...
  if (ret != RT_ERROR_NONE) {
    LLMLOGE(FAILED, "rtEventQueryStatus failed for req:%p, ret:%d.", req_handle, ret);
    rtEventDestroy(event);
    delete req_handle;
    transfer_reqs_.erase(req_handle);
    status = TransferStatus::FAILED;
    return FAILED;
  }
  if (event_status != RT_EVENT_RECORDED) {
    LLMLOGI("Transfer async request not yet completed, req:%p.", req_handle);
    status = TransferStatus::WAITING;
    return SUCCESS;
  }
  LLMLOGI("Transfer async request completed, req:%p.", req_handle);
