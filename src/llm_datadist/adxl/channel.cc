...
std::string Channel::GetStatisticChannelId() const {
  return StatisticManager::GetStatisticChannelId(channel_info_.statistic_channel_id,
...
Status Channel::TransferAsync(TransferOp operation,
  ...
  req_2_async_record_[id] = AsyncRecord{std::move(async_resources), transfer_start, transfer_start};
  LLM_DISMISS_GUARD(fail_guard);
  return SUCCESS;
}
...
Status Channel::GetTransferStatus(const TransferReq &req, TransferStatus &status) {
  ...
  StatisticManager::GetInstance().UpdateDirectTransferCost(GetStatisticChannelId(), cost);
