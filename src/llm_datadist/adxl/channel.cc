...
void Channel::ClearImportedMem() {
  fabric_mem_remote_memory_.Finalize();
}
...
Status Channel::ImportMem(const std::vector<ShareHandleInfo> &remote_share_handles, int32_t device_id) {
  ADXL_CHK_STATUS_RET(fabric_mem_remote_memory_.Import(remote_share_handles, device_id),
                      "Failed to import fabric mem.");
  return SUCCESS;
}

std::unordered_map<uintptr_t, VaInfo> Channel::GetNewVaToOldVa() {
  return fabric_mem_remote_memory_.GetNewVaToOldVa();
}
