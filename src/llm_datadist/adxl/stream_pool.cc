...
Status StreamPool::GetStream(rtStream_t &stream) {
  ...
  if (pool_.size() < max_stream_num_) {
    ...
    stream = new_stream;
    LLMEVENT("Create new stream, current stream pool size: %zu", pool_.size());
    return SUCCESS;
  }  
  LLMEVENT("Stream pool is full, current stream pool size: %zu", pool_.size());
  return FAILED;
}
...
void StreamPool::DestoryStream(rtStream_t &stream) {
  std::lock_guard<std::mutex> lock(stream_pool_mutex_);
  auto it = pool_.find(stream);
  if (it != pool_.end()) {
    (void)rtStreamAbort(stream);
    auto rt_ret = rtStreamDestroy(stream);
