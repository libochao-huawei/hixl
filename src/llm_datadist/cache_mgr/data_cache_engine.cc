...
ge::Status DataCacheEngine::SwapBlocks(const Cache &src, const Cache &dst, const uint64_t block_size,
  ...
  const uint64_t src_num_blocks =
      src_cache_entry.num_blocks > 0U ? src_cache_entry.num_blocks : static_cast<uint64_t>(src_cache_entry.batch_size);
  ...
  const uint64_t dst_num_blocks =
      dst_cache_entry.num_blocks > 0U ? dst_cache_entry.num_blocks : static_cast<uint64_t>(dst_cache_entry.batch_size);
