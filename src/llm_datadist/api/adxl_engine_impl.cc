...
hixl::TransferArgs ToHixlTransferArgs(const TransferArgs &optional_args) {
  hixl::TransferArgs hixl_args;
  (void)memcpy_s(&hixl_args, sizeof(hixl_args), &optional_args, sizeof(optional_args));
  return hixl_args;
}
...
Status AdxlEngine::AdxlEngineImpl::TransferAsync(const AscendString &remote_engine,
                                                 ...
                                                 const TransferArgs &optional_args,
                                                 TransferReq &req) {
  ADXL_CHK_BOOL_RET_STATUS(engine_ != nullptr && engine_->IsInitialized(), FAILED, "Hixl is not initialized.");
  ADXL_CHK_STATUS_RET(CheckTransferOpDescs(op_descs), "Failed to check transfer op descs.");
  ADXL_CHK_STATUS_RET(engine_->TransferAsync(remote_engine, ToHixlTransferOp(operation),
                                             ToHixlTransferOpDescs(op_descs), ToHixlTransferArgs(optional_args), req),
