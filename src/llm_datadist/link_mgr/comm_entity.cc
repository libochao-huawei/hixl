...
ge::Status RegBufferPool::Initialize() {
  ...
  if (is_host_) {
    ...
    LLM_CHK_ACL_RET(aclrtMalloc(&buffer_, buffer_size, ACL_MEM_MALLOC_HUGE_FIRST));
  }
...
ge::Status CommEntity::Finalize() {
  ...
  if (GetStream() != nullptr) {
    auto rt_ret = aclrtStreamAbort(GetStream());
    LLMLOGI("Call aclrtStreamAbort ret:%d.", ret);
    ret = rt_ret != ACL_ERROR_NONE ? ge::LLM_UNLINK_FAILED : ret;
  }
  ...
  if (GetStream() != nullptr) {
    auto rt_ret = aclrtDestroyStream(GetStream());
