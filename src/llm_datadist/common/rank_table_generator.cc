...
std::string LocalCommResGenerator::GetHccnOutput(std::string command) {
  ...
  std::string result;
  std::unique_ptr<FILE, decltype(&pclose)> pipe(popen(command_with_stderr.cstr(), "r"), pclose);
  if (!pipe) {
    LLMLOGE(ge::LLM_FAILED, "calling command %s failed, cannot create subprocess.", command_with_stderr.cstr());
  }
