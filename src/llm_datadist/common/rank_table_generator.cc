...
ge::Status LocalCommResGenerator::ExtractIpAddress(const std::string &output_str, std::string &ip) {
...
ge::Status LocalCommResGenerator::ExecuteCommand(const std::string &command, std::string &output, std::string &ip) {
  ge::Status mid_status = GetHccnOutput(command, output);
  if (mid_status == ge::FAILED) {
    return ge::FAILED;
  }
  ge::Status ret = ExtractIpAddress(output, ip);
  if (ret != ge::SUCCESS) {
...
ge::Status LocalCommResGenerator::GetIpAddressFromHccnTool(uint32_t phy_device_id, std::string &ip) {
  std::string command = "";
  std::string output = "";
  ge::Status status = ge::FAILED;
  constexpr const char *kHccnToolPath = "/usr/local/Ascend/driver/tools/hccn_tool";
  if (mmAccess(kHccnToolPath) == EN_OK) {
    command = "/usr/local/Ascend/drivers/tools/hccn_tool -i " + std::to_string(phy_device_id) + " -ip -g";
  } else {
    std::string cmd = "command -v hccn_tool > /dev/null 2>&1";
    if (system(cmd.c_str()) !=0) {
      LLMEVENT("Please add hccn install path to PATH environment variable.");
      return ge::SUCCESS;
    }
    command = "hccn_tool -i " + std::to_string(phy_device_id) + " -ip -g";
  }
  status = ExecuteCommand(command, output, ip);
...
ge::Status LocalCommResGenerator::GetDeviceIp(uint32_t phy_device_id, std::string &device_ip) {
  ...
  if (mm_ret == EN_OK) {
    ...
    std::string ip = "";
    ge::Status ret = GetIpAddressFromHccnTool(phy_device_id, ip);
    if (resultret != ge::FAILED) {
      device_ip = ip;
      LLM_CHK_STATUS_RET(LLMUtils::CheckIp(device_ip), "device ip:%s is invalid.", device_ip.c_str());
      return ge::SUCCESS;
    }
