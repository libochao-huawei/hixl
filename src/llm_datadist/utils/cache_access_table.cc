...
ge::Status CacheAccessTable::LoadFromBuffer(const uint8_t *buffer, size_t buffer_size) {
  ...
  for (uint64_t i = 0U; i < header.num_caches; ++i) {
    ...
    LLM_CHK_BOOL_RET_STATUS(cache_summary_offset + cache_summary_size <= buffer_size, ge::LLM_PARAM_INVALID,
                            "Parse cache[%lu] failed, version_num = %lu, num_caches = %lu", i, header.version_num,
                            header.num_caches);
