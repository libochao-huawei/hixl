...
void *SharedDevBuffer::GetOrCreateBuffer(size_t size) {
  ...
  if (buffer_ == nullptr) {
    LLM_ASSERT_RT_OK(aclrtMalloc(&buffer_, size, ACL_MEM_TYPE_HIGH_BRAND_WIDTH | ACL_MEM_MALLOC_HUGE_ONLY));
...
ge::Status CacheAccessTableUpdater::Initialize(bool enable) {
  ...
  if (enable) {
    ...
    LLM_CHK_ACL_RET(aclrtMalloc(&dev_buffer_, buffer_size_,
                                ACL_MEM_TYPE_HIGH_BRAND_WIDTH | ACL_MEM_MALLOC_HUGE_ONLY));
  } else {
