...
aclError RegisterProfType() {
  for (auto &iter: kProfTypeToNames) {
...
aclError AddDeviceList(const uint32_t *const device_id_list, const uint32_t device_nums) {
  ...
  for (size_t idx = 0; idx < device_nums; idx++) {
...
aclError ProcessProfData(void *const data, const uint32_t len) {
  ...
  if (((prof_switch & kAclProfHixlApi) != 0U) && (type == kStartProfiling)) {
    ret = ProfInnerStart(profiler_config);
  }
  if (((prof_switch & kAclProfHixlApi) != 0U) && (type == kStopProfiling)) {
...
aclError HixlProfCtrlHandle(uint32_t data_type, void *data, uint32_t data_len) {
  ...
  if (data_type == PROF_CTRL_SWITCH) {
    ...
    if (ret != ACL_SUCCESS) {
      HIXL_LOGE(ret, "[Hixl Profiling] [Process][ProfSwitch]failed to call ProcessProfData, result is %d", ret);
      return ret;
    }
    return ACL_SUCCESS;
  }
...
public:
  HixlRegProfCallback() {
    const auto prof_ret = MsprofRegisterCallback(HIXL, &HixlProfCtrlHandle);
    if (prof_ret != 0) {
...
namespace hixl {
  ...
  Status HixlProfilingReporter::GetStartTime(uint64_t &start_time) {
    const std::lock_guard<std::mutex> lk(g_prof_mutex);
    if (g_prof_run) {
      HIXL_LOGI("[Hixl Profiling] Profiling is enabled. Get start time.");
      start_time = MsprofSysCycleTime();
    }
    return SUCCESS;
  }
