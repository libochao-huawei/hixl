...
Status CompletePool::EnsureThreadLocked(Slot &slot, CommEngine engine, uint32_t thread_num,
  ...
  HIXL_CHK_HCCL_RET(HcommProxy::HcommThreadAlloc(engine, thread_num, notify_num_per_thread, &slot.thread));
  return SUCCESS;
}
...
void CompletePool::DestroySlotLocked(Slot &slot) const {
  ...
  if (slot.thread != 0U) {
    HIXL_CHK_ACL(HcommProxy::HcommThreadFree(&slot.thread, 1U), "HcommThreadFree failed");
    slot.thread = 0U;
  }
  if (slot.stream != nullptr) {
