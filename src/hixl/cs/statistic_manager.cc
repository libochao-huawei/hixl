...
void TransferStatisticInfo::Reset() {
  ...
  device_prepare_batch_mem.Reset();
  device_prepare_remote_flag.Reset();
  device_fill_args.Reset();
  device_launch_kernel.Reset();
  device_sync_wait.Reset();
...
void HixlCSStatisticManager::RenameChannel(const std::string &old_channel_id, const std::string &new_channel_id) {
  if (old_channel_id.empty() || old_channel_id == new_channel_id) {
    RegisterChannel(new_channel_id);
    return;
  }
  std::unique_lock<std::shared_mutex> lock(map_mutex_);
  const auto old_it = statistic_info_.find(old_channel_id);
  if (old_it == statistic_info_.end()) {
    statistic_info_.try_emplace(new_channel_id, std::make_shared<StatisticInfo>());
    return;
  }
  statistic_info_[new_channel_id] = old_it->second;
  statistic_info_.erase(old_it);
}
...
void HixlCSStatisticManager::UpdateStageCost(const std::string &channel_id, StatisticStage stage, uint64_t cost_us) {
  ...
  switch (stage) {
    case StatisticStage::kClientCreate:
      UpdateCost(cost_us, info->connect.client_create.times, info->connect.client_create.max_cost,
                 info->connect.client_create.total_cost);
      break;
    ...
    case StatisticStage::kCheckStatusDevice:
      ...
      break;
    case StatisticStage::kDevicePrepareBatchMem:
      UpdateCost(cost_us, info->transfer.device_prepare_batch_mem.times, info->transfer.device_prepare_batch_mem.max_cost,
                 info->transfer.device_prepare_batch_mem.total_cost);
      break;
    case StatisticStage::kDevicePrepareRemoteFlag:
      UpdateCost(cost_us, info->transfer.device_prepare_remote_flag.times,
                 info->transfer.device_prepare_remote_flag.max_cost, info->transfer.device_prepare_remote_flag.total_cost);
      break;
    case StatisticStage::kDeviceFillArgs:
      UpdateCost(cost_us, info->transfer.device_fill_args.times, info->transfer.device_fill_args.max_cost,
                 info->transfer.device_fill_args.total_cost);
      break;
    case StatisticStage::kDeviceLaunchKernel:
      UpdateCost(cost_us, info->transfer.device_launch_kernel.times, info->transfer.device_launch_kernel.max_cost,
                 info->transfer.device_launch_kernel.total_cost);
      break;
    case StatisticStage::kDeviceSyncWait:
      UpdateCost(cost_us, info->transfer.device_sync_wait.times, info->transfer.device_sync_wait.max_cost,
                 info->transfer.device_sync_wait.total_cost);
      break;
  ...
  if (info->connect.client_create.times.load(std::memory_order_relaxed) > kResetTimes ||
      info->connect.connect_total.times.load(std::memory_order_relaxed) > kResetTimes) {
    info->connect.Reset();
  }
...
StatisticInfoSnapshot HixlCSStatisticManager::GetSnapshot(const std::string &channel_id) const {
  ...
  snapshot.connect.client_create = ToSnapshot(info->connect.client_create);
  ...
  snapshot.transfer.device_prepare_batch_mem = ToSnapshot(info->transfer.device_prepare_batch_mem);
  snapshot.transfer.device_prepare_remote_flag = ToSnapshot(info->transfer.device_prepare_remote_flag);
  snapshot.transfer.device_fill_args = ToSnapshot(info->transfer.device_fill_args);
  snapshot.transfer.device_launch_kernel = ToSnapshot(info->transfer.device_launch_kernel);
  snapshot.transfer.device_sync_wait = ToSnapshot(info->transfer.device_sync_wait);
...
void HixlCSStatisticManager::DumpConnectStatisticInfo() {
  ...
  for (const auto &item : statistic_info_) {
    ...
    if (connect.client_create.times.load(std::memory_order_relaxed) == 0U &&
        connect.connect_total.times.load(std::memory_order_relaxed) == 0U &&
    ...
    HIXL_EVENT("HIXL CS connect statistic info[channel:%s, client_create times:%" PRIu64 ", client_create avg cost:%" PRIu64
               " us, total times:%" PRIu64 ", max cost:%" PRIu64 " us, avg cost:%" PRIu64 " us, tcp avg cost:%" PRIu64
               " us, match avg cost:%" PRIu64
               ...
               item.first.c_str(), connect.client_create.times.load(std::memory_order_relaxed),
               GetAvgCost(connect.client_create), connect.connect_total.times.load(std::memory_order_relaxed),
...
void HixlCSStatisticManager::DumpTransferStatisticInfo() {
  ...
  for (const auto &item : statistic_info_) {
    ...
    HIXL_EVENT("HIXL CS transfer statistic info[channel:%s, transfer times:%" PRIu64 ", total size:%" PRIu64
               ...
               " us, avg device query cost:%" PRIu64 " us, device_prepare_batch avg cost:%" PRIu64
               " us, device_prepare_flag avg cost:%" PRIu64 " us, device_fill_args avg cost:%" PRIu64
               " us, device_launch avg cost:%" PRIu64 " us, device_wait avg cost:%" PRIu64
               " us, bandwidth:%.3f GB/s, host times:%" PRIu64
               ...
               GetAvgCost(transfer.check_status_device), GetAvgCost(transfer.device_prepare_batch_mem),
               GetAvgCost(transfer.device_prepare_remote_flag), GetAvgCost(transfer.device_fill_args),
               GetAvgCost(transfer.device_launch_kernel), GetAvgCost(transfer.device_sync_wait),
               GetBandwidthGbps(total_bytes, total_cost),
