...
hixl::Status CheckMatchResultAndSetHandle(const hixl::MatchEndpointResp &resp, uint64_t &remote_endpoint_handle) {
...
hixl::Status CheckCreateChannelResult(const hixl::CreateChannelResp &resp) {
  HIXL_CHK_BOOL_RET_STATUS(resp.result == hixl::SUCCESS, hixl::FAILED,
...
hixl::Status RecvMatchEndpointHandleResponse(int32_t socket, uint64_t &remote_endpoint_handle, uint32_t timeout_ms) {
  ...
  return CheckMatchResultAndSetHandle(resp, remote_endpoint_handle);
}
...
hixl::Status RecvCreateChannelOnlyResponse(int32_t socket, uint32_t timeout_ms) {
  ...
  return CheckCreateChannelResult(resp);
}
}  // namespace
namespace hixl {
