...
void TransferPool::AbortSlotByIndexLocked(uint32_t slot_index) {
  ...
  DestroySlotThreadForAbortLocked(slot, slot_index);
...
void TransferPool::DestroySlotThreadForAbortLocked(Slot &slot, uint32_t slot_index) const {
  if (slot.thread != 0U) {
    Status sync_ret =
        SyncOneTransferContextLocked(slot.thread, TRANSFER_CONTEXT_OP_DESTROY, TRANSFER_THREAD_STATE_DELETED);
...
Status TransferPool::HandleSyncContextTimeout(const std::vector<TransferContextSyncEntry> &pending,
  ...
  for (size_t i = 0U; i < pending.size(); ++i) {
    ...
    HIXL_EVENT(
        "[TransferPool] destroy transfer context timeout after %u ms, force cleanup. thread=%lu state=%u device_id=%d",
...
Status TransferPool::DestroyTransferContextsLocked(const std::vector<ThreadHandle> &threads) const {
  return SyncContextsLocked(threads, TRANSFER_CONTEXT_OP_DESTROY, TRANSFER_THREAD_STATE_DELETED);
}
