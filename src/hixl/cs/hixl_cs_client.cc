...
Status HixlCSClient::BatchTransferHost(bool is_get, const CommunicateMem &communicate_mem_param, void **query_handle) {
  ...
  const uint64_t total_bytes = SumTransferBytes(communicate_mem_param.len_list, communicate_mem_param.list_num);
  ...
  query_mem_handle->list_num = communicate_mem_param.list_num;
  ...
  *query_handle = query_mem_handle;
  ...
  LogPerfStage("transfer_submit_host", GetClientRole(local_endpoint_->GetEndpoint()), statistic_channel_id_,
               query_mem_handle->submit_cost_us, SUCCESS, socket_, client_channel_handle_, communicate_mem_param.list_num,
               total_bytes);
  return SUCCESS;
}
...
Status HixlCSClient::BatchTransferDevice(bool is_get, const CommunicateMem &communicate_mem_param, void **queryhandle) {
  ...
  *(static_cast<uint64_t *>(slot.host_flag)) = kDeviceFlagInitValue;
  ...
  handle->list_num = communicate_mem_param.list_num;
  ...
  RecordDeviceTransferStage(statistic_channel_id_, StatisticStage::kDevicePrepareBatchMem, "device_prepare_batch_mem",
                            ...
                            client_channel_handle_, communicate_mem_param.list_num, total_bytes);
  ...
  *queryhandle = static_cast<void *>(handle);
  ...
  LogPerfStage("transfer_submit_device", GetClientRole(local_endpoint_->GetEndpoint()), statistic_channel_id_,
               handle->submit_cost_us, SUCCESS, socket_, client_channel_handle_, communicate_mem_param.list_num, total_bytes);
  {
    std::lock_guard<std::mutex> lock(mutex_);
...
Status HixlCSClient::BatchTransferDeviceSync(bool is_get, const CommunicateMem &communicate_mem_param,
  ...
  *(static_cast<uint64_t *>(handle->slot->host_flag)) = kDeviceFlagInitValue;
  ...
  HixlCSStatisticManager::GetInstance().UpdateStageCost(statistic_channel_id_, StatisticStage::kDeviceSyncWait, wait_cost_us);
  LogPerfStage("device_sync_wait", GetClientRole(local_endpoint_->GetEndpoint()), statistic_channel_id_, wait_cost_us,
               SUCCESS, socket_, client_channel_handle_, communicate_mem_param.list_num, total_bytes, timeout_ms);
  ...
  HixlCSStatisticManager::GetInstance().UpdateTransferCost(statistic_channel_id_, total_cost_us,
                                                           total_cost_us > wait_cost_us ? total_cost_us - wait_cost_us : 0UL,
                                                           wait_cost_us, total_bytes, communicate_mem_param.list_num, true);
  LogPerfStage("transfer_sync_device", GetClientRole(local_endpoint_->GetEndpoint()), statistic_channel_id_,
               total_cost_us, SUCCESS, socket_, client_channel_handle_, communicate_mem_param.list_num, total_bytes,
               timeout_ms);
  ...
  while (true) {
    ...
    if (st == HixlCompleteStatus::HIXL_COMPLETE_STATUS_COMPLETED) {
      ...
      HixlCSStatisticManager::GetInstance().UpdateTransferCost(statistic_channel_id_, total_cost_us, 0UL, total_cost_us,
                                                               total_bytes, communicate_mem_param.list_num, false);
...
Status HixlCSClient::CheckStatusHost(CompleteHandleInfo &query_handle, HixlCompleteStatus &status) {
  ...
  if (*atomic_flag == kFlagDoneValue) {
    ...
    HixlCSStatisticManager::GetInstance().UpdateStageCost(statistic_channel_id_, StatisticStage::kCheckStatusHost,
                                                          check_cost_us);
    HixlCSStatisticManager::GetInstance().UpdateTransferCost(statistic_channel_id_, ElapsedUs(query_handle.start_time),
                                                             query_handle.submit_cost_us,
                                                             ElapsedUs(query_handle.start_time) >= query_handle.submit_cost_us
                                                                 ? ElapsedUs(query_handle.start_time) - query_handle.submit_cost_us
                                                                 : 0UL,
    ...
    LogPerfStage("check_status_host", GetClientRole(local_endpoint_->GetEndpoint()), statistic_channel_id_,
                 check_cost_us, SUCCESS, socket_, client_channel_handle_, query_handle.list_num, query_handle.total_bytes);
...
Status HixlCSClient::CheckStatusDevice(DeviceCompleteHandle &queryhandle, HixlCompleteStatus &status) {
  ...
  if (flag_val == kDeviceFlagDoneValue) {
    *flag_ptr = kDeviceFlagInitValue;
    ...
    const uint64_t check_cost_us = ElapsedUs(check_start);
    const uint64_t total_cost_us = ElapsedUs(queryhandle.start_time);
    const uint64_t wait_cost_us = total_cost_us >= queryhandle.submit_cost_us ? total_cost_us - queryhandle.submit_cost_us
                                                                               : 0UL;
    ...
    HixlCSStatisticManager::GetInstance().UpdateStageCost(statistic_channel_id_, StatisticStage::kDeviceSyncWait,
                                                          wait_cost_us);
    HixlCSStatisticManager::GetInstance().UpdateTransferCost(statistic_channel_id_, total_cost_us,
                                                             queryhandle.submit_cost_us, wait_cost_us,
                                                             queryhandle.total_bytes, queryhandle.list_num, true);
    LogPerfStage("check_status_device", GetClientRole(local_endpoint_->GetEndpoint()), statistic_channel_id_,
                 check_cost_us, SUCCESS, socket_, client_channel_handle_, queryhandle.list_num, queryhandle.total_bytes);
...
Status HixlCSClient::Connect(uint32_t timeout_ms) {
  ...
  RecordPerfStage(statistic_channel_id_, StatisticStage::kConnectTotal, "connect_total",
                  GetClientRole(local_endpoint_->GetEndpoint()), connect_start, SUCCESS, socket_, client_channel_handle_, 0U,
                  0UL, timeout_ms);
...
Status HixlCSClient::ExchangeEndpointAndCreateChannelLocked(uint32_t timeout_ms) {
  ...
  RecordPerfStage(statistic_channel_id_, StatisticStage::kMatchEndpoint, "match_endpoint",
                  GetClientRole(local_endpoint_->GetEndpoint()), stage_start, SUCCESS, socket_, client_channel_handle_, 0U,
                  0UL, timeout_ms);
