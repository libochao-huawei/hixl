...
Status HixlCSClient::InitTransferPoolResource(const EndpointDesc &ep) {
  if (!IsDeviceEndpoint(ep)) {
    return SUCCESS;
  }
...
Status HixlCSClient::Create(const HixlClientDesc *client_desc, const HixlClientConfig *config) {
  ...
  HIXL_CHK_STATUS_RET(InitTransferPoolResource(*(client_desc->local_endpoint)),
                      "[HixlClient] InitTransferPoolResource failed");
