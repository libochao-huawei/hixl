...
Status HixlCSClient::BatchTransferTask(bool is_get, const CommunicateMem &communicate_mem_param) {
  ...
  if (is_get) {
    ...
    for (uint32_t i = 0; i < communicate_mem_param.list_num; i++) {
      hccl_ret =
          HcclProxy::HcommReadNbiOnThread(static_cast<ThreadHandle>(0), client_channel_handle_, communicate_mem_param.dst_buf_list[i],
    ...
    for (uint32_t i = 0; i < communicate_mem_param.list_num; i++) {
      hccl_ret =
          HcclProxy::HcommWriteNbiOnThread(static_cast<ThreadHandle>(0), client_channel_handle_, communicate_mem_param.dst_buf_list[i],
                        const_cast<void *>(communicate_mem_param.src_buf_list[i]), communicate_mem_param.len_list[i]);
  ...
  hccl_ret = HcclProxy::HcommChannelFenceOnThread(static_cast<ThreadHandle>(0), client_channel_handle_);
  if (hccl_ret != 0) {  // ret值为0时表示执行成功
...
Status HixlCSClient::BatchTransferHost(bool is_get, const CommunicateMem &communicate_mem_param, void **query_handle) {
  ...
  int32_t hccl_ret =
      HcclProxy::HcommReadNbiOnThread(static_cast<ThreadHandle>(0), client_channel_handle_, flag_addr, tag_mem_descs_[kTransFlagName].addr, kFlagSizeBytes);
  if (hccl_ret != 0) {  // ret值为0时表示执行成功
    HIXL_LOGE(FAILED,
              "[HixlClient] HcommReadNbiOnThread failed, client_channel_handle_ is %lu, dst_addr is %p, src_addr is %p, "
              "mem_len is %lu, hccl_ret is %d.",
