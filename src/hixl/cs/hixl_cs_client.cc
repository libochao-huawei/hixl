...
Status HixlCSClient::Create(const HixlClientDesc *client_desc, const HixlClientConfig *config) {
  ...
  RecordPerfStage(statistic_channel_id_, StatisticStage::kConnectTotal, "client_create", GetClientRole(local_endpoint_->GetEndpoint()),
                  perf_start, SUCCESS, socket_, client_channel_handle_);
  return SUCCESS;
}
  ...
  if (flag_val == kDeviceFlagDoneValue) {
    *flag_ptr = kDeviceFlagInitValue;
    ...
    HixlCSStatisticManager::GetInstance().UpdateStageCost(statistic_channel_id_, StatisticStage::kCheckStatusDevice,
                                                          check_cost_us);
    HixlCSStatisticManager::GetInstance().UpdateTransferCost(statistic_channel_id_, ElapsedUs(queryhandle.start_time),
                                                             queryhandle.submit_cost_us,
                                                             ElapsedUs(queryhandle.start_time) >= queryhandle.submit_cost_us
                                                                 ? ElapsedUs(queryhandle.start_time) - queryhandle.submit_cost_us
                                                                 : 0UL,
