...
Status HixlCSClient::InitUbResource() {
  ...
  Status pret =
      CompletePool::GetInstance().AddRefAndInitIfNeeded(ub_device_id_, kUbEngine, kUbThreadNum, kUbNotifyNumPerThread);
...
Status HixlCSClient::ConvertAddr(bool is_server, const char *name, T addr, T &converted_addr) {
  ...
  if (region.is_host_mem) {
    ...
    HIXL_LOGI("[HixlClient][UB] Convert %s addr: %p -> %p (is_server=%d)", name, addr, converted_addr, is_server);
  }
  return SUCCESS;
}

Status HixlCSClient::BatchTransferByUboe(bool is_get, const CommunicateMem &communicate_mem_param,
                                         void **query_handle) {
  // 创建转换后的地址列表
  std::vector<void *> dst_list(communicate_mem_param.list_num);
  std::vector<const void *> src_list(communicate_mem_param.list_num);
  // 遍历所有 buffer 进行地址转换
  for (uint32_t i = 0; i < communicate_mem_param.list_num; i++) {
    // is_get=true: src是server, dst是client
    // is_get=false: src是client, dst是server
    HIXL_CHK_STATUS_RET(ConvertAddr(is_get, "src", communicate_mem_param.src_buf_list[i], src_list[i]),
                        "[HixlClient][UB] Convert src addr failed");

    HIXL_CHK_STATUS_RET(ConvertAddr(!is_get, "dst", communicate_mem_param.dst_buf_list[i], dst_list[i]),
                        "[HixlClient][UB] Convert dst addr failed");
  }

  // 设置转换后的参数
  CommunicateMem converted{communicate_mem_param.list_num, dst_list.data(), src_list.data(),
                           communicate_mem_param.len_list};
  return BatchTransferDevice(is_get, converted, query_handle);
}
...
Status HixlCSClient::BatchTransfer(bool is_get, const CommunicateMem &communicate_mem_param, void **query_handle) {
  ...
  if (ep.protocol == COMM_PROTOCOL_UBOE) {
    return BatchTransferByUboe(is_get, communicate_mem_param, query_handle);
  }
