...
Status Endpoint::Initialize() {
  ...
  HIXL_CHK_HCCL_RET(HcclProxy::HcommEndpointCreate(&endpoint_, &handle_));
...
Status Endpoint::Finalize() {
  ...
  for (const auto &it : reg_mems_) {
    ...
    auto hccl_ret = HcclProxy::HcommMemUnreg(handle_, mem_handle);
  ...
  reg_mems_.clear();
  auto hccl_ret = HcclProxy::HcommEndpointDestroy(handle_);
  if (hccl_ret != HCCL_SUCCESS) {
    ret = hixl::HcclError2Status(hccl_ret);
...
Status Endpoint::RegisterMem(const char *mem_tag, const HcommMem &mem, MemHandle &mem_handle) {
  std::lock_guard<std::mutex> lock(mutex_);
  Status ret = SUCCESS;
  HcclResult hccl_ret = HcclProxy::HcommMemReg(handle_, mem_tag, mem, &mem_handle);
...
Status Endpoint::DeregisterMem(MemHandle mem_handle) {
  ...
  HIXL_CHK_HCCL_RET(HcclProxy::HcommMemUnreg(handle_, mem_handle));
  reg_mems_.erase(it);
  return SUCCESS;
}
...
Status Endpoint::ExportMem(std::vector<HixlMemDesc> &mem_descs) {
  ...
  for (auto &it : reg_mems_) {
    ...
    if (mem.export_desc == nullptr) {
      HIXL_CHK_HCCL_RET(HcclProxy::HcommMemExport(handle_, mem_handle, &mem.export_desc, &mem.export_len));
    }
...
Status Endpoint::MemImport(const void *mem_desc, uint32_t desc_len, HcommMem &out_buf) {
  ...
  HIXL_CHK_HCCL_RET(HcclProxy::HcommMemImport(handle_, mem_desc, desc_len, &out_buf));
  return SUCCESS;
}
