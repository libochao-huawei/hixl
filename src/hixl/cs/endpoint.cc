...
Status Endpoint::Initialize() {
  HIXL_CHECK_NOTNULL(HcommEndpointCreate);
  HIXL_CHECK_NOTNULL(HcommEndpointDestroy);
  HIXL_CHECK_NOTNULL(HcommMemReg);
  HIXL_CHECK_NOTNULL(HcommMemUnreg);
  HIXL_CHECK_NOTNULL(HcommMemImport);
  HIXL_CHECK_NOTNULL(HcommMemExport);
  HIXL_CHECK_NOTNULL(HcommMemUnimport);
...
Status Endpoint::RegisterMem(const char *mem_tag, const HcommMem &mem, MemHandle &mem_handle) {
  ...
  if (hccl_ret != HCCL_SUCCESS and hccl_ret != HCCL_E_AGAIN) {
    ret = hixl::HcclError2Status(hccl_ret);
