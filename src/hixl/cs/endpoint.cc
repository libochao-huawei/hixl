...
std::string BuildChannelName(const EndpointDesc &endpoint, const ChannelDesc &channel_desc, uint32_t port) {
  ...
  if (channel_name.length() > HCCL_CHANNEL_NAME_MAX_LEN) {
    const std::string suffix = "_" + std::to_string(port) + "_" + std::to_string(channel_desc.channel_index);
...
Status Endpoint::CreateChannel(const ChannelDesc &channel_desc, ChannelHandle &channel_handle) {
  ...
  HcommChannelDesc ch_desc{};
  HIXL_CHK_STATUS_RET(InitChannelDesc(endpoint_, channel_desc, port_, ch_desc));

  ChannelPtr channel = MakeShared<Channel>();
  HIXL_CHECK_NOTNULL(channel);
  std::string channel_name = BuildChannelName(endpoint_, channel_desc, port_);
