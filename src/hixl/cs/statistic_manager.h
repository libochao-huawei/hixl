...
struct TransferStatisticInfo {
  ...
  CostStatisticInfo device_prepare_batch_mem;
  CostStatisticInfo device_prepare_remote_flag;
  CostStatisticInfo device_fill_args;
  CostStatisticInfo device_launch_kernel;
  CostStatisticInfo device_sync_wait;
...
struct ConnectStatisticInfo {
  CostStatisticInfo client_create;
...
struct TransferStatisticSnapshot {
  ...
  CostStatisticSnapshot device_prepare_batch_mem;
  CostStatisticSnapshot device_prepare_remote_flag;
  CostStatisticSnapshot device_fill_args;
  CostStatisticSnapshot device_launch_kernel;
  CostStatisticSnapshot device_sync_wait;
...
struct ConnectStatisticSnapshot {
  CostStatisticSnapshot client_create;
...
enum class StatisticStage : uint32_t {
  ...
  kCheckStatusHost,
  kCheckStatusDevice,
  kDevicePrepareBatchMem,
  kDevicePrepareRemoteFlag,
  kDeviceFillArgs,
  kDeviceLaunchKernel,
  kDeviceSyncWait,
  kServerInitialize,
...
class HixlCSStatisticManager {
 public:
  ...
  void RenameChannel(const std::string &old_channel_id, const std::string &new_channel_id);
