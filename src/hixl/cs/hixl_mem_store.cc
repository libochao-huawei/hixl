...
Status HixlMemStore::ConvertHostAddr(HixlOneSideOpDesc &desc, uint32_t &local_host_cnt,
  ...
  if (remote_region.is_host_mem) {
    ...
    HIXL_LOGD("[HixlMemStore] Convert remote addr: %p -> %p, region_size=%zu, copy_len=%lu", host_addr, desc.remote_buf,
  ...
  if (local_region.is_host_mem) {
    ...
    HIXL_LOGD("[HixlMemStore] Convert local addr: %p -> %p, region_size=%zu, copy_len=%lu", host_addr, desc.local_buf,
