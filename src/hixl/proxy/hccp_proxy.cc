...
Status EnsureLibRaLoaded() {
  ...
  void *h = dlopen(kLibRaSo, RTLD_NOW);
  if (h == nullptr) {
    HIXL_LOGE(FAILED, "[HccpProxy] dlopen %s failed: %s", kLibRaSo, dlerror());
    return FAILED;
  }
  ...
  if (get_handle == nullptr || get_ba == nullptr) {
    HIXL_LOGE(FAILED, "[HccpProxy] dlsym RaRdevGetHandle/RaGetNotifyBaseAddr failed: %s", dlerror());
    (void)dlclose(h);
    return FAILED;
  }
  lr.handle = h;
...
Status HccpProxy::RaGetNotifyAddrLen(int32_t device_id, aclrtNotify notify, uint64_t &notify_addr,
  ...
  const auto phy_id = static_cast<unsigned int>(phy_device_id);
  ...
  HIXL_LOGI("[HccpProxy] Ra notify: phy_id=%u base=0x%llx offset=0x%llx va=0x%llx len=%u", phy_id,
            static_cast<unsigned long long>(base_va), static_cast<unsigned long long>(offset),
            static_cast<unsigned long long>(notify_addr), notify_len);
  return SUCCESS;
}
