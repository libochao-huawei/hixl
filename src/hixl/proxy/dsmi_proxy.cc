...
Status DsmiProxy::GetInterconType(int32_t device_id, uint32_t &intercon_type) {
  ...
  constexpr unsigned int kMinSpodInfoSize =
      static_cast<unsigned int>(offsetof(DsmiSpodInfo, super_pod_intercon_type) + sizeof(unsigned int));
...
void DsmiProxy::Reset() {
  LibDrvdsmiHostLoader &ldr = LibDrvdsmiHost();
  std::lock_guard<std::mutex> lock(ldr.mu);
  ldr.handle = nullptr;
  ldr.dsmi_get_board_info = nullptr;
  ldr.dsmi_get_device_info = nullptr;
}
