...
Status CheckOptions(const std::map<AscendString, AscendString> &options) {
  static std::unordered_set<std::string> kOptionsFields = {
      hixl::OPTION_LOCAL_COMM_RES,        hixl::OPTION_BUFFER_POOL,        adxl::OPTION_LOCAL_COMM_RES,
      adxl::OPTION_BUFFER_POOL,           hixl::OPTION_RDMA_TRAFFIC_CLASS, adxl::OPTION_RDMA_TRAFFIC_CLASS,
      hixl::OPTION_RDMA_SERVICE_LEVEL,    adxl::OPTION_RDMA_SERVICE_LEVEL, hixl::OPTION_ENABLE_USE_FABRIC_MEM,
  ...
  for (const auto &pair : options) {
    HIXL_CHK_BOOL_RET_SPECIAL_STATUS(kOptionsFields.find(pair.first.GetString()) == kOptionsFields.end(), PARAM_INVALID,
                                     ...
                                     "OPTION_RDMA_SERVICE_LEVEL, OPTION_ENABLE_USE_FABRIC_MEM and "
    ...
    if ((pair.first == hixl::OPTION_BUFFER_POOL) || (pair.first == adxl::OPTION_BUFFER_POOL)) {
      HIXL_CHK_BOOL_RET_STATUS(pair.second.GetString() == std::string("0:0"), PARAM_INVALID,
