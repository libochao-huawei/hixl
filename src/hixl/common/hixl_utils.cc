...
std::string ProtocolToString(CommProtocol protocol) {
  switch (protocol) {
    ...
    default:
      return "UNKNOWN(" + std::to_string(static_cast<int>(protocol)) + ")";
  }
}
...
std::string FormatCommAddr(const CommAddr &addr) {
  switch (addr.type) {
    ...
    case COMM_ADDR_TYPE_EID: {
      ...
      (void)snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "EID[%016llx:%016llx]",
                       static_cast<unsigned long long>(subnet_prefix), static_cast<unsigned long long>(interface_id));
      return std::string(buf);
    }
    default:
      return "UNKNOWN";
