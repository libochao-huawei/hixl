...
void ThreadPool::ThreadFunc(ThreadPool *const thread_pool, uint32_t thread_idx, bool is_temporary) {
  ...
  while (!thread_pool->is_stoped_) {
    std::function<void()> task;
    {
      ...
      if (thread_pool->is_stoped_ && thread_pool->tasks_.empty()) {
        if (is_temporary) {
          ...
          HIXL_LOGI("[ThreadPool:%s] temp thread exit on stop, idx:%u, idle:%u, busy:%u, total:%u",
                    thread_pool->thread_name_prefix_.c_str(), thread_idx,
                    thread_pool->idle_thrd_num_.load(), thread_pool->busy_thrd_num_.load(),
                    thread_pool->total_thrd_num_.load());
        }
        return;
      }
    ...
    if (is_temporary && !thread_pool->is_stoped_) {
      --thread_pool->total_thrd_num_;
      HIXL_LOGI("[ThreadPool:%s] temp thread exit after task, idx:%u, idle:%u, busy:%u, total:%u",
                thread_pool->thread_name_prefix_.c_str(), thread_idx,
                thread_pool->idle_thrd_num_.load(), thread_pool->busy_thrd_num_.load(),
                thread_pool->total_thrd_num_.load());
      return;
