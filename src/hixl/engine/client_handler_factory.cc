...
Status CreateCsClient(const EndpointConfig &local, const EndpointConfig &remote, CommType type,
                      std::map<CommType, HixlClientHandle> &handles, const std::string &ip, uint32_t port, uint8_t tc,
                      uint8_t sl) {
...
Status TryMatchUb(const EndpointConfig &local, const std::map<MatchKey, EndpointConfig> &peers,
                  std::map<CommType, bool> &expected, uint32_t &count, std::map<CommType, HixlClientHandle> &handles,
                  const std::string &ip, uint32_t port, uint8_t tc, uint8_t sl) {
  if (local.protocol != kProtocolUbCtp && local.protocol != kProtocolUbTp) {
    return SUCCESS;
  }
...
Status FindMatchedEndpoints(const HandlerCreateArgs &args, std::map<CommType, HixlClientHandle> &handles) {
  // 优先尝试uboe协议连接。
  if (TryMatchUboe(args.local_endpoints, args.remote_endpoints, handles, args.server_ip, args.server_port, args.rdma_tc,
                   args.rdma_sl) == SUCCESS) {
    return SUCCESS;
  }
  // 如果必须使用ROCE，直接匹配并创建ROCE链路
  if (MustUseRoce(args.local_endpoints, args.remote_endpoints)) {
    return TryMatchRoce(args.local_endpoints, args.remote_endpoints, handles, args.server_ip, args.server_port,
                        args.rdma_tc, args.rdma_sl);
  }
  // 匹配创建UB链路
  ...
  for (const auto &ep : args.local_endpoints) {
    HIXL_CHK_STATUS_RET(
        TryMatchUb(ep, peers, expected, count, handles, args.server_ip, args.server_port, args.rdma_tc, args.rdma_sl));
  ...
  if (TryMatchHccs(args.local_endpoints, args.remote_endpoints, handles, args.server_ip, args.server_port, args.rdma_tc,
                   args.rdma_sl) == SUCCESS) {
    return SUCCESS;
  }
