  ...
  if (endpoint_list_.empty()) {
    ...
    if (find_ret != options.cend()) {
      ...
      if (config.protocol_desc.size() == 1 && config.protocol_desc[0] == "uboe:device") {
        ...
        uboe_endpoint.protocol = "uboe";
        uboe_endpoint.comm_id = uboe_ip + ":16668";
        uboe_endpoint.placement = "device";
        uboe_endpoint.net_instance_id = "stub_superpod1_1";
        endpoint_list_.emplace_back(uboe_endpoint);
      }
    } else {
