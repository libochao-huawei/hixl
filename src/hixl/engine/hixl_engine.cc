...
Status HixlEngine::TransferSync(const AscendString &remote_engine, TransferOp operation,
  ...
  HIXL_CHK_BOOL_RET_STATUS(client_ptr != nullptr, 
                           ...
                           local_engine_.c_str(), remote_engine.GetString());
  if (operation == READ) {
    HIXL_API_PROFILING(HixlProfType::HixlProfTypeRead);
  } else {
    HIXL_API_PROFILING(HixlProfType::HixlProfTypeWrite);
  }
...
Status HixlEngine::TransferAsync(const AscendString &remote_engine, TransferOp operation,
  ...
  start_time = HixlProfilingReporter::GetStartTime(start_time);
...
Status HixlEngine::GetTransferStatus(const TransferReq &req, TransferStatus &status) {
  ...
  if (status == TransferStatus::COMPLETED) {
    ...
    if (op_type == READ) {
      HIXL_API_PROFILING_WITH_TIME(HixlProfType::HixlProfTypeRead, start_time);
    } else {
      HIXL_API_PROFILING_WITH_TIME(HixlProfType::HixlProfTypeWrite, start_time);
    }
    req_map_.erase(it);
  }
  return SUCCESS;
