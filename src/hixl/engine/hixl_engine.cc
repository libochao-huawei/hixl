...
Status HixlEngine::Initialize(const std::map<AscendString, AscendString> &options) {
  ...
  HIXL_CHK_STATUS_RET(ParseListenInfo(local_engine_, ip, port), 
                      "[HixlEngine] Failed to parse ip and port, local_engine_:%s",
                      local_engine_.c_str());
...
Status HixlEngine::RegisterMem(const MemDesc &mem, MemType type, MemHandle &mem_handle) {
  ...
  HIXL_CHK_STATUS_RET(server_.RegisterMem(mem, type, mem_handle), 
                      "[HixlEngine] Failed to register mem, type:%d, addr:%p, size:%lu",
                      static_cast<int32_t>(type), reinterpret_cast<void *>(mem.addr), mem.len);
...
Status HixlEngine::Connect(const AscendString &remote_engine, int32_t timeout_in_millis) {
  ...
  atuo ret = client_manager_.CreateClient(endpoint_list_, remote_engine.GetString(), client_ptr);
  if (ret != SUCCESS) {
    HIXL_LOGE(FAILED, "[HixlEngine] Failed to create HixlClient, local_engine: %s, remote engine: %s",
                      local_engine_.c_str(), remote_engine.GetString());
    client_ptr = nullptr;
    client_manager_.DestroyClient(remote_engine.GetString());
    return FAILED;
  }
  HIXL_CHECK_NOTNULL(client_ptr, "[HixlEngine] Created client is null, please check your parameters! remote_engine:%s",
  ...
  HIXL_CHK_STATUS_RET(client_ptr->SetLocalMemInfo(mem_info_list), "[HixlEngine] Failed to set local memory info");
