...
Status HixlEngine::GenDefaultEndpointConfig(const std::map<AscendString, AscendString> &options,
  ...
  if (find_ret != options.cend()) {
    ...
    if (config.protocol_desc.size() == 1 && config.protocol_desc[0] == "uboe:device") {
    ...
    HIXL_LOGE(PARAM_INVALID, "option[%s] is invalid, value = %s, only support [\"uboe:device\"] now.",
              OPTION_GLOBAL_RESOURCE_CONFIG, find_ret->second.GetString());
  }
...
Status HixlEngine::GenDefaultUboeEndpointConfig(EndpointConfig &endpoint_config) {
  ...
  // 端口默认16668
  endpoint_config.comm_id = uboe_ip + ":16668";
  endpoint_config.placement = kPlacementDevice;
  endpoint_config.net_instance_id = "stub_superpod1_1";
  return SUCCESS;
}
}  // namespace hixl
