...
class Hixl::HixlImpl {
 public:
  ...
  Status GetTransferStatus(std::vector<TransferStatusResult> &out_results, const GetTransferStatusOptions &options);
...
Status Hixl::HixlImpl::GetTransferStatus(std::vector<TransferStatusResult> &out_results,
                                         const GetTransferStatusOptions &options) {
  HIXL_CHK_BOOL_RET_STATUS(engine_ != nullptr, FAILED, "engine is nullptr, check engine init");
  auto ret = engine_->GetTransferStatus(out_results, options);
...
Status Hixl::GetTransferStatus(std::vector<TransferStatusResult> &out_results,
                               const GetTransferStatusOptions &options) {
  HIXL_CHK_BOOL_RET_STATUS(impl_ != nullptr, FAILED, "Impl is nullptr, check Hixl init.");
  const auto ret = impl_->GetTransferStatus(out_results, options);
  HIXL_CHK_BOOL_RET_STATUS(ret == SUCCESS, ret, "Failed to get transfer status");
  return SUCCESS;
}
