...
Status Hixl::HixlImpl::GetTransferStatus(std::map<TransferReq, TransferStatus> &statuses) {
  HIXL_CHK_BOOL_RET_STATUS(engine_ != nullptr, FAILED, "engine is nullptr, check engine init");
  auto ret = engine_->GetTransferStatus(statuses);
...
Status Hixl::GetTransferStatus(std::map<TransferReq, TransferStatus> &statuses) {
  HIXL_CHK_BOOL_RET_STATUS(impl_ != nullptr, FAILED, "Impl is nullptr, check Hixl init.");
  const auto ret = impl_->GetTransferStatus(statuses);
  HIXL_CHK_BOOL_RET_STATUS(ret == SUCCESS, ret, "Failed to get transfer status");
  return SUCCESS;
}
