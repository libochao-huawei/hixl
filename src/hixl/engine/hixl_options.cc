...
constexpr uint32_t kMinResourceLimit = 1U;
...
void from_json(const nlohmann::json &j, CommResourceConfigDesc &cfg) {
  ...
  if (j.contains("comm_resource_config.resource_limit")) {
    cfg.resource_limit = JsonToNumber<uint32_t>(j.at("comm_resource_config.resource_limit"));
  }
}
...
Status HixlOptions::ParseGlobalResourceConfig(const std::string &config_str) {
  try {
    ...
    if (cfg.comm_resource_config.resource_limit.has_value()) {
      uint32_t val = *cfg.comm_resource_config.resource_limit;
      HIXL_CHK_BOOL_RET_STATUS(val >= kMinResourceLimit && val <= TransferPool::kMaxPoolSize, PARAM_INVALID,
                               "comm_resource_config.resource_limit must be in [%u, %u], got %u", kMinResourceLimit,
                               TransferPool::kMaxPoolSize, val);
    }
