...
Status HixlOptions::ParseRdmaOptions(const std::map<AscendString, AscendString> &options) {
  ...
  if (!traffic_class_str.empty()) {
    ...
    HIXL_LOGI("Set rdma traffic class to %u", rdma_traffic_class_.value());
  }
  ...
  if (!service_level_str.empty()) {
    ...
    HIXL_LOGI("Set rdma service level to %u", rdma_service_level_.value());
  }
  HIXL_EVENT("ParseRdmaOptions success: traffic_class=%u, service_level=%u", rdma_traffic_class_.value_or(0),
             rdma_service_level_.value_or(0));
  return SUCCESS;
}
