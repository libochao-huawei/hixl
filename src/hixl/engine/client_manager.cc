...
namespace hixl {
Status ClientManager::CreateClient(const std::vector<EndpointConfig> &endpoint_list, const std::string &remote_engine,
                                   ClientPtr &client_ptr) {
  std::string ip;
  int32_t port = 0;
  HIXL_CHK_STATUS_RET(ParseListenInfo(remote_engine, ip, port), "Failed to parse ip, remote_engine:%s",
                      remote_engine.c_str());
