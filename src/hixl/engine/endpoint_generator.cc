...
Status EndpointGenerator::BuildEndpointListFromOptions(const std::map<AscendString, AscendString> &options,
  ...
  bool has_non_empty_endpoint_list = false;
  ...
  if (local_comm_res_cstr != nullptr && local_comm_res_cstr[0] != '\0') {
    ...
    has_non_empty_endpoint_list = HasNonEmptyEndpointList(config);
    HIXL_CHK_STATUS_RET(BuildEndpointListFromLocalCommRes(config, has_non_empty_endpoint_list, local_engine,
                                                          endpoint_list),
    ...
    if (is_uboe_enabled) {
      ...
      HIXL_CHK_STATUS_RET(BuildEndpointListFromLocalCommRes(config, false, local_engine, endpoint_list),
                          "BuildEndpointListFromLocalCommRes failed");
    }
  }

  if (is_uboe_enabled && !has_non_empty_endpoint_list) {
...
Status EndpointGenerator::BuildEndpointListFromLocalCommRes(const nlohmann::json &config,
                                                            bool has_non_empty_endpoint_list,
                                                            const std::string &local_engine,
                                                            std::vector<EndpointConfig> &endpoint_list) {
  SocType soc_type = SocType::kOther;
  HIXL_CHK_STATUS_RET(GetSocType(soc_type), "GetSocType failed");
  const bool auto_generate =
      (soc_type == SocType::kV2 || soc_type == SocType::kV3) && config.is_object() && !has_non_empty_endpoint_list;
  ...
  } else if (has_non_empty_endpoint_list) {
