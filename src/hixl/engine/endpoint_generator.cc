...
Status EndpointGenerator::AutoGenEndpointList(const HixlOptions &options, const std::string &local_engine,
  ...
  HIXL_CHK_STATUS_RET(FilterEndpointListByProtocolDesc(options, endpoint_list),
                      "FilterEndpointListByProtocolDesc failed");
  return FillLocalDeviceInfo(endpoint_list);
}
...
Status EndpointGenerator::ParseLocalCommRes(const nlohmann::json &config, std::vector<EndpointConfig> &endpoint_list) {
  ...
  HIXL_CHK_STATUS_RET(FillLocalDeviceInfo(endpoint_list), "FillLocalDeviceInfo failed");
  return SUCCESS;
}
...
Status EndpointGenerator::FillLocalDeviceInfo(std::vector<EndpointConfig> &endpoint_list) {
