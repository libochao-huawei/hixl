...
Status HixlClient::SendHeartbeat(bool &need_retry) {
  ...
  std::lock_guard<std::mutex> lock(ctrl_socket_mutex_);
  if (ctrl_socket_ < 0) {
    HIXL_LOGE(FAILED, "[HixlClient] SendHeartbeat failed, ctrl socket is invalid, fd=%d", ctrl_socket_);
    return FAILED;
  }
  CtrlMsgHeader header{};
  header.magic = kMagicNumber;
  ...
  HIXL_CHK_STATUS_RET(ret, "[HixlClient] SendHeartbeat header failed, fd=%d", ctrl_socket_);
  ...
  HIXL_CHK_STATUS_RET(ret, "[HixlClient] SendHeartbeat msg_type failed, fd=%d", ctrl_socket_);
  return SUCCESS;
}
...
Status HixlClient::SendNotify(const NotifyDesc &notify, int32_t timeout_ms) {
  ...
  std::lock_guard<std::mutex> lock(ctrl_socket_mutex_);
  if (ctrl_socket_ < 0) {
    HIXL_LOGE(FAILED, "HixlClient SendNotify failed, ctrl socket is invalid");
    return FAILED;
  }
