...
Status HixlClient::Initialize(const std::vector<EndpointConfig> &local_endpoint_list) {
  ...
  CtrlMsgPlugin::Initialize();
  {
    ...
    HIXL_DISMISSABLE_GUARD(socket_guard, ([this]() {
      if (ctrl_socket_ >= 0) {
        HIXL_LOGI("HixlClient close ctrl_socket start, socket:%d", ctrl_socket_);
        close(ctrl_socket_);
        HIXL_LOGI("HixlClient close ctrl_socket end, socket:%d", ctrl_socket_);
        ctrl_socket_ = -1;
      }
    }));
...
Status HixlClient::SendNotifyInternal(uint64_t req_id, const NotifyDesc &notify, int32_t timeout_ms) {
  ...
  HIXL_LOGI("HixlClient sent NotifyMsg, req_id:%lu, name:%s, socket:%d", req_id, notify_msg.name.c_str(), ctrl_socket_);

  HIXL_CHK_STATUS_RET(RecvNotifyAck(ctrl_socket_, req_id, timeout_ms),
