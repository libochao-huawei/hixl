...
void EndpointMatcher::LogMatchedEndpoints(const std::vector<HandlerCreateArgs::EndpointPair> &pairs,
                                          HandlerCreateArgs::HandlerType handler_type) {
  HIXL_LOGI("EndpointMatcher selected handler:%s, matched pairs:%zu", HandlerTypeToString(handler_type),
            pairs.size());
  for (size_t i = 0; i < pairs.size(); ++i) {
    const auto &pair = pairs[i];
    HIXL_LOGI("EndpointMatcher pair[%zu], protocol:%s, placement:%s, comm_type:%s", i,
