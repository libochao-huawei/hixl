...
Status BuildAddrInfo(const MemDesc &mem, MemType type, AddrInfo &addr_info) {
  ...
  HIXL_CHK_BOOL_RET_STATUS(mem.len <= max_addr && mem.addr <= max_addr - mem.len, PARAM_INVALID,
...
Status FabricMemEngine::SendNotify(const AscendString &remote_engine, const NotifyDesc &notify,
  ...
  (void)notify;
  (void)timeout_in_millis;
  HIXL_LOGE(UNSUPPORTED, "[FabricMemEngine] Method SendNotify is not supported.");
  return UNSUPPORTED;
}

Status FabricMemEngine::GetNotifies(std::vector<NotifyDesc> &notifies) {
  (void)notifies;
  HIXL_LOGE(UNSUPPORTED, "[FabricMemEngine] Method GetNotifies is not supported.");
  return UNSUPPORTED;
}
