...
Status FabricMemEngine::CheckOptions(const std::map<AscendString, AscendString> &options) {
  ...
  for (const auto &pair : options) {
    if (kOptionsFields.find(pair.first.GetString()) == kOptionsFields.end()) {
      HIXL_LOGE(PARAM_INVALID, "Invalid option '%s' is not supported, fabric mem engine only supports "
                               "OPTION_ENABLE_USE_FABRIC_MEM and OPTION_GLOBAL_RESOURCE_CONFIG",
...
Status FabricMemEngine::Initialize(const std::map<AscendString, AscendString> &options) {
  ...
  HIXL_CHK_STATUS_RET(CheckOptions(options), "[FabricMemEngine] Failed to check options.");
