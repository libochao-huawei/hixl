...
Status HixlServer::Initialize(const std::string &ip, int32_t port,
                              ...
                              std::optional<uint32_t> listen_port, std::optional<uint32_t> resource_limit) {
  ...
  if (listen_port.has_value() || resource_limit.has_value()) {
    nlohmann::json json;
    if (listen_port.has_value()) {
      json["comm_resource_config.listen_port"] = listen_port.value();
    }
    if (resource_limit.has_value()) {
      json["comm_resource_config.resource_limit"] = resource_limit.value();
    }
