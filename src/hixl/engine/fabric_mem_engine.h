...
class FabricMemEngine : public hixl::Engine {
 ...
 private:
  ...
  class OperationGuard {
   public:
    explicit OperationGuard(FabricMemEngine &engine) : engine_(engine), acquired_(engine_.TryAcquireOperation()) {}
    ~OperationGuard() {
      if (acquired_) {
  ...
  Status DisconnectLocked(const AscendString &remote_engine, int32_t timeout_in_millis, bool wait_in_flight = true);
  ...
  Status GetRtContext(std::shared_ptr<void> &ctx_holder, aclrtContext &ctx);
  void GetOptionalRtContext(std::shared_ptr<void> &ctx_holder, aclrtContext &ctx);
