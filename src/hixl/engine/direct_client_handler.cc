...
DirectClientHandler::DirectClientHandler(std::map<CommType, HixlClientHandle> handles) : handles_(std::move(handles)) {}
...
Status DirectClientHandler::Create(const HandlerCreateArgs &args, std::unique_ptr<DirectClientHandler> &out) {
  ...
  out = MakeUnique<DirectClientHandler>(std::move(handles));
  HIXL_CHECK_NOTNULL(out, "DirectClientHandler create failed");
  return SUCCESS;
}

HixlClientHandle DirectClientHandler::SingleHandle() const {
  return handles_.begin()->second;
}
CommType DirectClientHandler::SingleType() const {
  return handles_.begin()->first;
}
...
Status DirectClientHandler::GetTransferStatus(const TransferReq &req, TransferStatus &status) {
  ...
  Status ret = static_cast<Status>(HixlCSClientQueryCompleteStatus(SingleHandle(), it->second, &cs));
  if (ret != SUCCESS) {
    status = TransferStatus::FAILED;
