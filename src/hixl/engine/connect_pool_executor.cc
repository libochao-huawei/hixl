...
Status ConnectPoolExecutor::Initialize(const std::map<AscendString, AscendString> &options) {
  ...
  HIXL_CHK_BOOL_RET_STATUS(
      ...
      PARAM_INVALID, "task_queue_capacity:%d must in [%d, %d]", thread_num_, kLimitTaskQueueCapacityMin,
...
void ConnectPoolExecutor::WorkerHandler(const int32_t worker_id) {
  ...
  const aclError acl_ret = aclrtSetCurrentContext(ctx_);
  if (acl_ret != ACL_SUCCESS) {
    HIXL_REPORT_ERR_MSG("E19999", "Call aclrtSetCurrentContext fail, ret: 0x%X", static_cast<uint32_t>(acl_ret));
    HIXL_LOGE(FAILED, "ConnectPoolExecutor worker %d start failed", worker_id);
    return;
  }
  while (true) {
