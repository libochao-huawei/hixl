...
constexpr uint32_t kReplenishStreamRetryCount = 3U;
...
Status FabricMemTransferService::Transfer(const FabricMemTransferContext &context, TransferOp operation,
  ...
  HIXL_DISMISSABLE_GUARD(fail_guard, ([this, &streams]() { ReleaseStreams(streams); }));
...
Status FabricMemTransferService::TransferAsync(const FabricMemTransferContext &context, TransferOp operation,
  ...
  HIXL_DISMISSABLE_GUARD(fail_guard, ([this, &slot]() { ReleaseAsyncSlot(slot, false); }));
...
void FabricMemTransferService::ReplenishStreamsToMaxLocked() {
  while (stream_pool_.size() < max_stream_num_) {
    aclrtStream new_stream = nullptr;
    aclError ret = ACL_ERROR_NONE;
    for (uint32_t retry = 0U; retry < kReplenishStreamRetryCount; ++retry) {
      ret = aclrtCreateStreamWithConfig(&new_stream, 0, ACL_STREAM_FAST_LAUNCH | ACL_STREAM_FAST_SYNC);
      if (ret == ACL_ERROR_NONE) {
        break;
      }
    }
    if (ret != ACL_ERROR_NONE) {
      HIXL_LOGE(FAILED, "Replenish fabric mem stream failed, current:%zu, max:%zu, ret:%d.", stream_pool_.size(),
                max_stream_num_, ret);
      return;
    }
    stream_pool_[new_stream] = true;
  }
}

void FabricMemTransferService::AbortAndReplenishStreamLocked(aclrtStream stream) {
  if (stream == nullptr) {
    return;
  }
  ...
  ReplenishStreamsToMaxLocked();
}

void FabricMemTransferService::ReleaseAsyncSlot(AsyncSlot &slot, bool abort_and_replenish) {
  std::lock_guard<std::mutex> lock(stream_pool_mutex_);
  if (abort_and_replenish) {
    for (const auto &stream : slot.streams) {
      AbortAndReplenishStreamLocked(stream);
    }
  } else {
    ReturnStreamsLocked(slot.streams);
  }
