...
Status FabricMemControlClient::Fetch(const std::string &remote_engine, int32_t timeout_ms,
  ...
  Status ret = ParseListenInfo(remote_engine, remote_ip, remote_port);
  if (ret != SUCCESS) return ret;
  if (remote_port <= 0) return PARAM_INVALID;
  CtrlMsgPlugin::Initialize();
  int32_t conn_fd = -1;
  ret = CtrlMsgPlugin::Connect(remote_ip, static_cast<uint32_t>(remote_port), conn_fd, timeout_ms);
  if (ret != SUCCESS) return ret;
  HIXL_MAKE_GUARD(close_fd, ([conn_fd]() { (void)close(conn_fd); }));
  ret = SendTypedMsg(conn_fd, CtrlMsgType::kGetFabricMemInfoReq, "");
  if (ret != SUCCESS) return ret;

  CtrlMsgType msg_type{};
  std::string payload;
  ret = RecvTypedMsg(conn_fd, timeout_ms, msg_type, payload);
  if (ret != SUCCESS) return ret;
  if (msg_type != CtrlMsgType::kGetFabricMemInfoResp) return PARAM_INVALID;
  try {
    auto json = nlohmann::json::parse(payload);
