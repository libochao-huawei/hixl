...
void CheckShareHandleArray(const nlohmann::json &share_array) {
  if (!share_array.is_array() || share_array.size() != kShareHandleDataSize) {
    throw nlohmann::json::out_of_range::create(
        401, "share_handle size must be " + std::to_string(kShareHandleDataSize), &share_array);
  }
}
...
Status FabricMemControlServer::HandleConnection(int32_t fd) {
  ...
  HIXL_CHK_BOOL_RET_STATUS(msg_type == CtrlMsgType::kGetFabricMemInfoReq, PARAM_INVALID,
                           "Unexpected fabric mem request type:%d.", static_cast<int32_t>(msg_type));
  (void)payload;
