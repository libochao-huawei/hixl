...
void FabricMemControlServer::RegisterClientSession(const std::shared_ptr<State> &state, int32_t epoll_fd, int32_t fd,
  ...
  // Self-loopback: client fd already sends heartbeat; do not close the paired server fd on
  // receive-timeout, otherwise the client fd will fail with "Outbound heartbeat failed".
  const bool is_self_loopback =
      !client_id.empty() && !state->local_engine_id.empty() && client_id == state->local_engine_id;
  session.with_heartbeat = !is_self_loopback;
  if (session.with_heartbeat) {
