...
Status ValidateAdxlHeader(const FabricMemAdxlProtocolHeader &header) {
  HIXL_CHK_BOOL_RET_STATUS(header.magic == kFabricMemAdxlMagic, PARAM_INVALID, "Invalid adxl magic:0x%x.",
                           header.magic);
  HIXL_CHK_BOOL_RET_STATUS(header.body_size >= sizeof(int32_t) && header.body_size <= kMaxAdxlSessionBodySize,
                           PARAM_INVALID, "Invalid adxl body size:%lu.", header.body_size);
  return SUCCESS;
}
...
Status FabricMemControlClient::Fetch(const std::string &remote_engine, const std::string &channel_id,
  ...
  HIXL_CHK_STATUS_RET(PrepareKeepaliveFd(conn_fd), "Failed to prepare keepalive fd:%d.", conn_fd);
  HIXL_LOGI("Fetch received %zu share handles from remote:%s, conn_fd:%d.", share_handles.size(), remote_engine.c_str(),
            conn_fd);
  return SUCCESS;
}

Status FabricMemControlClient::PrepareKeepaliveFd(int32_t conn_fd) {
  HIXL_CHK_STATUS_RET(SetNonBlocking(conn_fd), "Failed to set non-blocking mode on keepalive fd:%d.", conn_fd);
  FabricMemAdxlControl::DrainInboundData(conn_fd);
  return SUCCESS;
}
