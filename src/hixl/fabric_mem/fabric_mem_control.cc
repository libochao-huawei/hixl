...
Status FabricMemControlClient::FetchOld(const std::string &remote_engine, int32_t timeout_ms,
  ...
  // Build minimal old-protocol ChannelConnectInfo
  nlohmann::json req;
  req["channel_id"] = "";
  req["comm_res"] = "";
  ...
  // Send kConnect (msg_type=1)
  Status send_ret = SendTypedMsg(conn_fd, static_cast<CtrlMsgType>(kOldConnectMsgType), req.dump());
  if (send_ret != SUCCESS) {
    (void)close(conn_fd);
    conn_fd = -1;
    return send_ret;
  ...
  // Receive kConnect response (msg_type=1)
  CtrlMsgType msg_type{};
  std::string payload;
  Status recv_ret = RecvTypedMsg(conn_fd, timeout_ms, msg_type, payload);
  if (recv_ret != SUCCESS || static_cast<int32_t>(msg_type) != kOldConnectMsgType) {
    (void)close(conn_fd);
    conn_fd = -1;
    HIXL_CHK_STATUS_RET(recv_ret, "Recv old connect response failed.");
    HIXL_LOGE(PARAM_INVALID, "Expected kConnect(1), got %d.", static_cast<int32_t>(msg_type));
    return PARAM_INVALID;
  }

  // Parse ChannelConnectInfo to extract share_handles
  try {
    auto json = nlohmann::json::parse(payload);
    share_handles.clear();
  ...
  // Receive kStatus (msg_type=3)
  recv_ret = RecvTypedMsg(conn_fd, timeout_ms, msg_type, payload);
  if (recv_ret != SUCCESS) {
    (void)close(conn_fd);
    conn_fd = -1;
    return recv_ret;
  ...
  // conn_fd is kept open — caller owns the lifecycle for heartbeat
  return SUCCESS;
}
