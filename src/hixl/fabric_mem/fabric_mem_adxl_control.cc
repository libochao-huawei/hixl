...
void FabricMemAdxlControl::DrainInboundData(int32_t fd) {
  if (fd < 0) {
    return;
  }
  constexpr size_t kDrainChunkSize = 4096U;
  char buffer[kDrainChunkSize];
  while (true) {
    const ssize_t n = recv(fd, buffer, kDrainChunkSize, 0);
    if (n > 0) {
      continue;
    }
    if (n == 0) {
