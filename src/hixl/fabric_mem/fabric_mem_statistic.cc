...
void FabricMemStatistic::UpdateCost(uint64_t cost, FabricMemCostStatisticInfo &cost_info) {
  ...
  while (current_max < cost && !cost_info.max_cost.compare_exchange_weak(current_max, cost,
...
void FabricMemStatistic::UpdateCosts(const std::string &channel_id, uint64_t transfer_cost, uint64_t real_copy_cost,
  ...
  if (info->transfer.times.load(std::memory_order_relaxed) > kResetTimes) {
    info->Reset();
  }
}
...
void FabricMemStatistic::Dump() const {
  ...
  for (const auto &item : transfer_statistic_info_) {
    ...
    HIXL_EVENT(
        ...
        "avg size:%lu kBytes, avg bandwidth:%.4f GB/s, max cost:%lu us, avg cost:%lu us, real copy times:%lu, "
...
Status FabricMemStatistic::StartPeriodicDump() {
  return dump_task_.Start(std::chrono::milliseconds(kStatisticTimerPeriodMs), [this]() { Dump(); });
}
