...
Status FabricMemChannelManager::DisconnectRemote(const AscendString &remote_engine, int32_t timeout_in_millis) {
  ...
  if (keepalive_fd >= 0) {
    const Status disconnect_ret = FabricMemControlClient::Disconnect(remote, local_engine, timeout_in_millis);
    if (disconnect_ret != SUCCESS) {
      HIXL_LOGW("[FabricMemChannelManager] Failed to notify remote disconnect, remote:%s, ret:%u.", remote.c_str(),
                static_cast<uint32_t>(disconnect_ret));
    }
    (void)close(keepalive_fd);
  }
...
void FabricMemChannelManager::DisconnectDeadRemote(const std::string &remote) {
  ...
  const Status ret = DisconnectRemote(AscendString(remote.c_str()), 0);
  if (ret == SUCCESS) {
    HIXL_LOGI("[FabricMemChannelManager] Auto-disconnected dead remote:%s due to keepalive failure.", remote.c_str());
  } else if (ret != NOT_CONNECTED) {
    HIXL_LOGW("[FabricMemChannelManager] Failed to auto-disconnect dead remote:%s, ret:%u.", remote.c_str(),
              static_cast<uint32_t>(ret));
  }
}
