...
# 读取/etc/hccl_rootinfo.json判断server或pod模式
TOPO_PATH=$(jq -r '.topo_file_path' /etc/hccl_rootinfo.json 2>/dev/null)
if [[ -z "$TOPO_PATH" || "$TOPO_PATH" == "null" ]]; then
    echo "警告: 无法读取/etc/hccl_rootinfo.json，使用默认server模式"
    IS_SERVER=1
elif [[ "$TOPO_PATH" == *"atlas_950"* ]]; then
    IS_SERVER=0
    echo "检测到pod模式 (atlas_950)"
else
    IS_SERVER=1
    echo "检测到server模式 (atlas_850)"
fi
